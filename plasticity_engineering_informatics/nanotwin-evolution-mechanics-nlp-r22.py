...
# ███ + PLASTICITY PARAMETER INTELLIGENT RECOMMENDER v10.1.0              ███
...
# ███   enter as grey non-ranked context bars.                        ███
# ███ FIX (v9.3.1): HONEST NAMING PASS — fine key `derived` RENAMED   ███
# ███   to `physics_inferred`.                                        ███
# ███ NEW (v10.0.0): PHYSICS-THREAD MoE + STACKED XAI CHART           ███
# ███   · PlasticityCandidate gains `moe_breakdown`.                  ███
# ███   · PlasticityLatentMoEScorer rebalanced across TEN experts:    ███
# ███       v9.x core    : Material, Thermal, Strain, Method,        ███
# ███                       Confidence, Reasoning                     ███
# ███       v10.x threads: Framework (A), Genesis (B),                 ███
# ███                       Thermo-Mech (C), Corpus Density            ███
# ███   · NEW `plot_stacked_latentmoe_v10` — exact deterministic       ███
# ███     feature attribution (XAI).                                   ███
# ███ NEW (v10.1.0): FIVE-PARAMETER PHYSICS-GATED MoE                  ███
# ███   · Every solver-consumed parameter (ρ₀, μ, γ̇₀, m, σ₀) now has  ███
# ███     its own user-selectable regime expert.                       ███
# ███   · Weight vector UNCHANGED at 1.00 — only expert outputs are    ███
# ███     scaled, so the stacked-XAI chart stays exact.                ███
# ███   · Three new experts:                                           ███
# ███       Sigma0ProcessingExpert   (σ₀, Genesis thread, 20 MPa fall) ███
# ███       MOrientationExpert       (m,  Thermo-Mech,    8-unit fall) ███
# ███       MuFrameworkExpert        (μ,  Framework,      8 GPa fall)  ███
# ███   · Existing PhysicsRegimeExpert falloff tightened 1.0 → 0.6     ███
# ███     decades; TheoryRegimeExpert 1.5 → 0.8 decades — sharper      ███
# ███     discrimination without over-fitting.                         ███
# ███   · Pre-Analysis Context sidebar block — five expanders in a     ███
# ███     2-col grid with live regime-preview metrics.                 ███
# ███   · Histogram customization: value labels, individual mode,      ███
# ███     dynamic green dotted line tracking the sidebar selection.    ███
# ███   · Burgers-vector regression test — pins b = 0.256 nm = a/√2    ███
# ███     everywhere, and confirms core_width_w (0.05 nm lower bound)  ███
# ███     is a DIFFERENT quantity.                                     ███
# ███   · `_regression_test_v101` — five-parameter gating invariants.  ███
# ============================================================================
...
# ============================================================================
# ███ v10.0.0 — STACKED LATENT MoE VISUALIZATION (EXACT XAI)             ███
# ============================================================================
def plot_stacked_latentmoe_v10(candidates, param_title, param_symbol,
                               unit, journal='nature',
                               fig_size=(6.5, 4.0),
    ...
    for expert_name in PlasticityLatentMoEScorer.EXPERT_ORDER:
        heights = np.array([
        ...
        if np.sum(heights) <= 1e-4:
            continue
        color = PlasticityLatentMoEScorer.EXPERT_COLORS.get(
            expert_name, '#BBBBBB')
        ax.bar(
            xs, heights, bar_width, bottom=bottoms,
            color=color, edgecolor='black', linewidth=0.5,
            label=expert_name, zorder=3)
        bottoms += heights
        drawn_experts.append(expert_name)

    for i, (x, c) in enumerate(zip(xs, cands_sorted)):
        is_best = (i == n - 1)
        ax.text(
            x, c.score + 0.012, f"{c.score:.2f}",
            ha='center', va='bottom',
            fontsize=7, fontweight='bold' if is_best else 'normal',
            color='black', zorder=6)
        if is_best:
            ax.text(
                x, c.score + 0.045, "⭐ BEST",
                ha='center', va='bottom',
    ...
    spec = PLASTICITY_ONTOLOGY[cands_sorted[0].param]
    ui_vals = [c.value_si / spec["ui_scale"] for c in cands_sorted]
    ui_param = cands_sorted[0].param
    _log_display = ui_param in ("rho0", "gamma0_dot")

    def _fmt_xtick(v: float) -> str:
        if _log_display and v > 0:
            return f"{v:.1e}"
        if abs(v) >= 1000:
            return f"{v:.2e}"
        return f"{v:.2f}"
...
# ============================================================================
# ███ v10.1.0 — FIVE REGIME TABLES (one per solver-consumed parameter) ███
# ============================================================================
# ρ₀ and γ̇₀ regime tables are already defined above (v9.0 / v9.1).
# The tables below add σ₀, m, and μ — completing full five-parameter coverage.

# ─── NEW: σ₀ processing-route regime (MPa, linear not log) ───────────
SIGMA0_PROCESSING_REGIMES: Dict[str, Dict[str, Any]] = {
    "sputter": dict(
        low=20.0, high=40.0, bench=25.0,
        desc="Sputter-deposited (high purity) — σ₀ ≈ 25 MPa "
             "(Jiang 2026 MSEA/MC; Zhang 2014 PL)"),
    "electrodeposit_pure": dict(
        low=40.0, high=75.0, bench=60.0,
        desc="Standard electrodeposition — σ₀ ≈ 50–75 MPa"),
    "electrodeposit_additives": dict(
        low=80.0, high=120.0, bench=100.0,
        desc="Electrodeposited with additives / internal stress "
             "— σ₀ ≈ 100 MPa (Chan 2011)"),
    "mechanical_polish": dict(
        low=150.0, high=300.0, bench=200.0,
        desc="Mechanically polished / post-processed surface layer "
             "— very high σ₀"),
}

# ─── NEW: m (SRS) loading-orientation regime (dimensionless) ─────────
M_ORIENTATION_REGIMES: Dict[str, Dict[str, Any]] = {
    "perpendicular_tb": dict(
        low=15.0, high=30.0, bench=20.0,
        desc="Loading ⊥ TBs — m ≈ 16–25 (Ye 2012 APL)"),
    "parallel_tb": dict(
        low=35.0, high=75.0, bench=50.0,
        desc="Loading ∥ TBs — m ≈ 40–72 (Ye 2012 APL)"),
    "random_polycrystal": dict(
        low=18.0, high=35.0, bench=25.0,
        desc="Random polycrystal — m ≈ 20–30"),
    "high_temp": dict(
        low=4.0, high=10.0, bench=6.0,
        desc="Elevated T (≈473 K) — m ≈ 4.69 (Parween 2026)"),
}

# ─── NEW: μ framework regime (GPa, linear not log) ──────────────────
MU_FRAMEWORK_REGIMES: Dict[str, Dict[str, Any]] = {
    "single_crystal": dict(
        low=70.0, high=80.0, bench=75.4,
        desc="Single-crystal C44 = 75.4 GPa (slip-system mechanics)"),
    "polycrystal": dict(
        low=42.0, high=55.0, bench=48.0,
        desc="Isotropic polycrystalline average ≈ 48 GPa "
             "(Lu 2015 ncomm) — Hall–Petch baseline"),
    "nt_cu_effective": dict(
        low=20.0, high=30.0, bench=23.5,
        desc="nt-Cu effective μ ≈ 23.5 GPa (Zhu 2011 AM)"),
    "chiu_continuum": dict(
        low=40.0, high=50.0, bench=46.0,
        desc="Continuum nt-Cu μ = 46.0 GPa (Chiu 2026 JMRT)"),
}


# ─── Canonicalisers (free-text → regime key) ─────────────────────────
def _norm_sigma0_processing(s: Optional[str]) -> Optional[str]:
    s = (s or "").lower()
    if "sputter" in s or "pvd" in s or "physical vapor" in s:
        return "sputter"
    if "additive" in s or "additively" in s:
        return "electrodeposit_additives"
    if "polish" in s or "post-process" in s or "postprocess" in s \
            or "mechanical" in s:
        return "mechanical_polish"
    if "electrodeposit" in s or "electroplat" in s:
        return "electrodeposit_pure"
    return None


def _norm_m_orientation(s: Optional[str]) -> Optional[str]:
    s = (s or "").lower()
    if "perpend" in s or "⊥" in s:
        return "perpendicular_tb"
    if "parallel" in s or "∥" in s:
        return "parallel_tb"
    if "random" in s or "polycrystal" in s:
        return "random_polycrystal"
    if "high temp" in s or "elevat" in s or "473" in s:
        return "high_temp"
    return None


def _norm_mu_framework(s: Optional[str]) -> Optional[str]:
    s = (s or "").lower()
    if "single" in s and ("c44" in s or "slip" in s):
        return "single_crystal"
    if "polycrystal" in s or "hall" in s or "isotropic" in s:
        return "polycrystal"
    if "zhu" in s or "nt-cu" in s or "nt cu" in s or "ntcu" in s:
        return "nt_cu_effective"
    if "chiu" in s or "continuum" in s:
        return "chiu_continuum"
    return None


def classify_sigma0_processing_regime(s: Optional[str]) -> Dict[str, Any]:
    key = _norm_sigma0_processing(s)
    if key is None:
        return dict(regime="unclassified — using PARAM_CANON plausible",
                    low=10.0, high=500.0, bench=50.0,
                    inferred=50.0, source="fallback")
    r = SIGMA0_PROCESSING_REGIMES[key]
    return dict(regime=r["desc"], low=r["low"], high=r["high"],
                bench=r["bench"], inferred=r["bench"],
                source=f"classifier({key})")


def classify_m_orientation_regime(s: Optional[str]) -> Dict[str, Any]:
    key = _norm_m_orientation(s)
    if key is None:
        return dict(regime="unclassified — using PARAM_CANON plausible",
                    low=1.0, high=200.0, bench=20.0,
                    inferred=20.0, source="fallback")
    r = M_ORIENTATION_REGIMES[key]
    return dict(regime=r["desc"], low=r["low"], high=r["high"],
                bench=r["bench"], inferred=r["bench"],
                source=f"classifier({key})")


def classify_mu_framework_regime(s: Optional[str]) -> Dict[str, Any]:
    key = _norm_mu_framework(s)
    if key is None:
        return dict(regime="unclassified — using PARAM_CANON plausible",
                    low=20.0, high=200.0, bench=48.0,
                    inferred=48.0, source="fallback")
    r = MU_FRAMEWORK_REGIMES[key]
    return dict(regime=r["desc"], low=r["low"], high=r["high"],
                bench=r["bench"], inferred=r["bench"],
                source=f"classifier({key})")
...
# ============================================================================
# v8.8.3 — DERIVED-QUANTITY BRIDGE (legacy; used by recommend_grounded)
# ============================================================================
    ...
    def step(self):
        try:
            ...
            sigma_y = compute_yield_stress(
                h_m, plastic_params['sigma0'], plastic_params['mu'],
...
_EXTRACT_SCHEMA = (
    '{"param": "rho0|mu|gamma0_dot|srs|sigma0_fric|sigma0_yield|tau_p", '
...
seven ASCII strings — do NOT use symbols or synonyms):
...
# ███ v10.1.0 — falloff tightened 1.0 → 0.6 decades                    ███
# ============================================================================
class PhysicsRegimeExpert:
    def __init__(self, falloff_decades: float = 0.6,
                 min_sigma_decades: float = 0.20):
...
# ███ v10.1.0 — falloff tightened 1.5 → 0.8 decades                    ███
# ============================================================================
class TheoryRegimeExpert:
    def __init__(self, falloff_decades: float = 0.8,
                 min_sigma_decades: float = 0.20):
        self.falloff_decades = float(falloff_decades)
        self.min_sigma_decades = float(min_sigma_decades)
...
# ============================================================================
# ███ v10.1.0 — THREE NEW REGIME EXPERTS                               ███
# ============================================================================

class Sigma0ProcessingExpert:
    """Genesis-thread expert for σ₀_fric — gated by the user's
    processing-route dropdown. Returns a Gaussian peak inside the
    regime band, exponential falloff outside.  Falloff is in MPa
    (linear, not log) because σ₀ spans only ~10× not 10⁵×."""
    def __init__(self, falloff_mpa: float = 20.0,
                 min_sigma_mpa: float = 5.0):
        self.falloff_mpa = float(falloff_mpa)
        self.min_sigma_mpa = float(min_sigma_mpa)

    def score(self, value_si: float,
              processing: Optional[str]) -> float:
        if not processing:
            return 0.5    # neutral when user leaves "(unspecified)"
        try:
            v = float(value_si) / 1e6     # → MPa
        except (TypeError, ValueError):
            return 0.5
        if v <= 0 or not np.isfinite(v):
            return 0.0
        r = classify_sigma0_processing_regime(processing)
        lo, hi, ctr = r["low"], r["high"], r["inferred"]
        if lo <= v <= hi:
            sigma = max((hi - lo) / 4.0, self.min_sigma_mpa)
            return float(np.exp(-((v - ctr) ** 2) / (2.0 * sigma ** 2)))
        dist = min(abs(v - lo), abs(v - hi))
        return float(np.exp(-dist / self.falloff_mpa))


class MOrientationExpert:
    """Thermo-Mech-thread expert for m (SRS).  Gates on the user's
    loading-orientation dropdown.  Falls back to the existing
    temperature-only check when no orientation is selected."""
    def __init__(self, falloff_m: float = 8.0,
                 min_sigma_m: float = 3.0):
        self.falloff_m = float(falloff_m)
        self.min_sigma_m = float(min_sigma_m)

    def score(self, value_si: float,
              orientation: Optional[str],
              target_temp: Optional[float]) -> float:
        try:
            v = float(value_si)
        except (TypeError, ValueError):
            return 0.5
        # ── Temperature floor (preserved from v10.0) ──────────────
        if target_temp and target_temp > 400 and v < 10:
            base = 1.0
        elif target_temp and target_temp <= 300 and 15 <= v <= 75:
            base = 1.0
        else:
            base = 0.5
        # ── Orientation refinement (multiplicative gating) ────────
        if not orientation:
            return base
        r = classify_m_orientation_regime(orientation)
        lo, hi, ctr = r["low"], r["high"], r["inferred"]
        if lo <= v <= hi:
            sigma = max((hi - lo) / 4.0, self.min_sigma_m)
            orient_score = float(np.exp(-((v - ctr) ** 2)
                                        / (2.0 * sigma ** 2)))
        else:
            dist = min(abs(v - lo), abs(v - hi))
            orient_score = float(np.exp(-dist / self.falloff_m))
        # Take the geometric mean so both signals must agree.
        return float(np.sqrt(base * orient_score))


class MuFrameworkExpert:
    """Framework-thread expert for μ.  Gates on the user's
    shear-modulus-convention dropdown (single-crystal C44 vs
    polycrystal 48 GPa vs nt-Cu 23.5 GPa vs Chiu 46 GPa)."""
    def __init__(self, falloff_gpa: float = 8.0,
                 min_sigma_gpa: float = 3.0):
        self.falloff_gpa = float(falloff_gpa)
        self.min_sigma_gpa = float(min_sigma_gpa)

    def score(self, value_si: float,
              framework: Optional[str]) -> float:
        if not framework:
            return 0.8    # neutral when no framework chosen
        try:
            v = float(value_si) / 1e9     # → GPa
        except (TypeError, ValueError):
            return 0.8
        if v <= 0 or not np.isfinite(v):
            return 0.0
        r = classify_mu_framework_regime(framework)
        lo, hi, ctr = r["low"], r["high"], r["inferred"]
        if lo <= v <= hi:
            sigma = max((hi - lo) / 4.0, self.min_sigma_gpa)
            return float(np.exp(-((v - ctr) ** 2) / (2.0 * sigma ** 2)))
        dist = min(abs(v - lo), abs(v - hi))
        return float(np.exp(-dist / self.falloff_gpa))


# ============================================================================
# ███ v10.1.0 — LATENT MoE SCORER WITH FIVE-PARAMETER GATING           ███
# ============================================================================
class PlasticityLatentMoEScorer:
    """Physics-Informed Mixture of Experts (MoE) ranker.

    Ten experts, weights linearly combined, weights sum to 1.00.

        v9.x core        v10.x physics threads
        ---------        ----------------------
        Material 0.15    Framework (A)  0.15   ← mu / gamma0_dot
        Thermal  0.10    Genesis   (B)  0.15   ← rho0 / sigma0_fric
        Strain   0.05    Thermo-Mech (C) 0.10  ← srs
        Method   0.10    Corpus Density 0.05   ← provenance-binned
        Confidence 0.05
        Reasoning  0.10
        -------------------------------
        Sum      1.00

    v10.1.0 extends framework-A / genesis-B / thermo-mech-C to cover
    all five solver-consumed parameters.  All gating is by user-sidebar
    dropdown; when the user leaves the dropdown unspecified, each
    expert returns its neutral default (0.5, 0.8) so old behaviour is
    preserved bit-for-bit.
    """

    EXPERT_ORDER: Tuple[str, ...] = (
        'Material', 'Thermal', 'Strain', 'Method', 'Confidence',
        'Reasoning', 'Framework (A)', 'Genesis (B)',
        'Thermo-Mech (C)', 'Corpus Density',
    )
    EXPERT_COLORS: Dict[str, str] = {
        ...
        'Framework (A)':  '#0072B2',
        'Genesis (B)':    '#D55E00',
        'Thermo-Mech (C)': '#000000',
        'Corpus Density': '#88CCEE',
    }

    def __init__(self, thermal_sigma: float = 100.0):
        # ── v10.x weight vector (UNCHANGED — sum = 1.00) ───────────────
        ...
        self.w_framework  = 0.15
        self.w_genesis    = 0.15
        self.w_thermomech = 0.10
        self.w_density    = 0.05

        self.thermal_sigma = thermal_sigma

        # ── Regime experts (v9.x unchanged; v10.1 new) ─────────────────
        self.regime_expert_rho0  = PhysicsRegimeExpert()
        self.theory_expert_gdot  = TheoryRegimeExpert()
        self.sigma0_proc_expert  = Sigma0ProcessingExpert()
        self.m_orient_expert     = MOrientationExpert()
        self.mu_fw_expert        = MuFrameworkExpert()

    # ── v9.x experts (unchanged) ────────────────────────────────────────
    ...
    def _reasoning_expert(reasoning: str, method: str) -> float:
        ...
        _COG_METHODS = (
            ...
            "hall_petch_intercept", "taylor_factor_cross_conversion",
            "peierls_nabarro_formula", "side_note_table",
            "derived_consensus", "physics_regime_inference",
            "theory_regime_inference",
            "sigma0_route_hall_petch_d", "sigma0_route_hall_petch_lam",
            "sigma0_route_model_inversion", "sigma0_route_peierls_nabarro",
        )
    ...
    # ── v10.x physics-thread experts (v10.1 extended) ──────────────────
    def _framework_expert(self, p: str, v_si: float, ext: Dict[str, Any],
                          target_theory: Optional[str],
                          theory_active: bool,
                          mu_framework: Optional[str] = None) -> float:
        """Thread A — Theory/Framework expert.
            gamma0_dot: TheoryRegimeExpert (s⁻¹ regime table)
            mu:         MuFrameworkExpert (GPa regime table)
        """
        if p == "gamma0_dot":
            if theory_active and target_theory:
                return self.theory_expert_gdot.score(
                    v_si,
        ...
        if p == "mu":
            if mu_framework:
                return self.mu_fw_expert.score(v_si, mu_framework)
            return 0.8
        return 0.0

    def _genesis_expert(self, p: str, v_si: float,
                        synthesis: Optional[str],
                        architecture: Optional[str],
                        twin_spacing: Optional[float],
                        regime_active: bool,
                        ext: Dict[str, Any],
                        sigma0_processing: Optional[str] = None) -> float:
        """Thread B — Genesis (origin of the microstructure).
            rho0:        PhysicsRegimeExpert (three-factor)
            sigma0_fric: Sigma0ProcessingExpert OR physics_inferred bonus
        """
        if p == "rho0":
            if regime_active:
                return self.regime_expert_rho0.score(
                    v_si, synthesis, architecture, twin_spacing)
            return 0.5
        if p == "sigma0_fric":
            if sigma0_processing:
                return self.sigma0_proc_expert.score(v_si,
                                                     sigma0_processing)
            prov = _norm_provenance(
                ext.get("_provenance", "") or ext.get("method", ""))
            return 0.9 if prov == "physics_inferred" else 0.5
        return 0.0

    def _thermomech_expert(self, p: str, v_si: float,
                           target_temp: float,
                           m_orientation: Optional[str] = None) -> float:
        """Thread C — Thermo-mechanical consistency (srs only).

        v10.0 path: T-only check (m ∈ [15,75] at T ≤ 300 K, m < 10 at T > 400 K).
        v10.1 path: if the user selected a loading orientation, delegate
                    to MOrientationExpert — which also preserves the T
                    check internally via a geometric mean.
        """
        if p != "srs":
            return 0.0
        if m_orientation:
            return self.m_orient_expert.score(v_si, m_orientation,
                                              target_temp)
        try:
            v = float(v_si)
        except (TypeError, ValueError):
        ...
        if target_temp and target_temp > 400 and v < 10:
            return 1.0
        if target_temp and target_temp <= 300 and 15 <= v <= 75:
            return 1.0
        return 0.5
    ...
    def score(self, extractions, target_material, target_temp,
              ...
              target_theory=None,
              mu_framework=None,
              sigma0_processing=None,
              m_orientation=None):
        regime_active_rho0 = any([
            synthesis not in (None, "", "(unknown)", "(unspecified)"),
            architecture not in (None, "", "(unknown)", "(unspecified)"),
        ...
        for ext in extractions:
            ...
            # ── v10.x physics-thread experts ───────────────────────────
            s_fw  = self._framework_expert(
                p, v_si_clamped, ext, target_theory, theory_active_gdot,
                mu_framework=mu_framework)
            s_gen = self._genesis_expert(
                p, v_si_clamped, synthesis, architecture, twin_spacing,
                regime_active_rho0, ext,
                sigma0_processing=sigma0_processing)
            s_tm  = self._thermomech_expert(
                p, v_si_clamped, target_temp,
            ...
            breakdown = {
                ...
                "Framework (A)":   self.w_framework  * s_fw,
                "Genesis (B)":     self.w_genesis    * s_gen,
                "Thermo-Mech (C)": self.w_thermomech * s_tm,
            ...
            if p == "gamma0_dot":
                cand_kwargs["theory"] = str(
                    ext.get("theory", "unspecified"))
...
class PlasticityRecommender:
    ...
    def __init__(self, db_dir: str = "json_metadatabase",
                 ...
                 mu_framework: Optional[str] = None,
                 sigma0_processing: Optional[str] = None,
                 m_orientation: Optional[str] = None,
        ...
        self.mu_framework         = mu_framework
        self.sigma0_processing    = sigma0_processing
        self.m_orientation        = m_orientation
        self.sigma0_regime: Dict[str, Any] = {}
        self.m_regime: Dict[str, Any] = {}
        self.mu_regime: Dict[str, Any] = {}
    ...
    def recommend_grounded(self, material, temp_k, strain_rate=1e-3,
        ...
        candidates = self.scorer.score(
            ...
            twin_spacing=self.rho0_twin_spacing,
            target_theory=self.gamma0_target_theory,
            mu_framework=self.mu_framework,
            sigma0_processing=self.sigma0_processing,
            m_orientation=self.m_orientation,
        )

        # Cache the new regime reports for post-analysis display.
        if self.sigma0_processing:
            self.sigma0_regime = classify_sigma0_processing_regime(
                self.sigma0_processing)
        if self.m_orientation:
            self.m_regime = classify_m_orientation_regime(self.m_orientation)
        if self.mu_framework:
            self.mu_regime = classify_mu_framework_regime(self.mu_framework)
...
def _plr_reset():
    ...
    st.session_state.pop(f"{_PLR}sigma0_processing", None)
    st.session_state.pop(f"{_PLR}m_orientation", None)
    st.session_state.pop(f"{_PLR}mu_framework", None)
...
def render_plasticity_recommender_sidebar(
    ...
    st.subheader("🤖 Intelligent Plasticity Recommender v10.1.0")
    st.caption(
        "**v10.1.0 — Five-parameter physics-gated MoE:** every "
        "solver-consumed parameter (ρ₀, μ, γ̇₀, m, σ₀) now has its "
        "own user-selectable regime expert.  Choose what you know in "
        "the **Pre-Analysis Context** block below; the LatentMoE scorer "
        "swings the ranking accordingly."
    )
    st.caption(
    ...
    # ========================================================================
    # ▼▼▼ v10.1 — Pre-Analysis Context: all five regimes ▼▼▼
    # ========================================================================
    st.markdown("### 🎯 Pre-Analysis Context")
    st.caption(
        "Choose what you know about your sample BEFORE clicking "
        "**Analyse JSON databases**.  Each dropdown activates a "
        "physics-regime expert that swings the LatentMoE score for "
        "the corresponding parameter.  Leave any as `(unspecified)` "
        "to use a neutral prior.")

    pre_cols = st.columns(2)

    # ── Col 1: ρ₀ + γ̇₀ ─────────────────────────────────────────────────
    with pre_cols[0]:
        with st.expander("🔬 ρ₀ synthesis + architecture + λ",
                         expanded=False):
            st.caption(
                "Three-factor physics-regime classifier (Hodge 2008, "
                "Yang 2024, Ye 2012).  λ modulates inside the band "
                "(smaller λ → higher ρ₀).")
        ...
        with st.expander("⚡ γ̇₀ theory framework", expanded=False):
            st.caption(
                "γ̇₀ spans 10 orders of magnitude across frameworks "
                "(JC ~1e-3, DDD ~5e3, MD ~1e7).  Tag mismatch drops "
                "the score to ~0.05.")
    ...
    # ── Col 2: σ₀ + m + μ ─────────────────────────────────────────────
    with pre_cols[1]:
        with st.expander("🎯 σ₀ processing route", expanded=False):
            st.caption(
                "σ₀ depends on synthesis purity + processing history. "
                "Sputtered ≈ 25 MPa; electrodeposited with additives "
                "≈ 100 MPa (Chan 2011).")
            sigma0_processing = st.selectbox(
                "Processing route for σ₀",
                ["(unspecified)",
                 "High-purity sputter-deposited (≈ 25 MPa)",
                 "Standard electrodeposition (≈ 50–75 MPa)",
                 "Electrodeposition with additives (≈ 100 MPa)",
                 "Mechanically polished / post-processed (> 150 MPa)"],
                index=0, key=f"{_PLR}sigma0_processing")

        with st.expander("📐 m (SRS) loading orientation", expanded=False):
            st.caption(
                "m is anisotropic w.r.t. twin-boundary orientation "
                "(Ye 2012).  Perpendicular loading: m ≈ 16–25; parallel: "
                "m ≈ 40–72.  Elevated T (473 K): m ≈ 4.69.")
            m_orientation = st.selectbox(
                "Loading orientation relative to TBs",
                ["(unspecified)",
                 "Perpendicular to TBs (m ≈ 16–25)",
                 "Parallel to TBs (m ≈ 40–72)",
                 "Random polycrystal (m ≈ 20–30)",
                 "Elevated T ≈ 473 K (m ≈ 4–10)"],
                index=0, key=f"{_PLR}m_orientation")

        with st.expander("🧱 μ shear-modulus convention", expanded=False):
            st.caption(
                "μ is framework-dependent (Lu 2015, Zhu 2011, Chiu 2026).  "
                "Single-crystal C44 = 75.4 GPa; polycrystal average = 48 GPa; "
                "nt-Cu effective = 23.5 GPa.")
            mu_framework = st.selectbox(
                "μ framework",
                ["(unspecified)",
                 "Single-crystal C44 (75.4 GPa) — slip-system mechanics",
                 "Polycrystalline isotropic average (48 GPa) — Hall–Petch",
                 "nt-Cu effective (23.5 GPa) — Zhu 2011",
                 "Chiu et al. continuum (46.0 GPa)"],
                index=0, key=f"{_PLR}mu_framework")

    # ── Canonicalise all five context args ────────────────────────────
    ...
    _sigma0_proc_arg = (None if sigma0_processing.startswith("(")
                        else sigma0_processing)
    _m_orient_arg   = (None if m_orientation.startswith("(")
                       else m_orientation)
    _mu_fw_arg      = (None if mu_framework.startswith("(")
                       else mu_framework)

    # ── Live previews of all five regimes ─────────────────────────────
    preview_cols = st.columns(5)
    regime_previews = [
        ("ρ₀",
         classify_rho0_regime(_rho0_syn_arg, _rho0_arch_arg, _rho0_lam_arg)
         if (_rho0_syn_arg or _rho0_arch_arg or _rho0_lam_arg) else None,
         preview_cols[0]),
        ("γ̇₀",
         classify_gamma0_regime(_gamma0_theory_arg)
         if _gamma0_theory_arg else None, preview_cols[1]),
        ("σ₀",
         classify_sigma0_processing_regime(_sigma0_proc_arg)
         if _sigma0_proc_arg else None, preview_cols[2]),
        ("m",
         classify_m_orientation_regime(_m_orient_arg)
         if _m_orient_arg else None, preview_cols[3]),
        ("μ",
         classify_mu_framework_regime(_mu_fw_arg)
         if _mu_fw_arg else None, preview_cols[4]),
    ]
    for sym, prev, col in regime_previews:
        with col:
            if prev is None:
                st.metric(sym, "—", delta="neutral prior",
                          delta_color="off")
            else:
                st.metric(
                    sym,
                    f"{prev['inferred']:g}",
                    delta=prev['regime'][:32],
                    delta_color="off")
    # ========================================================================
    # ▲▲▲ END Pre-Analysis Context ▲▲▲
    # ========================================================================
    ...
    if run_btn:
        recommender = PlasticityRecommender(
            ...
            mu_framework=_mu_fw_arg,
            sigma0_processing=_sigma0_proc_arg,
            m_orientation=_m_orient_arg,
            cascade_mode=cascade_mode)
        ...
        try:
            ...
            if getattr(recommender, "sigma0_regime", None):
                sr = recommender.sigma0_regime
                st.info(
                    f"🎯 **σ₀ processing regime (post-analysis):** "
                    f"{sr['regime']}  \n"
                    f"Range: `[{sr['low']:.1f}, {sr['high']:.1f}] MPa` · "
                    f"center `{sr['inferred']:.1f} MPa`")
            if getattr(recommender, "m_regime", None):
                mr = recommender.m_regime
                st.info(
                    f"📐 **m orientation regime (post-analysis):** "
                    f"{mr['regime']}  \n"
                    f"Range: `[{mr['low']:.1f}, {mr['high']:.1f}]` · "
                    f"center `{mr['inferred']:.1f}`")
            if getattr(recommender, "mu_regime", None):
                ur = recommender.mu_regime
                st.info(
                    f"🧱 **μ framework regime (post-analysis):** "
                    f"{ur['regime']}  \n"
                    f"Range: `[{ur['low']:.1f}, {ur['high']:.1f}] GPa` · "
                    f"center `{ur['inferred']:.1f} GPa`")
        except Exception as e:
            progress.empty()
...
def render_recommender_bars_pub(bundle, style):
    ...
    use_stacked_moe = st.checkbox(
        "📊 Use v10.x Stacked LatentMoE Chart (exact expert breakdown)",
        value=True, key="rec_use_stacked_moe",
        help="Replaces the v9.x provenance bar chart with a stacked bar "
             "chart in which each coloured segment is one MoE expert's "
             "exact weighted contribution.  Because the MoE is "
             "deterministic and linear, this is exact feature attribution "
             "— no SHAP/LIME approximation required.")

    if use_stacked_moe:
        ranked_cands = [c for c in cands
                        if not getattr(c, 'context', False)]
        if not ranked_cands:
            logger.warning(
                "render_recommender_bars_pub[%s]: all candidates are "
                "context — falling back to full list for stacked view",
                selected_param)
            ranked_cands = list(cands)

        fig = plot_stacked_latentmoe_v10(
            candidates=ranked_cands,
            param_title=meta['title'],
            param_symbol=meta['symbol'],
            unit=meta['unit'],
            journal=journal,
            fig_size=(max(4.0, 0.62 * style.figure_width),
                      max(3.0, 0.62 * style.figure_height)),
        )
        if fig is not None:
            st.pyplot(fig)
            buf = BytesIO()
            fig.savefig(
                buf, format='png', dpi=style.dpi,
                bbox_inches='tight', pad_inches=0.05,
                facecolor=fig.get_facecolor())
            plt.close(fig)
            buf.seek(0)
            return buf.getvalue()
        st.warning("Stacked MoE chart could not be generated "
                   "(empty candidate list).")

    # ── v9.x fallback: provenance chart ───────────────────────────────
...
def render_recommender_histograms_pub(bundle, style):
    ...
    # ═══════════════════════════════════════════════════════════════════
    # ███ v10.1 — Histogram Customization (opt-in)                  ███
    # ═══════════════════════════════════════════════════════════════════
    st.markdown("##### ⚙️ Histogram Customization")
    ...
    with hcol1:
        show_labels = st.checkbox(
            ...
            key="hist_show_labels",
            help="Toggle the ⭐ Best / 🎯 Optimal legend on each subplot.")
        show_optimal_line = st.checkbox(
            "Show optimal-range dotted line",
            value=True,
            key="hist_show_optimal",
            help="Dotted line marks the user-selected sidebar value — "
                 "shifts dynamically with sidebar choices.")
        show_best_line = st.checkbox(
            "Show best-candidate dashed line",
            value=True,
            key="hist_show_best",
            help="Dashed red line marks the top-scoring candidate.")

    with hcol2:
        individual_mode = st.checkbox(
            "Individual histogram mode",
            value=False,
            key="hist_individual",
            help="Plot one parameter at a time instead of a grid.")
        if individual_mode:
            selected_param = st.selectbox(
                "Parameter",
                available,
                format_func=lambda p: f"{PLASTICITY_ONTOLOGY[p]['symbol']} — "
                                      f"{PLASTICITY_ONTOLOGY[p]['label']}",
                key="hist_selected_param")
            params_to_plot = [selected_param]
        else:
            params_to_plot = available

    with hcol3:
        label_position = st.selectbox(
            "Label position",
            ["top-right", "upper right", "best", "none"],
            index=0,
            key="hist_label_pos",
            help="'none' hides labels entirely; overrides 'Show value labels'.")
        if label_position == "none":
            show_labels = False

    # ═══════════════════════════════════════════════════════════════════
    # ███ v10.1 — Dynamic optimal values from sidebar choices       ███
    # ═══════════════════════════════════════════════════════════════════
    optimal_values: Dict[str, float] = {}
    for p in available:
        sel = st.session_state.get(f"{_PLR}{p}_value_si")
        if sel is not None and not (isinstance(sel, float) and math.isnan(sel)):
            spec = PLASTICITY_ONTOLOGY[p]
            optimal_values[p] = float(sel) / spec["ui_scale"]
        else:
            best = bundle.best(p)
            if best:
                spec = PLASTICITY_ONTOLOGY[p]
                optimal_values[p] = best.value_si / spec["ui_scale"]

    # ═══════════════════════════════════════════════════════════════════
    # ███ Figure construction (existing logic + new dotted line)    ███
    # ═══════════════════════════════════════════════════════════════════
    ...
    loc_map = {"top-right": "upper right",
               "upper right": "upper right",
               "best": "best"}

    for idx, p in enumerate(params_to_plot):
        ax = axes[idx]
        spec = PLASTICITY_ONTOLOGY[p]
        ...
        if use_log:
            bins = np.logspace(
                np.log10(max(min(ui_vals), 1e-30)),
                np.log10(max(max(ui_vals), min(ui_vals) * 10)),
        ...
        # ── NEW: dynamic optimal-range dotted line (green) ──────
        if show_optimal_line and p in optimal_values:
            opt = optimal_values[p]
            ax.axvline(opt, color='#10b981', linestyle=':',
                       linewidth=style.line_width + 1.0, alpha=0.85,
                       label=f"🎯 Optimal ({opt:.3g})", zorder=6)

        # ── EXISTING: best-candidate dashed line (red) ──────────
        if show_best_line:
            best = bundle.best(p)
            if best is not None:
        ...
        if show_labels:
            ...
            if handles:
                ax.legend(loc=loc_map.get(label_position, "best"),
...
def render_recommender_visuals_dashboard():
    ...
    st.caption(
        ...
        "candidates.  v10.1.0: bar-chart tab offers an exact-XAI stacked "
        "LatentMoE view (each coloured segment IS one expert's weighted "
        "contribution).  Histograms tab now offers value labels, individual "
        "mode, and a dynamic green dotted line that tracks the sidebar "
        "selection.")
    chart_type = st.selectbox(
        "📈 Chart Type",
        ["Radar Chart", "Bar Chart", "Sankey Diagram", "Treemap",
         "Histograms (publication)"],
        index=0, key="rec_chart_type_v84")
...
def main():
    ...
    <strong>✅ PURE FFT SPECTRAL + AI PLASTICITY RECOMMENDER v10.1.0:</strong><br>
    ...
    • <span style="color: green;">🧮 v8.9.0 FRICTION STRESS LAB:</span> σ₀ physics-inferred from context via HP(d), HP(λ), solver-law inversion, P–N+Taylor floor.<br>
    • <span style="color: green;">🔬 v9.0.0 PHYSICS-GROUNDED ρ₀:</span> snippet-enhanced NER ∧ three-factor regime classifier ∧ physics-regime LatentMoE expert.<br>
    • <span style="color: green;">⚡ v9.1.0 THEORY-AWARE γ̇₀:</span> Johnson-Cook vs CPFEM vs DDD vs MD disambiguation via `theory` tagging.<br>
    • <span style="color: green;">🎯 v9.2.2 3-BUCKET LEGEND:</span> coarse display rollup over 6 fine keys. `llm_reasoned` is RESERVED.<br>
    • <span style="color: green;">🩶 v9.3.0 UNION CASCADE:</span> every viable route runs; non-incumbent routes enter as greyed non-ranked context bars.<br>
    • <span style="color: green;">🔬 v9.3.1 HONEST NAMING:</span> `derived` → `physics_inferred`; coarse label reads "Deterministic (regex · physics-inferred)".  🔓 audit toggle reveals the full competitive landscape.<br>
    • <span style="color: green;">🌟 v10.0.0 PHYSICS-THREAD MoE:</span> 10-expert ranker with exact-XAI stacked bar chart — each coloured segment IS one expert's weighted contribution.<br>
    • <span style="color: green;">🎯 v10.1.0 FIVE-PARAMETER GATING:</span> all five solver-consumed parameters (ρ₀, μ, γ̇₀, m, σ₀) now have user-gated regime experts.  Falloffs tightened for sharper discrimination.<br>
    • <span style="color: green;">🎯 5/5 target coverage:</span> ρ₀, μ, γ̇₀, m, σ₀ close automatically.<br>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        if st.button("🔄 Reset to Defaults", type="secondary"):
            ...
            st.rerun()
        st.markdown("---")
        with st.expander("🧠 AI Plasticity Recommender v10.1.0", expanded=False):
...
def _regression_test_v10() -> None:
    import dataclasses as _dc
    fields = {f.name: f for f in _dc.fields(PlasticityCandidate)}
    assert "moe_breakdown" in fields, \
        "PlasticityCandidate missing v10 moe_breakdown field"
    assert fields["moe_breakdown"].default_factory is dict, \
        "moe_breakdown must use default_factory=dict"

    legacy = PlasticityCandidate(
        param='mu', value_si=48e9, raw_value=48.0, raw_unit='GPa',
        score=0.8, confidence=0.7, material='Cu', temp_k=None,
        strain_rate=None, method='explicit', source_file='',
        source_title='', evidence='')
    assert legacy.moe_breakdown == {}, \
        "moe_breakdown should default to empty dict"

    sc = PlasticityLatentMoEScorer()
    w_sum = (sc.w_material + sc.w_thermal + sc.w_strain
             + sc.w_method + sc.w_confidence + sc.w_reasoning
             + sc.w_framework + sc.w_genesis + sc.w_thermomech
             + sc.w_density)
    assert abs(w_sum - 1.00) < 1e-9, \
        f"v10 weight vector must sum to 1.00, got {w_sum}"

    for expert in PlasticityLatentMoEScorer.EXPERT_ORDER:
        assert expert in PlasticityLatentMoEScorer.EXPERT_COLORS, \
            f"expert {expert!r} has no palette entry"

    ext_mu = {"param": "mu", "value": 48.0, "unit": "GPa",
              "material": "Cu", "temp": 300.0, "strain_rate": 1e-3,
              "method": "explicit", "confidence": 0.9,
              "reasoning": "", "evidence": "",
              "_source_file": "t", "_source_title": "t",
              "_provenance": "llm_extract", "_context": False}
    s_mu_fw = sc._framework_expert(
        "mu", 48e9, ext_mu,
        target_theory="ddd", theory_active=True)
    assert s_mu_fw == 0.8, \
        f"mu framework score must be neutral 0.8, got {s_mu_fw}"

    ext_g = {"param": "gamma0_dot", "value": 5e3, "unit": "s^-1",
             "material": "Cu", "temp": 300.0, "strain_rate": 5e3,
             "method": "explicit", "confidence": 0.9, "theory": "ddd",
             "reasoning": "", "evidence": "",
             "_source_file": "t", "_source_title": "t",
             "_provenance": "llm_extract", "_context": False}
    s_g_fw = sc._framework_expert(
        "gamma0_dot", 5e3, ext_g,
        target_theory="ddd", theory_active=True)
    assert s_g_fw > 0.5, \
        f"DDD γ̇₀ against DDD target should score >0.5, got {s_g_fw}"

    ext_g_cpfem = dict(ext_g, theory="cpfem", value=1e-3)
    s_g_mismatch = sc._framework_expert(
        "gamma0_dot", 1e-3, ext_g_cpfem,
        target_theory="ddd", theory_active=True)
    assert s_g_mismatch < 0.1, \
        f"CPFEM γ̇₀ against DDD target should score <0.1, got {s_g_mismatch}"

    assert sc._density_expert("llm_extract") == 1.0
    assert sc._density_expert("regex_ner")   == 1.0
    assert sc._density_expert("llm_reasoned") == 1.0
    assert abs(sc._density_expert("physics_inferred") - 0.7) < 1e-9
    assert abs(sc._density_expert("regime_prior") - 0.4) < 1e-9
    assert abs(sc._density_expert("llm_prior") - 0.2) < 1e-9

    ext_full = [{
        "param": "sigma0_fric", "value": 5e7, "unit": "Pa",
        "material": "Cu", "temp": 300.0, "strain_rate": 1e-3,
        "method": "hall_petch_intercept", "confidence": 0.55,
        "reasoning": "Step 1: HP relation.\nStep 2: invert.\n"
                     "Step 3: σ₀ = σ_y − k·d^(−1/2).",
        "evidence": "", "_source_file": "t", "_source_title": "t",
        "_provenance": "physics_inferred", "_context": False,
    }]
    buckets = sc.score(ext_full, "Cu", 300.0)
    bucket = buckets.get("sigma0_fric", [])
    assert bucket, "scorer dropped the sigma0_fric candidate"
    bd = bucket[0].moe_breakdown
    assert set(bd.keys()) == set(PlasticityLatentMoEScorer.EXPERT_ORDER), \
        f"breakdown keys mismatch: {set(bd.keys())}"
    assert abs(sum(bd.values()) - bucket[0].score) < 1e-9, \
        "sum(breakdown) must equal total score"

    fig = plot_stacked_latentmoe_v10(
        candidates=bucket,
        param_title="Friction stress",
        param_symbol=r"\sigma_0",
        unit="MPa",
        journal='nature')
    assert fig is not None, "stacked chart returned None for valid input"
    plt.close(fig)

    assert plot_stacked_latentmoe_v10(
        [], "t", "s", "u") is None, \
        "stacked chart should return None for empty candidates"

    logger.info("✅ _regression_test_v10 passed")


def _regression_test_burgers_vector() -> None:
    """v10.1 — Verify b = 0.256 nm for Cu everywhere, and that
    core_width_w is a *separate* quantity (its 0.05 nm lower bound
    is physically correct for w, NOT a Burgers-vector error)."""
    # 1. MaterialProperties — the ground truth
    cu = MaterialProperties.get_cu_properties()
    assert abs(cu['plasticity']['b'] - 0.256e-9) < 1e-12, (
        f"Burgers vector in MaterialProperties.get_cu_properties() = "
        f"{cu['plasticity']['b']}, expected 0.256e-9")

    # 2. PLASTICITY_ONTOLOGY defaults
    assert abs(PLASTICITY_ONTOLOGY["burgers_vector_b"]["defaults"]["Cu"]
               - 0.256) < 1e-9, \
        "PLASTICITY_ONTOLOGY Burgers vector for Cu drifted"

    # 3. PARAM_CANON plausible range
    b_lo, b_hi = PARAM_CANON["burgers_vector_b"]["plausible"]
    assert b_lo <= 0.256 <= b_hi, \
        f"PARAM_CANON Burgers vector range {b_lo}-{b_hi} excludes 0.256"

    # 4. core_width_w is a DIFFERENT quantity — its 0.05 nm lower
    #    bound is physically correct for the dislocation core half-width,
    #    NOT a Burgers-vector error.  Auto-default w = b/(1-ν) ≈ 0.388 nm.
    w_lo, w_hi = PARAM_CANON["core_width_w"]["plausible"]
    assert w_lo == 0.05, \
        f"core_width_w lower bound = {w_lo}, expected 0.05 nm"
    assert w_lo < 0.256, \
        "core width lower bound must be < Burgers vector (w < b always)"
    b, nu = 0.256e-9, 0.34
    w_default = b / (1 - nu)
    assert abs(w_default * 1e9 - 0.388) < 0.01, \
        f"w_default arithmetic drifted: {w_default*1e9:.4f} nm"

    # 5. Cu lattice parameter consistency: a = 0.3615 nm, b = a/√2
    a = 0.3615e-9
    b_from_a = a / np.sqrt(2)
    assert abs(b_from_a - 0.256e-9) < 0.001e-9, \
        f"b = a/√2 derivation drifted: {b_from_a*1e9:.4f} nm"

    logger.info("✅ _regression_test_burgers_vector passed — "
                "b = 0.256 nm (a/√2, a = 0.3615 nm); "
                "w_lo = 0.05 nm (correctly distinct from b)")


def _regression_test_v101() -> None:
    """v10.1 — five-parameter physics-gated MoE invariants."""

    # ── 1. All five regime tables exist and have bench values.
    assert RHO0_REGIMES and len(RHO0_REGIMES) >= 4
    assert GAMMA0_DOT_REGIMES and len(GAMMA0_DOT_REGIMES) >= 5
    assert len(SIGMA0_PROCESSING_REGIMES) >= 4
    assert len(M_ORIENTATION_REGIMES) >= 3
    assert len(MU_FRAMEWORK_REGIMES) >= 4

    # ── 2. All five classifiers return the 'fallback' shape on None.
    r_rho = classify_rho0_regime(None, None, None)
    r_gam = classify_gamma0_regime(None)
    r_sig = classify_sigma0_processing_regime(None)
    r_m   = classify_m_orientation_regime(None)
    r_mu  = classify_mu_framework_regime(None)
    for r, name in ((r_rho, 'rho0'), (r_gam, 'gamma0'),
                    (r_sig, 'sigma0'), (r_m, 'm'), (r_mu, 'mu')):
        for k in ("regime", "low", "high", "bench", "inferred", "source"):
            assert k in r, f"classify_{name}(None) missing key {k!r}"

    # ── 3. Weight vector still sums to 1.00 (v10.0 invariant preserved).
    sc = PlasticityLatentMoEScorer()
    w_sum = (sc.w_material + sc.w_thermal + sc.w_strain
             + sc.w_method + sc.w_confidence + sc.w_reasoning
             + sc.w_framework + sc.w_genesis + sc.w_thermomech
             + sc.w_density)
    assert abs(w_sum - 1.00) < 1e-9, \
        f"v10.1 weight vector must still sum to 1.00, got {w_sum}"

    # ── 4. Three new experts are gated (return neutral when no input).
    assert sc.sigma0_proc_expert.score(50e6, None) == 0.5
    assert sc.mu_fw_expert.score(48e9, None) == 0.8
    # m_orient_expert returns the existing T-based check when no
    # orientation is selected — for T=300K, m=20 should score 1.0.
    assert sc.m_orient_expert.score(20.0, None, 300.0) == 1.0

    # ── 5. σ₀ discrimination: sputter vs electrodeposited with additives.
    s_sputter = sc.sigma0_proc_expert.score(
        25e6, "High-purity sputter-deposited (≈ 25 MPa)")
    s_additive = sc.sigma0_proc_expert.score(
        25e6, "Electrodeposition with additives (≈ 100 MPa)")
    assert s_sputter > 0.85, \
        f"sputter σ₀ = 25 MPa should score >0.85, got {s_sputter}"
    assert s_additive < 0.10, \
        f"sputter σ₀ = 25 MPa under additive regime should score <0.10, " \
        f"got {s_additive}"

    # ── 6. μ discrimination: single-crystal 75 GPa vs polycrystal 48 GPa.
    s_sc_75 = sc.mu_fw_expert.score(
        75.4e9, "Single-crystal C44 (75.4 GPa) — slip-system mechanics")
    s_pc_75 = sc.mu_fw_expert.score(
        75.4e9, "Polycrystalline isotropic average (48 GPa) — Hall–Petch")
    assert s_sc_75 > 0.85, \
        f"single-crystal μ = 75.4 GPa should score >0.85, got {s_sc_75}"
    assert s_pc_75 < 0.15, \
        f"single-crystal μ = 75.4 GPa under polycrystal regime should " \
        f"score <0.15, got {s_pc_75}"

    # ── 7. m orientation discrimination: perpendicular vs parallel.
    s_perp = sc.m_orient_expert.score(
        20.0, "Perpendicular to TBs (m ≈ 16–25)", 300.0)
    s_par  = sc.m_orient_expert.score(
        20.0, "Parallel to TBs (m ≈ 40–72)", 300.0)
    assert s_perp > 0.85, \
        f"m=20 under perpendicular regime should score >0.85, got {s_perp}"
    assert s_par < 0.15, \
        f"m=20 under parallel regime should score <0.15, got {s_par}"

    # ── 8. End-to-end: scorer threads all five context kwargs.
    ext = [{
        "param": "sigma0_fric", "value": 25e6, "unit": "Pa",
        "material": "Cu", "temp": 300.0, "strain_rate": 1e-3,
        "method": "explicit", "confidence": 0.9,
        "reasoning": "", "evidence": "",
        "_source_file": "t", "_source_title": "t",
    ...
    b_neutral = sc.score(ext, "Cu", 300.0)
    b_sputter = sc.score(ext, "Cu", 300.0,
                          sigma0_processing="High-purity sputter-deposited")
    b_additive = sc.score(ext, "Cu", 300.0,
                          sigma0_processing="Electrodeposition with additives")
    s_neutral = b_neutral["sigma0_fric"][0].score
    s_sputter = b_sputter["sigma0_fric"][0].score
    s_additive = b_additive["sigma0_fric"][0].score
    assert s_sputter > s_neutral, \
        f"sputter context should boost σ₀=25 MPa: {s_sputter} vs {s_neutral}"
    assert s_additive < s_neutral, \
        f"additive context should drop σ₀=25 MPa: {s_additive} vs {s_neutral}"
    assert s_sputter / max(s_additive, 1e-9) > 1.5, \
        f"discrimination ratio < 1.5: {s_sputter/max(s_additive, 1e-9)}"

    # ── 9. Burgers vector sanity (also covered by Patch G).
    cu = MaterialProperties.get_cu_properties()
    assert abs(cu['plasticity']['b'] - 0.256e-9) < 1e-12

    logger.info("✅ _regression_test_v101 passed — "
                "five-parameter physics-gated MoE "
                "(ρ₀/μ/γ̇₀/m/σ₀ all discriminating)")


def _run_regression_suite() -> None:
    """Run the v8.8–v10.1 battery.  Names resolve via globals().get()
    so a missing test warns instead of blanking the app.  Gated behind
    NTCU_REGRESSION because module-level code re-executes on every
    widget interaction in single-file Streamlit apps."""
    ...
    suite = [
        ...
        ('burgers_vector', '_regression_test_burgers_vector'),
        ('v10.1.0', '_regression_test_v101'),
    ]
