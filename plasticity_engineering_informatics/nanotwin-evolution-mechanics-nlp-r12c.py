...
# ███ + PLASTICITY PARAMETER INTELLIGENT RECOMMENDER v8.6.0              ███
...
# ----------------------------------------------------------------------------
# v8.5.0 / v8.5.1 / v8.6.0: plot_candidate_scores
# ----------------------------------------------------------------------------
def plot_candidate_scores(candidates, scores, provenance, best_idx=None, *,
    ...
    for x, s, p in zip(xs, scs, provs):
        ...
        # Keep the marker legible on any bar colour: hollow white marker with
        # a contrasting outline picked per-bar.
    ...
    # ── Value labels: offset-in-points, adaptive halo ───────────────────
    for x, s in zip(xs, scs):
        # Adaptive text colour: dark on light bars, white on dark bars
        bar_rgb = _color_to_rgb(faces[x])
        lum = _relative_luminance(bar_rgb)
    ...
    if show_colorbar and color_by == 'score':
        ...
        cbar.set_label('Score', fontsize=style['font_size_small'])
        cbar.ax.tick_params(labelsize=style['font_size_small'] - 1)
    ...
    if best_x is not None:
        handles.append(Patch(facecolor=faces[best_x],
                             ...
                             label='Best match'))
    # horizontal legend ABOVE the axes -> can never collide with bars/labels
...
def render_candidate_score_chart(param_key, candidates, scores, provenance,
    ...
    fig = plot_candidate_scores(candidates, scores, provenance, best_idx,
                                ...
                                unit=spec_meta['unit'], journal=journal)
    screen, hires = BytesIO(), BytesIO()
    fig.savefig(screen, format='png', dpi=200, bbox_inches='tight', pad_inches=0.08)
    fig.savefig(hires,  format='png', dpi=hires_dpi, bbox_inches='tight', pad_inches=0.05)
    plt.close(fig)
    screen.seek(0)
    st.image(screen, use_container_width=True)   # use_column_width on older Streamlit
...
def render_plasticity_recommender_sidebar(
    ...
    st.subheader("🤖 Intelligent Plasticity Recommender v8.6.0")
    st.caption(
...
def render_recommender_bars_pub(bundle: PlasticityRecommendationBundle,
    ...
    full colormap control.

    v8.5.0 fixes (see "Fixing the LatentMoE Candidate-Score Figure"):
      1. Labels no longer collide with bar tops/spines — offset-POINT
         annotations + white halo bbox + 22% y headroom + pad_inches.
      2. No more "1.00e+13 m^-2" ticks — mathtext via sci_tex(); the unit
         is stated ONCE in the axis label (journal convention).
      3. LLM vs heuristic provenance is now visible — encoded as marker
    ...
    v8.6.0 colormap engine:
      · 60+ curated colormaps in 5 categories (Sequential / Diverging /
        Cyclic / Categorical / Classic).
      · color_by ∈ {'score', 'index', 'uniform'} controls the mapping.
      · Optional colorbar legend (score mode only).
      · Best-match override in 7 preset accent colors, or 'auto' which
        samples the opposite end of the colormap (score mode) / Okabe-Ito
        blue (index/uniform).
      · Adaptive text halo — value labels flip between black and white
        based on bar luminance.
      · 5 one-click presets (Ocean / Heat / Rainbow / Diverge / Classic).
    """
    ...
    with st.expander("🎨 Colormap & Coloring Options", expanded=True):
        ...
        with col_a:
            # Curated, categorised, publication-friendly colormap list.
            all_cmap_names: List[str] = list(BAR_CMAP_FLAT)
            # Default: prefer style.bar_colormap, then style.colormap, then viridis
            ...
            # Allow an externally-queued value (from a preset) to win:
            ...
            # Live preview strip of the chosen colormap
            try:
        ...
        with col_b:
            ...
            show_cbar = st.checkbox(
                ...
                value=bool(style.bar_show_colorbar),
                key="rec_bar_show_cbar",
                help="Adds a colourbar showing the score→colour mapping "
                     "(only meaningful when coloring by score).",
            )

        with col_c:
            # Best-match colour override
            best_color_options = {
            ...
            # Match a previously-selected override back to a display label:
            ...
            # Manual normalisation range (score mode only)
            if color_mode == 'score':
    ...
    # ── v8.6.0: one-click presets (safe pending-state mechanism) ────────
    ...
    for col, (name, cmap_name, mode_name) in zip(preset_cols, presets):
        with col:
            if st.button(name, use_container_width=True,
                ...
                # Queue the preset and rerun; _apply_pending_bar_preset()
                # will seed the widget keys at the top of the next pass.
    ...
    provenance = [c.method for c in cands]   # 'llm_inferred' / 'heuristic' / ...
    ...
    fig = plot_candidate_scores(
        ...
        label_pad=max(6.0, float(style.font_size_annotation)),
        headroom=1.22,
        despine=True,
        # ── v8.6.0 colormap arguments ───────────────────────────────────
...
def render_recommender_style_controls() -> RecommenderVisualStyle:
    ...
    # ── v8.6.0: bar-chart defaults (persisted to the dataclass) ─────────
    with st.expander("📊 Bar-Chart Defaults (v8.6.0)", expanded=False):
        st.caption(
            "Set the bar chart's default colormap and colour-mapping mode. "
            "You can still override these per-render inside the "
            "**Bar Chart** tab, and one-click presets are available there."
        )
        c1, c2 = st.columns(2)
        with c1:
...
def main():
    ...
    <strong>✅ PURE FFT SPECTRAL + AI PLASTICITY RECOMMENDER v8.6.0:</strong><br>
    ...
    with st.sidebar:
        ...
        with st.expander("🧠 AI Plasticity Recommender v8.6.0", expanded=False):
