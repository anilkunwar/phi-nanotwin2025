...
# CWD-independent, root-or-subfolder, Git-LFS aware, self-invalidating cache.
# ----------------------------------------------------------------------------
...
# ORDER MATTERS: srs must be tested before gamma0_dot, since "strain_rate_sensitivity"
# also contains the substring "strain_rate".
PARAM_PATTERNS = [
    ("srs",        ("strain_rate_sensitivity",)),
    ("gamma0_dot", ("reference_strain_rate",)),
    ("mu",         ("shear_modulus",)),
    ("sigma0",     ("friction_lattice_stress", "friction_stress")),
    ("rho0",       ("initial_dislocation_density", "dislocation_density")),
]

# Candidate snippet headers per file (kept in sync with the original list form).
CAND_FIELDS = {
...
# ----------------------------------------------------------------------------
# Stage-A helpers (preserved from original)
# ----------------------------------------------------------------------------
...
def _evidence_roots() -> List[Path]:
    """Candidate roots, anchored to __file__ when available, else CWD.

    Returned list is deduplicated and every entry is a directory that exists.
    """
    try:
        ...
        here = Path.cwd()
    candidates = [
        # Script directory and its parent (covers /app/app.py + /app/json_metadatabase)
        here,
        here.parent,
        here / JSON_META_DIR,
        here.parent / JSON_META_DIR,
        # CWD variants
        Path.cwd(),
        Path.cwd() / JSON_META_DIR,
        # Explicit deployment roots
        Path("/app"),
        Path("/app") / JSON_META_DIR,
        Path("/workspace"),
        Path("/workspace") / JSON_META_DIR,
        Path("/mnt/data"),
        Path("/mnt/data") / JSON_META_DIR,
    ]
    # Also honor an explicit env-var override if it points at a real dir.
    env_dir = os.environ.get("JSON_META_DIR")
    if env_dir:
...
def _is_lfs_pointer(p: Path) -> bool:
    """Detect Git-LFS pointer stubs (which are ~130 bytes of ASCII text)."""
    try:
        with open(p, "rb") as f:
            head = f.read(130)
...
def _limited_rglob(root: Path, max_depth: int = 3) -> List[Path]:
    """Bounded-depth *.json discovery so we don't walk an entire container FS."""
    out: List[Path] = []
    try:
...
def discover_evidence_files() -> Tuple[Dict[str, Path], Dict[str, Any]]:
    """Fresh every call. Returns (files, report). Never cached."""
    found: Dict[str, Path] = {}
    report: Dict[str, Any] = {
        "roots": [],
        "json_seen": [],
    ...
    for root in _evidence_roots():
        ...
        for p in _limited_rglob(root, max_depth=3):
            try:
                size = p.stat().st_size
            except OSError:
                continue
            report["json_seen"].append(f"{p.name}  ({size:,} B)  @ {p.parent}")
            if p.parent.name == JSON_META_DIR or "metadatabase" in p.parent.name.lower():
                report["metadatabase_dirs"].setdefault(str(p.parent), []).append(p.name)

            if _is_lfs_pointer(p):
                report["lfs_pointers"].append(str(p))
                continue
            # A real metadatabase is at least a few hundred bytes; smaller → treat as stub.
            if size < 200:
                report["skipped_small"].append(str(p))
                continue

            name = p.name.lower().replace(" ", "").replace("(1)", "")
            for param, keys in PARAM_PATTERNS:
                if param not in found and any(k in name for k in keys):
                    found[param] = p
                    report["matched"][param] = str(p)

    return found, report


def _fingerprint(files: Dict[str, Path]) -> str:
    """Cache key = (name, size, mtime) per file — auto-invalidates on any change."""
    items = []
    for k in sorted(files.keys()):
        v = files[k]
        try:
            st_ = v.stat()
            items.append((k, str(v), st_.st_size, int(st_.st_mtime)))
        except OSError:
            items.append((k, str(v), -1, -1))
    return hashlib.md5(repr(items).encode()).hexdigest()[:16]


def _show_discovery_report(files: Dict[str, Path], report: Dict[str, Any]) -> None:
    matched_n = len(files)
    with st.expander(
        f"🔍 Evidence discovery — matched {matched_n}/5 databases",
        expanded=(matched_n < 5),
    ):
        st.write("**Matched:**", {k: v.name for k, v in files.items()} or "nothing yet")
        st.write("**Scanned roots:**")
        for r in report["roots"]:
            try:
                contents = [p.name for p in Path(r).iterdir()
                            if p.suffix.lower() in (".json", ".csv", ".bib")][:15]
            except OSError:
                contents = ["<unreadable>"]
            st.code(f"{r}\n  └─ {contents}")
        if report["metadatabase_dirs"]:
            st.write("**metadatabase directories found:**", report["metadatabase_dirs"])
        if report["json_seen"]:
            st.write("**All JSONs seen (first 20):**")
            st.code("\n".join(report["json_seen"][:20]))
        if report["lfs_pointers"]:
            st.error(
                "Git-LFS pointer stubs detected — the actual file contents "
                "were never checked out:\n" +
                "\n".join(report["lfs_pointers"]) +
                "\n\nFix: run `git lfs install && git lfs pull` locally, then "
                "commit/push. Or re-upload the real files via the Streamlit "
                "uploader below."
            )
        if report["skipped_small"]:
            st.warning("Files skipped for being <200 B (likely stubs):\n" +
                       "\n".join(report["skipped_small"]))
        st.info(
            "Fix: place the 5 `*_metadatabase.json` files either next to this "
            "script or inside `json_metadatabase/` next to it, then press R. "
            "The parse cache auto-invalidates on any file change."
        )


@st.cache_data(show_spinner=False)
def _parse_evidence_cached(
    fp_key: str, file_map: Dict[str, str]
) -> List[Dict[str, Any]]:
    """Parse each JSON file into a list of serializable Evidence dicts."""
    corpus: List[Dict[str, Any]] = []
    seen: set = set()
    for param, fpath in file_map.items():
        p = Path(fpath)
        for rec in robust_load_json(p):
            if not isinstance(rec, dict):
                continue
            uid = (rec.get("unique_id") or rec.get("DOI") or rec.get("doi") or
...
def load_evidence_corpus(json_dir: str = None) -> List[Dict[str, Any]]:
    """Stage-A entry point. Returns a list of Evidence dicts.

    `json_dir` is only a *hint*; discovery is anchored to __file__ and CWD so
    it works regardless of where the process was launched from. If fewer than
    five files are found, a Streamlit uploader is offered as a fallback.
    """
    files, report = discover_evidence_files()

    # Optional hint: if the caller passed an explicit directory, scan it too.
    if json_dir and len(files) < 5:
        hint = Path(json_dir)
        if not hint.is_absolute():
            # try both relative-to-CWD and relative-to-__file__
            here = Path(__file__).resolve().parent if "__file__" in globals() else Path.cwd()
            candidates = [hint, here / json_dir, Path.cwd() / json_dir]
        else:
            candidates = [hint]
        for h in candidates:
            if not h.is_dir():
                continue
            for p in _limited_rglob(h, max_depth=3):
                name = p.name.lower().replace(" ", "")
                for param, keys in PARAM_PATTERNS:
                    if param not in files and any(k in name for k in keys):
                        files[param] = p
                        report["matched"][param] = str(p)

    if len(files) < 5:
        _show_discovery_report(files, report)
        up = st.file_uploader(
            "…or upload the 5 *_metadatabase.json files manually",
            type="json",
        ...
        for f in up or []:
            ...
            for param, keys in PARAM_PATTERNS:
                if param not in files and any(k in name for k in keys):
                    ...
                    files[param] = tmp
                    report["matched"][param] = str(tmp)

    if not files:
        st.warning("No evidence files found in any candidate location.")
        return []

    corpus = _parse_evidence_cached(
        _fingerprint(files), {k: str(v) for k, v in files.items()}
    )
    st.caption(
        f"Evidence DB: {len(corpus)} records · "
        f"{len({c['paper_id'] for c in corpus})} unique papers · "
        f"params matched: {sorted(files.keys())}"
    )
    if len(files) < 5:
        st.warning(f"Only {len(files)}/5 databases loaded: {sorted(files.keys())}")
    return corpus
...
def main():
    ...
    # ---- 10-second truth probe: what does the container actually see? ----
    st.sidebar.markdown(f"**CWD:** `{os.getcwd()}`")
    try:
        _here = Path(__file__).resolve().parent
        st.sidebar.markdown(f"**Script dir:** `{_here}`")
    except NameError:
        st.sidebar.markdown("**Script dir:** `<no __file__>`")
    _jsons = []
    for _dp, _dn, _fn in os.walk("."):
        if _dp.count(os.sep) > 3:
            _dn[:] = []
        _jsons += [os.path.join(_dp, f) for f in _fn if f.endswith(".json")]
    st.sidebar.write("**JSONs visible (CWD walk):**", _jsons[:20] or "NONE")
    _md = Path("json_metadatabase")
    if _md.is_dir():
        st.sidebar.write("**./json_metadatabase contents:**",
                         [p.name for p in _md.iterdir()])
    else:
        st.sidebar.write("**./json_metadatabase** does not exist from CWD")
