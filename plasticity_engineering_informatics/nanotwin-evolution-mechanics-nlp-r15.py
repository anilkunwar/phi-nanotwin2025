...
# ███   · GAZETTEER-driven entity matching (material/property/method/    ███
# ███     structure/unit) with NFKC + μ↔mu + C₄₄↔c44 normalization.      ███
# ███   · PARAM_CANON: ONE alias list shared by retrieval, LLM prompt,   ███
# ███     and gatekeeper — retrieval and validation can no longer drift. ███
# ███   · Boolean/NL query generation for debugging + optional external  ███
# ███     search APIs (build_boolean_queries / build_query_texts).       ███
# ███   · JSONPath-preserving record flattening (provenance of every hit). ███
# ███   · Value-anchored scorer: a bare number only earns full points    ███
# ███     when a property label or unit sits within ±80 / ±15 chars.     ███
# ███   · Deterministic property-alias→value→unit regex extractor used   ███
# ███     as the LLM fallback (plugs into existing PROVENANCE_MARKERS).  ███
# ███   · HybridRetriever: lexical ∧ (optional FAISS/SBERT) fused by RRF ███
# ███     so exact numerals stay authoritative (embeddings destroy them).███
# ███   · Grounded LLM prompt with strict verbatim-only JSON contract    ███
...
def alias_pattern(alias: str) -> str:
    """'c44' → '(?<![a-z0-9])c[\\s_\\-]{0,2}4[\\s_\\-]{0,2}4(?![a-z0-9])'"""
    a = norm_text(alias)
    out = []
...
# ---- 7. VALUE EXTRACTION ---------------------------------------------------
_NUM  = r"\d+(?:\.\d+)?"
_NUM_UNIT_RE = re.compile(rf"({_NUM})\s*(gpa|mpa|pa)\b")
_EQ_NUM_RE   = re.compile(rf"[=:]\s*({_NUM})(?![a-z0-9])")
_BARE_NUM_RE = re.compile(rf"(?<![a-z0-9.])({_NUM})(?![a-z0-9])")


def find_value_after(text, pos, target_unit="gpa", window=80):
    seg = text[pos: pos + window]
    m = _NUM_UNIT_RE.search(seg)
    if m:
        return dict(value=float(m.group(1)), unit=m.group(2))
    m = _EQ_NUM_RE.search(seg)
    if m:
        u = re.search(r"\b(gpa|mpa|pa)\b", seg[m.end(): m.end() + 25])
        return dict(value=float(m.group(1)),
                    unit=u.group(1) if u else target_unit)
    return None
...
def rrf(rank_lists, k=60):
    """Reciprocal Rank Fusion. Small k emphasizes top hits."""
    agg: Dict[str, float] = {}
...
def gatekeep(items, param_key, provenance="llm"):
    canon = PARAM_CANON.get(param_key, {})
...
def recommend_param_values(param_key, material="Cu", value_hints=None,
                           ...
                           retriever=None):
    """Full retrieve-then-extract pipeline for a single parameter."""
    ...
    if use_llm and records:
        try:
            ...
            logger.warning("LLM extraction failed → heuristic fallback: %s", e)
    if not cands:
...
def get_retriever(folder: str = "json_metadatabase",
    ...
    """Cached singleton. Purged by st.cache_resource.clear() in
    'Force reload corpus'."""
...
class PlasticityParameterExtractor:
    """Extracts plasticity parameters from text, in one of three modes:
        ...
        mode="grounded"            → v8.8.0: LLM only reads pre-filtered
                                     top-k records from the hybrid retriever
                                     (see recommend_param_values())
    """
...
class PlasticityRecommender:
    """End-to-end: hybrid retrieve → grounded LLM extract → gatekeep →
    ...
    def recommend_grounded(self, material: str, temp_k: float,
        ...
        for i, param in enumerate(PARAM_ORDER):
            if progress_callback:
                progress_callback(i + 1, len(PARAM_ORDER),
                                  f"grounded-extract {param}")
            cands, records = recommend_param_values(
                param_key=param,
                material=material,
            ...
            for c in cands:
                extractions.append({
                    ...
                    "temp": temp_k,
                    "strain_rate": strain_rate,
                    "method": "explicit" if c.provenance == "llm" else "heuristic",
        ...
        if "gamma0_dot" not in present:
            extractions.append({
                ...
                "confidence": 0.3,
                "evidence": "",
                "reasoning": ("Step 1: grounded retriever found no explicit "
