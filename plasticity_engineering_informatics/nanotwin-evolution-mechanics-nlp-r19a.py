...
# ███ + PLASTICITY PARAMETER INTELLIGENT RECOMMENDER v9.3.0              ███
...
# ============================================================================
# ███ v9.2.2 — 3-BUCKET DISPLAY TAXONOMY + v9.3.0 INCUMBENT PINS       ███
# ============================================================================
# FINE keys (6) — internal vocabulary, drives MoE method-expert weights,
# confidence caps (prior 0.5 / derived 0.75), pyramid sanity, tooltips.
# COARSE keys (3) — reader actions: check the span / treat as hypothesis /
# re-run it.  Display-layer rollup only.
# ============================================================================

FINE_PROVENANCE_KEYS: Tuple[str, ...] = (
    'llm_extract', 'llm_reasoned', 'regex_ner',
    'regime_prior', 'llm_prior', 'derived',
)

PROVENANCE_GROUPS: Dict[str, Tuple[str, ...]] = {
    'llm_grounded':  ('llm_extract', 'llm_reasoned'),
    'llm_prior':     ('llm_prior',),
    'deterministic': ('regex_ner', 'regime_prior', 'derived'),
}
...
LEGEND_MARKERS: Dict[str, Dict[str, Any]] = {
    ...
    'deterministic': dict(marker='^', ms=7.5, fill=True,
                          label='Deterministic (regex · derived)'),
}
...
PROVENANCE_MARKERS: Dict[str, Dict[str, Any]] = {
    ...
    'derived':       dict(marker='v', ms=7.0, fill=False,
                          label='Derived (side-note cluster)'),
}

# ════════════════════════════════════════════════════════════════════════
# ███ v9.3.0 — INCUMBENT ROUTE PIN-LIST + CONTEXT CLASSIFIER          ███
# ════════════════════════════════════════════════════════════════════════
# Routes that produced the v9.2.2 findings (read straight from the
# legend-mix log).  In union cascade mode, candidates from OTHER coarse
# buckets enter the pool flagged as context — plotted greyed but excluded
# from ranking and best-match selection.  The frozen findings literally
# cannot move.  Empty tuple → no pinning, all routes competitive (the
# pre-v9.3.0 behaviour).
...
_DERIVED_HINTS: Tuple[str, ...] = (
    ...
    'solver_law', 'model_inversion', 'consensus', 'derived',
)
...
def _norm_provenance(p: Any) -> str:
    ...
    _regression_test_v921:
        ...
        1. exact fine key
        2. derived arithmetic      — 'model_inversion' etc. never LLM
    ...
    design and the present-only legend can never show it.  With Option A
    it is stamped at Tier-1 emission from the same signal that selects
    the CoT prompt.
    """
    s = str(p).strip().lower()
...
def _norm_granularity(g: Any) -> str:
    ...
    'coarse'.  (v9.2.1 fix: previously _legend_key defaulted unknown→
    coarse while marker_table defaulted unknown→fine → KeyError.)"""
...
def _stamp_provenance(ext: Dict[str, Any], fine_key: str) -> None:
    ...
    Call at the point of emission, where the route is known — never
...
def plot_candidate_scores(candidates, scores, provenance, best_idx=None, *,
    ...
    # ── v9.2.1: normalize ONCE; _legend_key and marker_table agree for
    # every input, including invalid ones.
    ...
    # ── v9.3.0: context flags are reindexed alongside everything else.
    ctxs = ([bool(context_flags[i]) for i in order]
            if context_flags is not None else [False] * n)

    # ── observability — which buckets actually fired this run.
    ...
    if best_x is not None:
        if best_color_override:
            ...
            # ── v9.2.1: fixed Okabe–Ito yellow.  The old cmap(1 − norm)
            # complement equals the highest-scoring normal bar's colour
            # when best == lowest.  Override me for yellow-dominant cmaps.
            faces[best_x] = '#F0E442' if color_by == 'score' else '#0072B2'

    # ── v9.3.0: bars drawn in two batches so context can carry its own
    # alpha/edge treatment without affecting the competitive bars.
    ...
    if ctx_x:
        # Greyed variation — non-ranked, deliberately less assertive
    ...
    # ── v9.3.0: winner reference line when context is present — context
    # bars may be taller than the winner, but the eye needs an anchor.
    if best_x is not None and any(ctxs):
        ax.axhline(scs[best_x], color='0.35', linewidth=0.7,
                   linestyle=(0, (4, 3)), zorder=2)

    # ── present-only legend, granularity-aware, optional (n=…) counts,
    # plus a single 'Context (non-ranked)' entry when any are present.
    ...
    if handles:
        ncol = min(4, len(handles))       # wrap >4 entries (fine mode)
    ...
    if title:
        ...
        if handles and len(handles) > ncol:   # wrapped legend = extra row
...
def render_candidate_score_chart(param_key, candidates, scores, provenance,
    ...
    try:
        ...
        _ver = (0, 0)
    if _ver >= (1, 46):
        st.image(screen, width='stretch')            # Streamlit ≥ 1.46
    else:
        st.image(screen, use_container_width=True)   # legacy
...
def _pick_best(cands):
    ...
    trust = {
        ...
        'regex_ner':    2, 'derived':      2,   # deterministic, reproducible
...
def gatekeep(items, param_key, provenance="regex_ner", conf_cap=None):
    ...
    v9.3.0: `_context` flag on the input dict is copied through to the
    produced ValueCandidate, so union-cascade context tagging survives
    the gatekeeper boundary.
    """
    canon = PARAM_CANON.get(param_key, {})
    (lo, hi) = canon.get("plausible", (float("-inf"), float("inf")))
...
def recommend_param_values(param_key, material="Cu", value_hints=None,
    ...
    cascade_mode='fallback' (v8.8.2 default): short-circuit — a tier only
    runs if the previous tier produced nothing.  Each param's pool is
    therefore the output of exactly one tier.

    cascade_mode='union' (v9.3.0): every viable tier runs.  Non-incumbent
    routes (i.e., routes whose coarse bucket is not in
    INCUMBENT_ROUTES[param]) are tagged `_context=True` — plotted as
    greyed, non-ranked variation bars.  Findings are structurally frozen.
    """
...
def recommend_rho0_physics(
    ...
    # ── Tier 1 + Tier 2 via generic cascade ───────────────────────────
    ...
    # ── Tier 2b — snippet context per mention ─────────────────────────
    ...
    if cascade_mode == 'union' or not cands:
        ...
        # Dedupe against any identical-value candidates already in pool
...
def recommend_gamma0_theory_aware(
    ...
    if cascade_mode == 'union' and records:
        ...
        # Dedupe against existing Tier-1 values to avoid duplicate bars
...
# ════════════════════════════════════════════════════════════════════════
# ███ PlasticityCandidate — v9.3.0 adds `context` field                 ███
# ════════════════════════════════════════════════════════════════════════
@dataclass
    ...
    def score(self, extractions, target_material, target_temp,
        ...
        for ext in extractions:
            ...
            cand_kwargs = dict(
                ...
                context=bool(ext.get("_context", False)),   # ── v9.3.0
            )
            if p == "gamma0_dot":
...
class PlasticityRecommendationBundle:
    ...
    def best(self, param: str) -> Optional[PlasticityCandidate]:
        """v9.3.0 — context candidates are excluded from best-match.
        Preserves v9.2.2 findings regardless of which extra routes ran."""
        lst = self.candidates.get(param, [])
        if not lst:
            return None
        ranked = [c for c in lst if not getattr(c, 'context', False)]
        if not ranked:
            # Degenerate: all context — return top by score as last resort.
...
class PlasticityRecommender:
    ...
    def recommend_grounded(self, material, temp_k, strain_rate=1e-3,
        ...
        # ── Context field harvest (all deterministic; no pinning applies) ─
        ...
        for p_key in context_keys:
            ...
            for _h in hits:
                if isinstance(_h, dict):
                    _stamp_provenance(_h, 'regex_ner')
                    # Context field params are not in INCUMBENT_ROUTES,
                    # so _is_context returns False (they're un-pinned).
                    _h['_context'] = False
        ...
        for c in derived_v883:
            ...
            extractions.append({
                ...
                "_source_title": c.property_label,
                "_provenance": "derived",
                "_context": _is_context(target, "derived"),
            })
        ...
        # ── σ₀ consensus injection + v9.3.0 four-route disaggregation ──
        ...
        try:
            ...
            if cons:
                ...
                derived_cand = PlasticityCandidate(
                    ...
                    provenance='derived',
                    context=_is_context('sigma0_fric', 'derived'),
                )
            ...
            # ── v9.3.0 σ₀ disaggregation: emit each derivation route as
            # a context candidate.  The consensus stays competitive; the
            # routes are the argument for why the consensus is what it is.
            ...
            for r in derived_stress.get('routes', []):
                ...
                route_cand = PlasticityCandidate(
                    ...
                    reasoning=r.note,
                    provenance='derived',
                    context=True,   # spec §5: always context
                )
                bucket.append(route_cand)
            if bucket:
                bucket.sort(key=lambda c: c.score, reverse=True)
        except Exception:
            logger.warning("v8.9.0/v9.3.0 derived-σ₀ bridge failed:\n%s",
                           traceback.format_exc())
    ...
    def recommend(self, material, temp_k, strain_rate=1e-3, max_docs=20,
        ...
        for i, doc in enumerate(docs):
            ...
            for e in ext:
                ...
                # Legacy path doesn't have param-specific pins; leave
                # _context False so behaviour is unchanged from v9.2.x.
                e["_context"] = False
...
def _plr_render_parameter_selector(param: str,
    ...
    if best is not None:
        ...
        options.append((
            ...
            best.value_si, best))
    # Alternates (skip context candidates — they're not competitive)
    n_alt = 0
    ...
    # Context route summary (informative only)
...
def render_plasticity_recommender_sidebar(
    ...
    st.subheader("🤖 Intelligent Plasticity Recommender v9.3.0")
    st.caption(
        "**v9.3.0 union cascade + context candidates:** every viable "
        "extraction route runs (grounded LLM ∧ regex NER ∧ LLM prior). "
        "Routes that did *not* produce the pinned v9.2.2 finding enter as "
        "greyed, non-ranked context bars — the reader sees the full "
        "variation without the finding ever moving. σ₀ additionally "
        "disaggregates into its four derivation routes (HP(d), HP(λ), "
        "solver-law inversion, P–N+Taylor)."
    )
    st.caption(
        "**v9.2.2 honesty pass:** `llm_reasoned` is a *reserved* fine key "
        "— it stays unwired until a real chain-of-thought prompt path "
        "exists, so the present-only legend never promises more than the "
        "pipeline delivers. **v9.1.0 theory-aware γ̇₀** and **v9.0.0 "
        "physics-grounded ρ₀** remain active."
    )
    ...
    # ── v9.3.0 — union vs fallback cascade ────────────────────────────
    cascade_mode = st.radio(
        ...
        help=("Union: every viable route runs; non-incumbent routes enter "
              ...
              "short-circuit — a tier only runs if previous tiers produced "
              "nothing (fewer Ollama calls, one-bucket-per-figure legend)."))
...
def render_recommender_bars_pub(bundle, style):
    ...
    with gcol1:
        legend_granularity = st.radio(
            ...
            help=("Coarse: LLM-grounded / LLM-prior / Deterministic. "
                  "Fine: verbatim / regex / regime / prior / derived. "
    ...
    # ── v9.3.0: build candidate / score / provenance / context_flags lists ──
...
def render_recommender_visuals_dashboard():
    ...
    st.caption(
        ...
        "candidates. v9.3.0: non-incumbent routes appear as greyed "
        "non-ranked context bars — findings stay pinned by "
        "INCUMBENT_ROUTES.")
...
def main():
    ...
    <strong>✅ PURE FFT SPECTRAL + AI PLASTICITY RECOMMENDER v9.3.0:</strong><br>
    ...
    • <span style="color: green;">🎯 v9.2.2 3-BUCKET LEGEND:</span> coarse display rollup over 6 fine keys. `llm_reasoned` is a RESERVED fine key — unwired until a genuine CoT prompt path exists.<br>
    • <span style="color: green;">🩶 v9.3.0 UNION CASCADE:</span> every viable route runs; non-incumbent routes enter as greyed non-ranked context bars. σ₀ disaggregates into its four derivation routes. Findings stay pinned by INCUMBENT_ROUTES.<br>
    ...
    with st.sidebar:
        ...
        if st.button("🔄 Reset to Defaults", type="secondary"):
            ...
            st.rerun()
        st.markdown("---")
        with st.expander("🧠 AI Plasticity Recommender v9.3.0", expanded=False):
...
def _regression_test_v92() -> None:
    ...
    fig = plot_candidate_scores(
        ...
        ['heuristic', 'llm', 'llm_prior', 'derived'],
    ...
    assert 'Deterministic (regex · derived)' in texts
    plt.close(fig)
    ...
    assert 'Derived (side-note cluster)' in texts
    plt.close(fig)
...
def _regression_test_v921() -> None:
    ...
    assert _norm_provenance('model_inversion') == 'derived'
    ...
    assert PROVENANCE_MARKERS['derived']['fill'] is False
    ...
    fig = plot_candidate_scores(
        ...
        ['explicit', 'llm_prior', 'heuristic', 'derived',
...
def _regression_test_v922() -> None:
    ...
    assert pc.provenance == ""                                  # B1 default
    assert pc.context is False                                  # v9.3 default
...
# ════════════════════════════════════════════════════════════════════════
# ███ v9.3.0 — UNION CASCADE + CONTEXT-CANDIDATE INVARIANTS           ███
# ════════════════════════════════════════════════════════════════════════
def _regression_test_v930() -> None:
    ...
    # ── 1. Pin-list structure is consistent with the taxonomy ──────────
    ...
    # ── 2. _is_context: pinned buckets are never context, others are ────
    ...
    # ── 3. Un-pinned params: nothing is context ────────────────────────
    ...
    # ── 7. gatekeep copies `_context` from input dicts ─────────────────
    fake = [{"value": 48.0, "unit": "GPa", "confidence": 0.9,
             "property_label": "shear modulus", "source": "test",
             "_context": True}]
    ...
    # ── 8. Scorer preserves `_context` through the extraction dict ─────
...
def _run_regression_suite() -> None:
    """Run the v8.8–v9.3.0 battery.  Names resolve via globals().get()
