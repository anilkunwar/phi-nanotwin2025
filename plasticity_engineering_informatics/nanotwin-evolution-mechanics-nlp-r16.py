...
# ███ + PLASTICITY PARAMETER INTELLIGENT RECOMMENDER v8.9.0              ███
...
# ============================================================================
# ███ v8.9.0 — NER PIPELINE + CASCADE + DERIVED LAB (all-in-one)        ███
# ============================================================================
...
# ---- Parameters whose plausible lower bound is strictly positive ------------
# v8.9.0: `gatekeep` uses this set to reject v ≤ 0 for these params, which
# was the root cause of the G = 0.000 GPa degenerate that fed τ_P = 0.
...
# ============================================================================
# v8.8.3 — SIDE-NOTE TABLE PARSER (v8.9.0: rejects zero/negative values)
# ============================================================================
...
def parse_side_note_table(text: str) -> List[Dict[str, Any]]:
    ...
    degenerate that was propagating silently into the P–N route and
    producing τ_P = 0 (rescued only by the plausible-range clamp).
    """
    out: List[Dict[str, Any]] = []
    for m in _SIDE_NOTE_ROW_RE.finditer(text):
        ...
        # v8.9.0 — reject degenerate zero / negative rows.
        if value <= 0.0:
            logger.info(
...
# ============================================================================
# v8.8.3 — DERIVED-QUANTITY BRIDGE (v8.9.0: G=0 guard, w never clamped)
# ============================================================================
...
def derive_sigma0_and_tau_p(
    ...
    derivation now lives in `build_derived_routes`.  v8.9.0 additions:
    · G=0 degeneracy guard
    · w defaults to b/(1−ν), never silently clamped
    """
    ...
    # v8.9.0 — reject degenerate explicit G and fall back to mu default.
    if G_v is not None and G_v <= 1.0:
        logger.warning(
            "derive_sigma0_and_tau_p[%s]: rejecting explicit G=%s "
            "(below 1.0 Pa; likely side-note zero-collapse)", bundle_material, G_v)
        G_v = None
    if G_v is None:
    ...
    # v8.9.0 — w is never silently clamped.  Physical default w = b/(1−ν)
    # is used unless a well-provenanced captured w exists.
    ...
    # PATH B — Taylor cross-conversion
    ...
    # PATH C — P–N formula (v8.9.0: uses w = b/(1−ν) default unless captured)
...
# ============================================================================
# ███ v8.9.0 — FRICTION / LATTICE STRESS LAB (σ₀ & τ_P DERIVATION)      ███
# ============================================================================
NM           = 1e-9
MPA_PER_GPA  = 1.0e3
...
def _entry_candidates(bundle, key) -> List:
    """Return the candidate list for a given parameter key.

    Handles:
      · the PlasticityRecommendationBundle dataclass (has `.candidates` dict)
      · a raw {param: [candidates]} dict
      · the {param: {'candidates': [...]}} dict schema
    """
    if bundle is None:
        return []
...
def _cand_value_ui_unit(c, param_key) -> Optional[float]:
    """Return a candidate's value in the ontology's UI unit for `param_key`.

    Handles both v8.7 PlasticityCandidate (value_si in SI) and v8.8
    ValueCandidate (value in PARAM_CANON unit).
    """
...
def build_derived_routes(ctx, mu_GPa=None, w_mode='auto') -> List[DerivedRoute]:
    ...
    # ---- Routes A/B: Hall–Petch intercepts ------------------------------
    ...
    # ---- Route C: exact inverse of compute_yield_stress -----------------
    lam = ctx['lam'].value
    ...
    # ---- Route D: Peierls–Nabarro + Taylor (floor) ----------------------
...
def gatekeep(items, param_key, provenance="llm", conf_cap=None):
    ...
    v8.9.0: additionally rejects v ≤ 0 for params whose plausible range has
    a strictly positive lower bound.  This is the second line of defence
    against the G = 0.000 GPa degenerate (the first is the side-note parser).
    """
    canon = PARAM_CANON.get(param_key, {})
    (lo, hi) = canon.get("plausible", (float("-inf"), float("inf")))
    ...
    for it in items:
        ...
        # v8.9.0 — reject degenerate non-positive values for positive params.
...
# v8.9.0 — sidebar renders only the 5 target recommendations as cards.
# Context fields are derivation INPUTS and are handled by the Lab tab.
...
def _normalize_unit(value: float, unit: str, param: str) -> float:
    ...
    # v8.9.0 — a value of 0 for a positive-lower-bound param is a
    # degenerate sentinel; propagate as NaN so downstream rejects it.
...
class PlasticityLatentMoEScorer:
    ...
    def _method_expert(method: str) -> float:
        return {
            ...
            # v8.9.0 — derived consensus weight.
    ...
    def score(self, extractions, target_material, target_temp,
        ...
        for ext in extractions:
            ...
            score = (self.w_material * s_mat + self.w_thermal * s_temp
                     + self.w_strain * s_strain + self.w_method * s_method
                     + self.w_confidence * s_conf + self.w_reasoning * s_reason)
...
class PlasticityRecommendationBundle:
    ...
    def coverage_five(self) -> int:
        """v8.9.0 — how many of the five target recommendations are filled."""
...
class PlasticityRecommender:
    ...
    def recommend_grounded(self, material, temp_k, strain_rate=1e-3,
        ...
        try:
            # Build a provisional bundle to let the Lab code introspect
...
def render_plasticity_recommender_sidebar(
    ...
    st.subheader("🤖 Intelligent Plasticity Recommender v8.9.0")
    st.caption(
        "**v8.9.0 Friction & Lattice Stress Lab:** σ₀ is now *derived* "
        "from the side-note context (σ_y, d, λ, b, ν, M, k_y, w, G) via a "
        "weighted-median consensus of HP(d), HP(λ), solver-law inversion, "
        "and P–N+Taylor floor. The 5 target recommendations close as "
        "ρ₀, μ, γ̇₀, m, **σ₀**. Open the **🧮 Friction Stress Lab** tab in "
        "the main bar to inspect, adopt, or override the derivation."
    )
    st.caption(
        "**v8.9.0 bug fixes:** G = 0.000 GPa degenerate from the side-note "
        "parser is now rejected at three layers (parser, `_normalize_unit`, "
        "`gatekeep`). w is never silently clamped for P–N — auto mode "
        "defaults to the physical w = b/(1−ν)."
    )
    ...
    # v8.9.0 — slim down: only the 5 target recommendations as cards.
...
def render_friction_stress_lab():
    ...
    # -- w-mode control (the exponential hazard) --------------------------
    ...
    if st.button('✅ Adopt consensus σ₀ for simulation', type='primary'):
        ...
        ov['sigma0'] = cons['value_MPa'] * 1e6  # Pa (solver canonical)
        ov['sigma0_fric'] = cons['value_MPa']   # MPa (display fallback)
        st.success(
...
def apply_plasticity_overrides(solver) -> None:
    try:
        ...
        for k, v in overrides.items():
            ...
            if k == "sigma0":
                # v8.9.0 — solver-visible σ₀ override (Pa from Lab, or
                # converted from MPa when set via the sidebar fallback).
                try:
                    fv = float(v)
                    sigma0_explicit = fv
                except (TypeError, ValueError):
            ...
            if k == "sigma0_fric" and sigma0_explicit is None:
                # v8.9.0 — Lab wrote a fallback in MPa; convert to Pa.
                try:
...
def render_recommender_visuals_dashboard():
    ...
    st.caption(
        ...
        "candidates. v8.9.0 adds the derived-σ₀ consensus (▼).")
...
def main():
    ...
    <strong>✅ PURE FFT SPECTRAL + AI PLASTICITY RECOMMENDER v8.9.0:</strong><br>
    ...
    • <span style="color: green;">🩹 v8.9.0 G=0 fix:</span> degenerate zero rejected at parser, unit-normalizer, and gatekeeper layers.<br>
    • <span style="color: green;">🩹 v8.9.0 w-clamp fix:</span> w = b/(1−ν) default; ζ slider; no silent clamping.<br>
    ...
    with st.sidebar:
        ...
        if st.button("🔄 Reset to Defaults", type="secondary"):
            ...
            st.rerun()
        st.markdown("---")
        with st.expander("🧠 AI Plasticity Recommender v8.9.0", expanded=False):
    ...
    # ---------------------------------------------------------------------
    # v8.9.0 — Friction Stress Lab (always visible)
    # ---------------------------------------------------------------------
    st.markdown("---")
    render_friction_stress_lab()

    # AI visuals dashboard
    render_recommender_visuals_dashboard()


# ============================================================================
# REGRESSION TESTS (v8.8.1 / v8.8.2 / v8.8.3 / v8.8.4 / v8.9.0)
# ============================================================================
...
def _regression_test_v884() -> None:
    ...
    assert "reasoning" in fields and fields[-1] == "reasoning"
    stale = object.__new__(ValueCandidate)
    stale.value = 1.0; stale.unit = "Pa"
    stale.provenance = "llm_prior"
    stale.property_label = ""; stale.method = ""; stale.evidence = ""
    stale.source = ""; stale.confidence = 0.5
...
def _regression_test_v890() -> None:
    """v8.9.0 — bug fixes + derived consensus regression."""
    # 1) G = 0.000 GPa rejection at parse_side_note_table level
    bad_table = (
        "| Field         | Value    |\n"
        "| Shear modulus | 0.000    |\n"    # will not match \d+ — regex allows 0
        "| Shear modulus | 0 | GPa |\n"     # explicit zero row
    )
    ...
    # 2) gatekeep rejects non-positive for positive-lower-bound params
    ...
    pn = next(r for r in routes if r.key == 'peierls_nabarro')
    # G=48 GPa, ν=0.34, b=0.256 nm → w ≈ 0.388 nm → τ_P ≈ 10-30 MPa
    ...
    # k_y=0.11 MPa·m^½, λ=20 nm → term ≈ 0.11/√(20e-9) ≈ 778 MPa
...
# _regression_test_v881(); _regression_test_v882()
# _regression_test_v883(); _regression_test_v884(); _regression_test_v890()
