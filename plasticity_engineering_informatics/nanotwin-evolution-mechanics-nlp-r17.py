...
# ███ + PLASTICITY PARAMETER INTELLIGENT RECOMMENDER v9.0.0              ███
...
# ███     matching Tier-1 hits so the regime expert fires even on verbatim ███
# ███     values.                                                         ███
...
# ============================================================================
# ███ v9.0.0 — NER PIPELINE + CASCADE + DERIVED LAB + PHYSICS ρ₀        ███
# ============================================================================
...
def render_plasticity_recommender_sidebar(
    ...
    st.subheader("🤖 Intelligent Plasticity Recommender v9.0.0")
    st.caption(
        "**v9.0.0 Physics-grounded ρ₀ inference:** snippet-enhanced NER "
        "captures (synthesis, grain-architecture, twin-lamella λ) around "
        "each ρ₀ mention and feeds a three-factor regime classifier that "
        "bounds the LLM to a physically derived 10¹³–10¹⁷ m⁻² window. "
        "A dedicated LatentMoE expert (weight 0.25 for ρ₀ only) rewards "
        "in-regime candidates with a Gaussian peak at the log-center. "
        "The ρ₀ histogram becomes **multi-modal** (one peak per synthesis "
        "regime) instead of a flat 6-decade smear."
    )
    st.caption(
        "**v8.9.0 Friction & Lattice Stress Lab:** σ₀ is *derived* from "
        "the side-note context (σ_y, d, λ, b, ν, M, k_y, w, G) via a "
        "weighted-median consensus of HP(d), HP(λ), solver-law inversion, "
        "and P–N+Taylor floor. Open the **🧮 Friction Stress Lab** tab."
    )
...
def render_recommender_visuals_dashboard():
    ...
    st.caption(
        ...
        "candidates. v9.0.0 adds physics-regime ρ₀ inference.")
...
def main():
    ...
    <strong>✅ PURE FFT SPECTRAL + AI PLASTICITY RECOMMENDER v9.0.0:</strong><br>
    ...
    with st.sidebar:
        ...
        if st.button("🔄 Reset to Defaults", type="secondary"):
            ...
            st.rerun()
        st.markdown("---")
        with st.expander("🧠 AI Plasticity Recommender v9.0.0", expanded=False):
