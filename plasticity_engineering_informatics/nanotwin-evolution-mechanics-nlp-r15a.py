...
# ============================================================================
# v8.8.2: PROVENANCE_MARKERS now carries three glyphs — 'D' for verbatim
# grounded LLM extraction, '^' for deterministic-regex fallback, and 'o'
# for the newly-restored Tier-3 prior-inference path. All three are
# independent of bar colour (colour-blind safe) and are surfaced as
# proxy-artist legends above the axes.
# ============================================================================
PROVENANCE_MARKERS = {
...
def _norm_provenance(p):
    """Collapse any provenance string into one of the three canonical keys.

    v8.8.2: the 'prior'/'estimate'/'no evidence'/'world knowledge' test MUST
    precede the 'llm' test — otherwise 'llm_prior' would be silently
    downgraded to 'llm' and lose its dedicated marker + confidence cap.
    """
    p = str(p).strip().lower()
    if any(k in p for k in ('prior', 'estimate', 'no evidence', 'world knowledge')):
        return 'llm_prior'
    return 'llm' if any(k in p for k in ('llm', 'inferred', 'model', 'ai')) \
        else 'heuristic'
...
# ============================================================================
# ███████████████████████████████████████████████████████████████████████████
# ███  v8.8.0 / v8.8.1 / v8.8.2 — NER PIPELINE + THREE-TIER CASCADE       ██
# ███████████████████████████████████████████████████████████████████████████
# ============================================================================
...
PARAM_CANON: Dict[str, Dict[str, Any]] = {
    "mu": dict(
        aliases=["shear modulus", "shear moduli", "c44", "elastic constants",
                 "elastic moduli", "soec", "mu", "g", "lame parameter",
                 "second order elastic constants"],
        unit="GPa", plausible=(20.0, 200.0),
    ),
    "rho0": dict(
        aliases=["dislocation density", "rho0", "initial dislocation density",
                 "forest dislocation density", "mobile dislocation density"],
        unit="m^-2", plausible=(1e10, 1e17),
    ),
    "gamma0_dot": dict(
        aliases=["reference shear strain rate", "reference strain rate",
                 "strain rate", "deformation rate", "loading rate",
                 "gamma0_dot", "gamma dot 0"],
        unit="s^-1", plausible=(1e-8, 1e6),
    ),
    "srs": dict(
        aliases=["strain rate sensitivity exponent",
                 "rate sensitivity exponent", "m exponent",
                 "strain-rate sensitivity", "srs"],
        unit=None, plausible=(0.001, 200.0),
    ),
    "sigma0": dict(
        aliases=["friction stress", "lattice friction", "peierls stress",
                 "initial yield stress", "yield strength", "sigma0"],
        unit="MPa", plausible=(0.1, 2000.0),
    ),
    "twin_spacing": dict(
        aliases=["twin spacing", "twin boundary spacing", "twin thickness",
                 "lambda"],
        unit="nm", plausible=(1.0, 1000.0),
    ),
    "applied_stress": dict(
        aliases=["applied stress", "loading stress", "external stress"],
        unit="MPa", plausible=(0.1, 5000.0),
    ),
    "W": dict(
        aliases=["interface width", "well depth", "twin well depth"],
...
# ----------------------------------------------------------------------------
# The v8.8.0 scanner (pattern `\d+(?:\.\d+)?`) could not match:
#   · scientific notation          6.0e13, 1.0E+13, 3e-4
#   · ×-10^N notation               1.0×10^13, 1.0 x 10**13, 5.0*10^15
#   · non-stress units              m^-2, m-2, s^-1, /s, nm
# Consequently, Tier 2 (deterministic regex) was structurally blind to the
# very rows that contain rho0 and gamma0_dot values, so the cascade fell
# straight from Tier 1 → Tier 3 for those parameters, hurting coverage on
# the sparse set. The scanner below fixes that.
# ============================================================================
UNIT_TOKENS = {
    "GPa":  r"gpa",
    "MPa":  r"mpa",
...
def unit_regex_for(param_key: str) -> Optional[str]:
    ...
    Stress-family units (GPa/MPa/Pa) are mutually convertible and therefore
    accepted interchangeably; other parameters get only their canonical unit
    (or None for a dimensionless parameter)."""
...
def heuristic_extract(records, param_key, target_unit=None):
    ...
    v8.8.1 FIX: pass (b) uses merge_no_clash() so a duplicate 'unit' key from
    find_value_after() can never raise
    ``TypeError: dict() got multiple values for keyword argument 'unit'``.

    v8.8.2 UPGRADE:
      · scanner is now `_NUM_ANY` (sci-notation + ×-10^N aware)
      · unit list is now param-aware via unit_regex_for()
      · dropped the trailing `(?![a-z0-9])` guard on the bare-number regex,
        which used to reject '6.0e13' because it ends in a letter.
    """
    canon = PARAM_CANON.get(param_key, {})
...
# ============================================================================
# v8.8.2 — TIER 3 PRIOR-INFERENCE PROMPT (SEPARATE CONTRACT)
# ----------------------------------------------------------------------------
# Critical principle: do NOT loosen the Tier-1 prompt. Mixing "verbatim only"
# with "you may infer" in one prompt re-opens the hallucination door. Tier 3
# is a SEPARATE prompt with a SEPARATE contract, honouring the dual-mode
# pattern established in v8.2.0 (strict_extract vs reasoned_inference).
# ============================================================================
...
# ---- 10. GATEKEEPER (v8.8.2: adds conf_cap) --------------------------------
@dataclass
...
def gatekeep(items, param_key, provenance="llm", conf_cap=None):
    ...
    v8.8.2: `conf_cap` allows the caller to bound the confidence of
    prior-inferred candidates at 0.5 without touching the Tier-1 verbatim
    contract. If conf_cap is None, it defaults to 0.5 for the 'llm_prior'
    provenance and 1.0 otherwise — the safety net that makes inference
    acceptable to surface next to verbatim hits.
    """
    canon = PARAM_CANON.get(param_key, {})
    (lo, hi) = canon.get("plausible", (float("-inf"), float("inf")))
    ...
    if conf_cap is None:
        conf_cap = 0.5 if provenance == "llm_prior" else 1.0
...
# ============================================================================
# v8.8.2 — THREE-TIER CASCADE ORCHESTRATOR
# ----------------------------------------------------------------------------
# Tier 1  grounded LLM   (verbatim-only contract — UNCHANGED)
# Tier 2  deterministic regex (sci-notation + param-aware units)
# Tier 3  LLM PRIOR INFERENCE (separate prompt + separate contract)
#
# Returns (cands, records, diag) — a 3-tuple, matching the "companion patch"
# in the Restoring-Coverage document. Preserves the v8.7 world-knowledge
# path WITHOUT loosening the Tier-1 verbatim contract.
# ============================================================================
def recommend_param_values(param_key, material="Cu", value_hints=None,
    ...
    (candidates, records, diagnostics)
        candidates  : List[ValueCandidate]  best-first
        records     : List[Dict]  the records that fed tier-1/tier-2/tier-3
        diagnostics : Dict  per-parameter provenance accounting
    """
...
# ============================================================================
# ███ END v8.8.0 / v8.8.1 / v8.8.2 NER MODULE                              ██
# ============================================================================
...
PLASTICITY_ONTOLOGY: Dict[str, Dict[str, Any]] = {
    ...
    "sigma0": {
        ...
        "symbol": "σ₀",
        "aliases": PARAM_CANON["sigma0"]["aliases"] + [
...
class PlasticityFAISSRetriever:
    ...
    v8.8.0 deprecates this in favor of HybridRetriever.  Kept in place so
    existing session state and pickles still work.
    """
...
REASONING CHAINS (use these formulas, do NOT just look up a constant):
  ...
  mu (T, material):
    ...
    Step 2: Pick room-T baseline:
            ...
            Mg: 17 GPa, Cr: 115 GPa, Co: 75 GPa, Au: 27 GPa, Ag: 30 GPa,
            CoCrFeNi (HEA): 80 GPa.
  ...
  gamma0_dot (T, strain_rate, material):
    ...
    Step 2: If not, infer from the physical regime:
            ...
            - MD simulations: typically 1e6 to 1e8 s^-1
    Step 3: If the text gives no regime hint, default to 1e-3 s^-1.
    Step 4: Emit ONE JSON object with param="gamma0_dot",
            value=<number>, unit="s^-1", method="LLM_inferred" (or
            "explicit" if Path A), reasoning="<your 3-5 step chain>".
    Step 5: Output value in s^-1. NEVER omit this parameter.

  srs (m, material, T):
    Step 1: Identify material class.
    Step 2: Pick baseline:
            FCC metals (Cu, Al, Ni, Au, Ag): m = 20
            BCC metals (Fe, W, Mo, Cr):      m = 100
            HCP metals (Mg, Ti, Zn, Zr):     m = 50
            HEA / MPEA:                       m = 30
...
class PlasticityLatentMoEScorer:
    ...
    v8.8.2: prior-inferred candidates surface as method='prior inference'
    ...
    outrank verbatim-grounded hits.
    """
    ...
    def _method_expert(method: str) -> float:
        # v8.8.2: 'prior inference' / 'llm_prior' deliberately score lower
        # than 'explicit' / 'llm' — the cascade should prefer grounded hits.
        return {
...
class PlasticityRecommendationBundle:
    ...
    # v8.8.2: per-parameter cascade diagnostics (list of dicts, one per param)
...
class PlasticityRecommender:
    ...
    LatentMoE rank → priors.

    v8.8.2: the primary entry-point `recommend_grounded()` now runs the
    three-tier cascade for each parameter, preserving the Tier-1 verbatim
    contract while restoring the v8.7 world-knowledge coverage via a
    separately-prompted Tier-3 prior-inference step.
    """
    ...
    def __init__(self, db_dir: str = "json_metadatabase",
        ...
        # v8.8.2: opt-in prior inference (default True to restore 5/5)
    ...
    # ------------------------------------------------------------------
    # v8.8.2: primary entry point — three-tier cascade per parameter
    # ------------------------------------------------------------------
    def recommend_grounded(self, material: str, temp_k: float,
        ...
        """Run the three-tier cascade (grounded LLM → regex → prior inference)
        for each parameter in PARAM_ORDER, then LatentMoE-rank the union."""
        ...
        for i, param in enumerate(PARAM_ORDER):
            ...
            for c in cands:
                # Map ValueCandidate.provenance ('llm'|'heuristic'|'llm_prior')
                # onto the LoE's 'method' vocabulary. The scorer recognises
                # both spellings, so provenance survives to the bar chart.
    ...
    # ------------------------------------------------------------------
    # Legacy v8.7 pathway — retained as an explicit fallback for when the
    # hybrid retriever is unavailable.
    # ------------------------------------------------------------------
    def recommend(self, material: str, temp_k: float, strain_rate: float = 1e-3,
        ...
        for i, doc in enumerate(docs):
            ...
            for e in ext:
                ...
                # v8.8.2 provenance tag for legacy path
...
def _plr_render_parameter_selector(param: str,
    ...
    if best is not None:
        # v8.8.2: surface provenance at a glance in the radio label
        prov_tag = {
            "explicit": "✓ grounded",
            "llm_inferred": "~ llm",
...
def render_plasticity_recommender_sidebar(
    ...
    """Sidebar with v8.8.2 three-tier cascade + hybrid retrieval + priors."""
    st.subheader("🤖 Intelligent Plasticity Recommender v8.8.2")
    st.caption(
        "**v8.8.2 three-tier cascade:** Tier 1 verbatim-grounded LLM · "
        "Tier 2 deterministic regex (sci-notation + param-aware units) · "
        "Tier 3 LLM PRIOR INFERENCE (separate prompt, confidence-capped at "
        "0.5, bounded by PARAM_CANON plausible ranges).  Restores full "
        "5/5 coverage without loosening the Tier-1 contract."
    )
    ...
    # ── v8.8.2 — opt-in prior inference (restores full 5-parameter coverage) ──
    ...
    # ── v8.8.2: coverage metric + diagnostics dataframe ─────────────────────
    st.caption(
        f"Retrieval: **{bundle.retrieval_backend}** · "
        f"LLM: **{'yes' if bundle.llm_used else 'no (heuristic)'}**"
    )
    st.metric("Parameters with candidates", f"{bundle.coverage()}/5")
    ...
    if bundle.diagnostics:
        with st.expander("🔎 Per-parameter extraction diagnostics",
            ...
            st.caption(
                ...
                "at 0.5)  ·  **default_fallback** = gamma0_dot user-input "
                "guarantee."
            )
...
def render_recommender_bars_pub(bundle: PlasticityRecommendationBundle,
    ...
    provenance = [c.method for c in cands]  # scorer already carried method
...
def render_recommender_visuals_dashboard():
    ...
    st.caption(
        ...
        "candidates, sources, and distributions.  **v8.8.2** adds the "
        "three-tier cascade — see the sidebar diagnostics table for the "
...
def main():
    ...
    • <span style="color: green;">📐 TIER 2:</span> deterministic regex with sci-notation scanner (6.0e13, 1.0×10¹³) and param-aware unit tokens (m^-2, s^-1, nm).<br>
    • <span style="color: green;">🧠 TIER 3:</span> LLM PRIOR INFERENCE — separate prompt, separate contract, confidence capped at 0.5, bounded by PARAM_CANON plausible ranges. <b>Restores full 5/5 coverage WITHOUT loosening the Tier-1 verbatim contract.</b><br>
    • <span style="color: green;">🔒 GATEKEEPER:</span> unit conversion + plausibility + dedupe + confidence-cap + provenance tagging.<br>
    • <span style="color: green;">📊 COVERAGE METRIC:</span> 5/5 counter + per-parameter cascade diagnostics table.<br>
    • <span style="color: green;">🎨 COLORMAP ENGINE:</span> 60+ colormaps · score/index/uniform · adaptive text halo · one-click presets.<br>
    • <span style="color: green;">✍️ TYPOGRAPHY ENGINE:</span> independent annotation/legend/colorbar label/colorbar tick font sizes + annotation offset + auto head-room.<br>
    </div>
    """, unsafe_allow_html=True)
    ...
    # ========================================================================
    # MAIN CONTENT (mirror of the source file, unchanged beyond v8.8.2 hook)
    # ========================================================================
...
# ============================================================================
# REGRESSION TESTS (v8.8.1 / v8.8.2)
# ============================================================================
...
def _regression_test_v882() -> None:
    ...
    Verifies:
      1. Sci-notation scanner matches '6.0e13' and '1.0×10^13' for rho0.
      2. Param-aware unit tokens match 'm^-2' and 'm-2' for rho0.
      3. gatekeep() respects the conf_cap for 'llm_prior' provenance.
      4. 'llm_prior' provenance string is preserved (not silently folded
         into 'llm') by _norm_provenance().
    """
    # 1 + 2: sci-notation and param-aware units
...
# Uncomment either of the lines below (or run the module directly) to run
