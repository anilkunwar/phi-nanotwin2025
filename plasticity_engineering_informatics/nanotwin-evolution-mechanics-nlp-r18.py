...
# ███ + PLASTICITY PARAMETER INTELLIGENT RECOMMENDER v9.1.0              ███
...
# ============================================================================
PROVENANCE_MARKERS = {
    'llm':       dict(marker='D', ms=6.0, label='LLM-inferred (grounded)'),
    'heuristic': dict(marker='^', ms=7.5, label='Heuristic fallback'),
    'llm_prior': dict(marker='o', ms=6.5, label='LLM prior (no corpus evidence)'),
    'derived':   dict(marker='v', ms=7.0, label='Derived (side-note cluster)'),
}


def _norm_provenance(p):
    p = str(p).strip().lower()
    if any(k in p for k in ('prior', 'estimate', 'no evidence', 'world knowledge')):
        return 'llm_prior'
    if 'deriv' in p:
        return 'derived'
...
GAZETTEER: Dict[str, List[str]] = {
    ...
    "unit":          ["GPa", "MPa", "Pa", "s^-1", "s-1", "/s", "nm",
                      ...
                      "K", "C", "°C", "Celsius"],
    # ── v9.0.0 — physics-regime ρ₀ entity types ──────────────────────────
    "synthesis_method": [
    ...
    # ── v9.1.0 — theory-framework entity type for γ̇₀ disambiguation ────
    "theory_framework": [
        # Johnson-Cook / empirical power-law
...
def score_record(text, param_key, material="Cu", value_hints=None):
    ...
    for etype, w in (("related", .3), ("method", .8), ("structure", .5),
                     ...
                     # v9.0.0 — synthesis / architecture boosts
                     ("synthesis_method",       .7),
                     ("grain_architecture",     .6),
                     # ── v9.1.0 — theory-framework boost for γ̇₀ ────────
                     ("theory_framework",       .9)):
...
def extract_rho0_with_context(text: str, window: int = 400) -> List[Dict[str, Any]]:
    """Snippet-enhanced NER for ρ₀.

    For every ρ₀ mention, capture the (±`window`-char) snippet and look
    inside it for three contextual entities that physically determine the
    dislocation density:

      • synthesis_method    — sputter / electrodeposition / MBE / additive
      • grain_architecture  — columnar / equiaxed / single / polycrystal
      • twin_spacing        — twin lamella thickness λ in nm

    Returns one dict per ρ₀ mention.  `rho0_value` may be None when the
    mention is qualitative; the context factors are still populated so the
    regime classifier can fire even without a verbatim quantity.
    """
...
RHO0_REGIMES: Dict[Tuple[str, str], Dict[str, Any]] = {
    # (synthesis_key, architecture_key) -> {low, high, bench, desc}
...
def _norm_synthesis(s: Optional[str]) -> Optional[str]:
    """Collapse a free-text synthesis label to one of
    {sputter, mbe, additive, electrodeposit} or None."""
    s = (s or "").lower()
...
def _norm_architecture(a: Optional[str]) -> Optional[str]:
    """Collapse a free-text architecture label to one of
    {columnar, single, equiaxed} or None."""
    a = (a or "").lower()
    if "columnar" in a:
        return "columnar"
    if "single" in a:
...
def classify_rho0_regime(
    ...
    """Map (synthesis, architecture, λ) → physical ρ₀ regime.

    Returns {regime, low, high, bench, inferred, source}.  `inferred` is a
    log-interpolated center inside [low, high] based on λ, biased toward
    the regime benchmark when λ is unknown.  No hard quantity is emitted —
    the LLM is free to sample anywhere inside [low, high].
    """
    ...
    if regime is None:
        # Fall back to the ontology's PARAM_CANON plausible range, but
        # anchor the log-center at the material-agnostic default so the
        # LLM still has a bias point.
        return dict(
            regime="unclassified — using PARAM_CANON plausible range",
            low=1e10, high=1e17, bench=1e12,
            inferred=1e12, source="fallback")

    # Twin-lamella modulation inside the regime.
    # Map λ ∈ [5, 100] nm → pos ∈ [0.90, 0.10] in log-space:
    #   smaller λ ⇒ more CTB/ITB intersections ⇒ higher ρ₀.
...
# The JC / power-law / experimental / phase-field regimes are physically
# indistinguishable at typical corpus resolution (all cluster in 1e-4..1e-2).
...
def _norm_theory(t: Optional[str]) -> Optional[str]:
    """Collapse a free-text theory label to one of:
    {johnson_cook, power_law, cpfem, ddd, md, phase_field, experimental}
    or None."""
    t = (t or "").lower()
...
def classify_gamma0_regime(theory: Optional[str]) -> Dict[str, Any]:
    """Map a target theory framework → physical γ̇₀ regime.

    Returns {regime, low, high, bench, inferred, source}.  The
    classification is log-uniform across the union of regimes when the
    theory is unrecognised; that fallback is only exercised when the LLM
    is asked to reason without a target framework.
    """
...
def parse_side_note_table(text: str) -> List[Dict[str, Any]]:
    """Return a list of {param, value, unit, evidence} dicts.

    v8.9.0: rows whose numeric value collapses to zero or negative are
    rejected outright.  This closes the `shear_modulus_G = 0.000 GPa`
    degenerate that was propagating silently into the P–N route.
    """
    out: List[Dict[str, Any]] = []
...
def derive_sigma0_and_tau_p(
    ...
    """Legacy v8.8.3 bridge.  Kept for backward compat; the primary v8.9.0
    derivation now lives in `build_derived_routes`."""
...
def _pick_best(cands):
    if not cands:
        return None
    trust = {'llm': 3, 'heuristic': 2, 'derived': 2, 'llm_prior': 1, 'missing': 0}
    def key(c):
...
def build_derived_routes(ctx, mu_GPa=None, w_mode='auto') -> List[DerivedRoute]:
    ...
    elif w_cap and ctx['w'].provenance in ('llm', 'heuristic') \
...
def build_rho0_physics_inference_prompt(
    ...
    """Prompt for physics-regime ρ₀ inference.

    Distinct from `build_prior_inference_prompt` because it *anchors* the
    LLM to a physically-derived [low, high] window and forces a
    three-factor reasoning chain with a `dominant_factor` tag.
    """
...
MANDATORY REASONING CHAIN (write each step into `reasoning`):
  ...
  Step 1 — SYNTHESIS sets the decade baseline:
            • Sputter / PVD: atoms arrive with low KE; dislocations
              annihilate at the free surface during growth → 1e13–1e14 m^-2.
            • Electrodeposition: organics + H co-deposition + constrained
              growth → 1e14–1e15 m^-2 (columnar),
              1e16–1e17 m^-2 (equiaxed, additive-assisted).
            • MBE epitaxial: lowest measurable, 1e11–1e12 m^-2.

  Step 2 — GRAIN ARCHITECTURE refines within the baseline:
            • Columnar (esp. (111)-oriented): dislocations glide to
              column walls and annihilate → LOWER half of the regime.
            • Equiaxed / nanocrystalline: random GB intersections trap
              dislocations → UPPER half of the regime, +1–2 decades.
            • Single crystal: bottom of the plausible range.

  Step 3 — TWIN LAMELLA SPACING λ MODULATES within the regime:
            • λ ≤ 10 nm → many CTB/ITB intersections, high nucleation
              density → push toward UPPER bound of the regime.
            • λ ≈ 20–30 nm → stay near the regime benchmark.
            • λ ≥ 50 nm → relaxed growth → push toward LOWER bound.

  Step 4 — COMBINE multiplicatively in log-space:
              log10(ρ₀) ≈ log10(ρ_synthesis)
                         + Δ_architecture
                         + α · log10(λ_ref / λ)
            with Δ_architecture ∈ [0, +2] (columnar → 0, equiaxed → +2)
            and α ≈ 0.3–0.5.
            Final ρ₀ ∈ [{regime['low']:.2e}, {regime['high']:.2e}] m^-2,
            log-center ≈ {regime['inferred']:.3e} m^-2.

STRICT RULES:
  1. Confidence MUST NOT exceed 0.50 (physics-regime inference, not a
     measured value).
  2. Propose 2–3 candidates, all inside the regime range, biased toward
     the log-center.
  ...
  5. Stay strictly inside the PARAM_CANON plausible range [{lo:g}, {hi:g}].
...
def build_gamma0_theory_aware_prompt(material: str,
    ...
    """Theory-tagged γ̇₀ extraction prompt.

    The LLM is given the regime table a priori and must tag every
    extracted value with `theory`.  This is what makes the NER
    *understand* the literature rather than just pattern-match.
    """
...
The same symbol γ̇₀ means physically different things across modelling
frameworks.  Before extracting any value, you MUST identify which
theoretical framework the EVIDENCE record uses, and tag the extraction
with the matching `theory` field.  The regimes are:

  FRAMEWORK              TYPICAL γ̇₀              DESCRIPTION
  ───────────────────────────────────────────────────────────────────
  johnson_cook           1e-4 to 1e-3 s⁻¹       Johnson-Cook empirical fit
  power_law              1e-4 to 1e-3 s⁻¹       Power-law plasticity fit
  cpfem                  1e-4 to 1e-3 s⁻¹       Slip-system reference γ̇₀
                                                  (CPFEM/VPSC/Taylor)
  ddd                    1e3 to 1e4 s⁻¹         Discrete Dislocation
                                                  Dynamics (ParaDiS)
  md                     1e6 to 1e8 s⁻¹         Molecular Dynamics
  phase_field            1e-4 to 1e-2 s⁻¹       Phase-field continuum
  experimental           1e-4 to 1e-3 s⁻¹       Tensile / SHPB test

THEORY IDENTIFICATION RULES (examine the SAME record as the γ̇₀ value):
  • "Johnson-Cook", "J-C", "JC model"           → johnson_cook
  • "power law", "power-law plasticity"          → power_law
  • "crystal plasticity", "CPFEM", "slip system",
    "Taylor factor", "VPSC", "Schmid"            → cpfem
  • "dislocation dynamics", "DDD", "ParaDiS",
    "mesoscale dislocation"                     → ddd
  • "molecular dynamics", "MD simulation",
    "atomistic", "EAM potential"                → md
  • "phase-field", "phase field", "Allen-Cahn"   → phase_field
  • "tensile test", "SHPB", "split Hopkinson",
    "quasi-static test"                          → experimental

STRICT RULES:
1. Report ONLY numbers that appear VERBATIM in an EVIDENCE record.
2. The `theory` field MUST be EXACTLY one of:
   {{"johnson_cook", "power_law", "cpfem", "ddd", "md", "phase_field",
     "experimental", "unspecified"}}
3. The `theory` tag MUST reflect the framework of the SAME record that
   contains the γ̇₀ value — not the user's target framework.
4. If the record does not state a framework, use "unspecified".
5. Convert units to s⁻¹ (e.g., 1 /ms = 1e3 s⁻¹, 1 /ns = 1e9 s⁻¹).
6. Output JSON ARRAY ONLY. Schema per element:
   [{{"value": 0.001, "unit": "s^-1", "value_gpa": null,
      "property_label": "reference strain rate",
      "method": "explicit",
      "theory": "cpfem",
      "evidence_id": 0,
      "evidence_span": "verbatim quote <=20 words",
      "confidence": 0.9,
      "reasoning": "Record mentions 'crystal plasticity slip system
                     reference rate γ̇₀ = 1e-3 s⁻¹'."}}]
7. If nothing qualifies, answer [].
...
def build_gamma0_theory_inference_prompt(
    ...
    """Physics-regime γ̇₀ inference — anchored to the target theory."""
    canon = PARAM_CANON.get("gamma0_dot", {})
...
MANDATORY REASONING CHAIN (write each step into `reasoning`):
  ...
  Step 1 — IDENTIFY the theory framework.  The framework determines the
            *purpose* of γ̇₀:
              • Johnson-Cook / power-law / experimental:
                  γ̇₀ is the quasi-static anchor for normalised overstress
                  (σ/σ_y)(ε̇/γ̇₀).  Typical γ̇₀ ∈ [1e-4, 1e-3] s⁻¹.
              • CPFEM:
                  γ̇₀ is the slip-system reference rate in the
                  viscoplastic flow rule
                  γ̇^α = γ̇₀ (|τ^α|/g^α)^(1/m) sgn(τ^α).
                  Typical γ̇₀ ∈ [1e-4, 1e-3] s⁻¹.
              • DDD:
                  γ̇₀ is imposed *high* (1e3–1e4 s⁻¹) to keep the
                  simulation timestep tractable; the MD-style
                  overstress is sacrificed for ensemble averaging.
              • MD:
                  γ̇₀ is the atomistic attempt frequency scaled to the
                  strain-rate unit (1e6–1e8 s⁻¹).
              • Phase-field:
                  γ̇₀ is the continuum quasi-static rate (1e-4–1e-2 s⁻¹).

  Step 2 — CROSS-CHECK with material and temperature:
              • Cu at 300 K, JC: γ̇₀ ≈ 1e-3 s⁻¹.
              • Cu at 300 K, DDD: γ̇₀ ≈ 5e3 s⁻¹.
              • Cu at 300 K, MD:  γ̇₀ ≈ 1e7 s⁻¹.

  Step 3 — COMBINE: final γ̇₀ ∈ [{regime['low']:.2e}, {regime['high']:.2e}] s⁻¹,
            log-center ≈ {regime['inferred']:.3e} s⁻¹.

STRICT RULES:
  1. Confidence MUST NOT exceed 0.50 (theory-regime inference).
  2. Propose 2–3 candidates inside the regime, biased toward the center.
...
# ============================================================================
# ███ v9.1.0 — SNIPPET-ENHANCED γ̇₀ EXTRACTOR                          ███
# ============================================================================
def extract_gamma0_with_context(text: str,
                                 window: int = 500) -> List[Dict[str, Any]]:
    """Snippet-enhanced NER for γ̇₀.

    For every γ̇₀ mention, capture the ±`window`-char snippet and look
    inside it for theory-framework keywords (JC, DDD, CPFEM, MD,
    phase-field).  Returns one dict per mention with theory attribution.

    The window is larger than for ρ₀ (500 vs 400) because theory
    mentions often appear in the *preceding* paragraph (e.g., "Using a
    Johnson-Cook fit … the reference strain rate γ̇₀ was set to …").
    """
...
class ValueCandidate:
    ...
    reasoning: str = ""
    dominant_factor: str = ""
    theory: str = ""              # ── v9.1: johnson_cook | power_law | cpfem
                                  #         | ddd | md | phase_field | experimental
                                  #         | unspecified
...
def gatekeep(items, param_key, provenance="llm", conf_cap=None):
    """Unit conversion + plausibility + dedupe + confidence cap.

    v8.9.0: rejects v ≤ 0 for params with strictly positive lower bound.
    v9.0:   carries `dominant_factor` through from LLM output.
    v9.1:   carries `theory` through from LLM output.
    """
    canon = PARAM_CANON.get(param_key, {})
    (lo, hi) = canon.get("plausible", (float("-inf"), float("inf")))
...
def recommend_param_values(param_key, material="Cu", value_hints=None,
    ...
    if use_llm and records:
        try:
            ...
            cands = gatekeep(parse_llm_json(raw), param_key, provenance="llm")
        except Exception as e:
            diag["reason"] += f"llm failed: {e}; "
            logger.warning("Tier-1 LLM failed (%s) → heuristic fallback: %s",
                           param_key, e)
    diag["n_llm"] = len(cands)

    if not cands and records:
        cands = gatekeep(heuristic_extract(records, param_key),
                         param_key, provenance="heuristic")
    diag["n_heuristic"] = len(cands)

    if not cands and use_llm and allow_prior_inference:
        try:
            prompt = build_prior_inference_prompt(param_key, material, records)
            raw = ollama_extract(prompt, model=ollama_model)
            cands = gatekeep(parse_llm_json(raw), param_key,
                             provenance="llm_prior", conf_cap=0.5)
            diag["reason"] += "no corpus evidence → prior inference; "
        except Exception as e:
    ...
    diag["tier"] = ("llm" if diag["n_llm"] else
                    ...
                    "llm_prior" if cands else "none")
    diag["n_prior"] = len(cands) if diag["tier"] == "llm_prior" else 0
...
def recommend_rho0_physics(
    ...
    """Physics-grounded ρ₀ cascade with snippet-enhanced NER.

    Returns (candidates, snippet_findings, regime_dict).

    Tier 1  — grounded LLM verbatim extraction (generic cascade, but with
              prior inference disabled).
    Tier 2  — snippet-enhanced NER: per-ρ₀-mention context capture.
              Any Tier-1 candidate whose value matches a snippet is
              annotated with the snippet's (synthesis, arch, λ), which is
              what feeds the regime expert downstream.
    Tier 3  — physics-regime LLM inference inside the classifier window.
    Tier 3b — pure-classifier fallback if the LLM is unreachable.
    """
    if retriever is None:
        retriever = get_retriever()
    ...
    # ── Tier 2 — snippet context per mention ──────────────────────────
    ...
    # Annotate matching Tier-1 candidates with snippet context.
    for c in cands:
    ...
    # Merge user-supplied factors into the findings so the regime expert
    # fires even when the corpus did not surface any context.
    user_syn = synthesis
    ...
    # ── Tier 3 — physics-regime LLM inference ─────────────────────────
    if not cands:
        try:
...
def recommend_gamma0_theory_aware(
    ...
    """Theory-aware γ̇₀ cascade.

    Tier 1  — theory-tagged grounded LLM extraction.
    Tier 2  — snippet-enhanced NER captures the theory context per mention.
              Tier-1 candidates whose value matches a snippet are
              annotated with the snippet's theory tag.
    Tier 3  — theory-regime LLM inference inside the classifier window.
    Tier 3b — pure-classifier fallback.
    """
    if retriever is None:
        retriever = get_retriever()
    ...
    if records:
        try:
            ...
            cands = gatekeep(parsed_items, "gamma0_dot", provenance="llm")
            # Re-attach theory tags if the gatekeeper dropped them in
            # any cascaded path.
            for c in cands:
                for item in parsed_items:
                    try:
                        iv = float(item.get("value_gpa")
                                   if item.get("value_gpa") is not None
                                   else item.get("value"))
                    except (TypeError, ValueError):
                        continue
                    if iv <= 0:
    ...
    # Annotate matching Tier-1 candidates with snippet theory
    for c in cands:
        if c.theory and c.theory != "unspecified":
            continue   # already tagged by LLM
        for f in snippet_findings:
    ...
    # Merge user-supplied theory into findings so the theory expert
    # fires even when the corpus did not surface any context.
    if target_theory:
        snippet_findings.append({
    ...
    # ── Tier 3 — theory-regime LLM inference ──────────────────────
    if not cands and target_theory:
        try:
            prompt = build_gamma0_theory_inference_prompt(
                material, target_theory, records)
            raw = ollama_extract(prompt, model=ollama_model)
            cands = gatekeep(parse_llm_json(raw), "gamma0_dot",
                             provenance="llm_prior", conf_cap=0.5)
...
class PlasticityCandidate:
    ...
    reasoning: str = ""
    clamped: bool = False
    theory: str = ""              # ── v9.1: γ̇₀ theory tag (empty for others)
...
class PhysicsRegimeExpert:
    """Rewards ρ₀ candidates that land inside the (synthesis, arch, λ)
    regime window, with a Gaussian peak at the log-center.

    Outside the window the score decays exponentially:
        exp(−distance_in_decades / falloff_decades)
    so a candidate 1 decade outside the regime scores ≈ 0.37, 2 decades
    ≈ 0.14, 3 decades ≈ 0.05 — a gentle but decisive penalty.
    """
...
class TheoryRegimeExpert:
    """Rewards γ̇₀ candidates that land inside the theory-predicted regime.

    Used for gamma0_dot only.  Outside the regime, exponential decay
    (1.5 decades ≈ 0.37 — slightly gentler than the ρ₀ expert because
    γ̇₀ is less sharply localised within a regime).
    """
    ...
    def score(self, value_si: float,
        ...
        Two factors combine multiplicatively:
          (a) Theory-tag match: did the LLM tag this candidate with the
              same theory the user asked for?  (binary-ish)
          (b) Regime membership: does the value fall inside the
              theory's [low, high] band?
        """
        try:
            v = float(value_si)
        except (TypeError, ValueError):
        ...
        # (a) Theory-tag match — strong binary prior
        tag_score = 1.0
        if target_theory and candidate_theory:
            ...
            if t_target and t_cand:
                if t_target == t_cand:
                    tag_score = 1.0
                # JC / power_law / experimental / phase_field share the
                # same quasi-static regime — treat as near-matches.
                elif ({t_target, t_cand} <= _GAMMA0_QUASI_STATIC_GROUP):
                    tag_score = 0.85
                else:
                    tag_score = 0.05  # cross-theory candidate is suspicious
            elif t_cand and t_target and t_cand != t_target:
                tag_score = 0.10
            # if candidate_theory is unspecified, fall back to regime
            # membership only
...
class PlasticityLatentMoEScorer:
    ...
    def score(self, extractions, target_material, target_temp,
              ...
              target_theory=None):
        """Score all extraction candidates.

        For ρ₀, if any of (synthesis, architecture, twin_spacing) is known,
        the PhysicsRegimeExpert is activated with weight 0.25 and the
        remaining weights are re-normalised accordingly.

        For γ̇₀ (v9.1), if a target_theory is supplied, the
        TheoryRegimeExpert is activated with weight 0.25; reasoning weight
        is raised to 0.40 because the theory chain needs to be walked.
        """
        ...
        for ext in extractions:
            ...
            # ── 1. Unit normalisation + physical clamp ────────────────
            try:
            ...
            if p == "rho0" and regime_active_rho0:
                w_mat, w_t, w_s, w_m, w_c, w_r, w_reg = (
                    ...
                    0.05,   # strain rate
                    0.05,   # method prior
                    0.05,   # LLM self-confidence
                    0.30,   # reasoning quality
                    0.25,   # physics regime  ← dominant
                )
            elif p == "gamma0_dot" and theory_active_gdot:
                w_mat, w_t, w_s, w_m, w_c, w_r, w_reg = (
                    0.10,   # material  — γ̇₀ is largely theory-bound
                    0.05,   # thermal   — small
                    0.05,   # strain rate — *this* is the parameter
                    0.10,   # method
                    0.05,   # confidence
                    0.40,   # reasoning — needs the theory chain
                    0.25,   # theory regime  ← dominant disambiguator
                )
            else:
            ...
            # Only set theory for gamma0_dot candidates (avoid noise on
...
class PlasticityRecommender:
    ...
    def recommend_grounded(self, material, temp_k, strain_rate=1e-3,
        ...
        for i, param in enumerate(PARAM_ORDER):
            ...
            if param == "rho0":
                ...
                diag = dict(
                    ...
                    n_llm=sum(1 for c in cands if c.provenance == "llm"),
                    n_heuristic=0,
                    n_prior=sum(1 for c in cands
                                if c.provenance == "llm_prior"),
                    tier=("llm" if any(c.provenance == "llm" for c in cands)
                          else "llm_prior"),
            ...
            for c in cands:
                method = ("explicit" if c.provenance == "llm"
                          else "heuristic" if c.provenance == "heuristic"
        ...
        for p_key in context_keys:
            ...
            hits = gatekeep(heuristic_extract(recs, p_key),
                            p_key, provenance="heuristic")
            context_bucket[p_key] = hits
        ...
        for c in derived_v883:
            ...
            diagnostics.append(dict(param=target, records=0, n_llm=0,
                                    n_heuristic=0, n_prior=0, tier="derived",
                                    reason=f"derived via {c.method}"))
        ...
        # ── v8.9.0 — derived σ₀ consensus injection (completes 5/5) ─────
        ...
        try:
            ...
            if cons:
                ...
                diagnostics.append(dict(
                    ...
                    n_heuristic=0, n_prior=0, tier='derived_consensus',
                    reason=(f"▼ consensus σ₀ = {cons['value_MPa']:.1f} MPa "
                            f"from {cons['n_routes']} routes "
                            f"(spread {cons['rel_spread']:.0%}, "
                            f"conf {cons['conf']:.2f})")))
        except Exception:
            logger.warning("v8.9.0 derived-σ₀ bridge failed:\n%s",
                           traceback.format_exc())
    ...
    def recommend(self, material, temp_k, strain_rate=1e-3, max_docs=20,
        ...
        for i, doc in enumerate(docs):
            ...
            for e in ext:
                ...
                e["_provenance"] = ("llm" if e.get("method") in
...
def _plr_render_parameter_selector(param: str,
    ...
    if best is not None:
        prov_tag = {
            ...
            "derived":         "△ derived",
            "derived_consensus": "▼ consensus",
            "hall_petch_intercept":           "△ HP-intercept",
            "taylor_factor_cross_conversion": "△ σ₀/M",
            "peierls_nabarro_formula":        "△ P-N formula",
            "heuristic":       "· regex",
            "side_note_table": "· side-note",
            "default_fallback":"! default",
            "physics_regime_inference": "🧭 regime",
            "theory_regime_inference":  "⚡ theory",
        }.get(best.method, best.method)
        # γ̇₀ candidates get an extra theory tag in the label
        theory_tag = ""
    ...
    for i, c in enumerate(bundle.candidates.get(param, [])[1:], start=1):
        ...
        options.append((
            f"   Alt {i}: {_pl_fmt(param, c.value_si)} "
...
def render_plasticity_recommender_sidebar(
    ...
    st.subheader("🤖 Intelligent Plasticity Recommender v9.1.0")
    st.caption(
        "**v9.1.0 Theory-aware γ̇₀ NER:** the same symbol γ̇₀ means "
        "physically different things in Johnson-Cook (≈1e-3 s⁻¹), CPFEM "
        "(≈1e-3 s⁻¹), DDD (≈5e3 s⁻¹), and MD (≈1e7 s⁻¹) frameworks. "
        "The LLM now tags every γ̇₀ extraction with its `theory` and a "
        "dedicated LatentMoE expert (weight 0.25) penalises cross-theory "
        "tag mismatches. The γ̇₀ histogram becomes **multi-modal with "
        "theory-labelled peaks** instead of a flat 8-decade smear."
    )
    st.caption(
        "**v9.0.0 Physics-grounded ρ₀ inference:** snippet-enhanced NER "
        "captures (synthesis, grain-architecture, twin-lamella λ) around "
        "each ρ₀ mention and feeds a three-factor regime classifier. "
        "**v8.9.0 Friction & Lattice Stress Lab** derives σ₀ from side-"
        "note context via HP(d), HP(λ), solver-law inversion, and P–N+"
        "Taylor floor."
    )
    ...
    # ── v9.0.0 — ρ₀ physics-regime qualitative inputs ─────────────────
    ...
    # ── v9.1.0 — γ̇₀ theory-regime qualitative input ─────────────────
    ...
    if run_btn:
        ...
        try:
            ...
            status.success(
                ...
                f"across {bundle.coverage()}/{len(PARAM_ORDER)} params "
...
def render_friction_stress_lab():
    ...
    st.caption(
        ...
        'recommender\'s side-note context. Derived rows carry the ▼ glyph.')
    ...
    glyph = {'llm': 'D', 'heuristic': '^', 'llm_prior': 'o',
             'derived': '▼', 'missing': '–'}
    cols = st.columns(5)
...
def render_recommender_bars_pub(bundle, style):
    ...
    fig = plot_candidate_scores(
        ...
        best_idx=0, param_title=meta['title'], param_symbol=meta['symbol'],
    ...
    for p_idx, p in enumerate(PARAM_ORDER):
        ...
        for j, c in enumerate(bundle.candidates.get(p, [])):
            ...
            labels.append(f"{_pl_fmt(p, c.value_si)}<br><i>{c.method}</i>"
                          f"{theory_extra}<br>score={c.score:.2f}")
            parents.append(p); values.append(max(c.score, 0.01))
            rgba = cmap(float(np.clip(c.score, 0, 1)))
            colors.append(f'rgb({int(rgba[0]*255)},{int(rgba[1]*255)},'
                          f'{int(rgba[2]*255)})')
...
def render_recommender_visuals_dashboard():
    ...
    st.caption(
        ...
        "candidates. v9.1.0 adds theory-aware γ̇₀ inference.")
...
def main():
    ...
    <strong>✅ PURE FFT SPECTRAL + AI PLASTICITY RECOMMENDER v9.1.0:</strong><br>
    ...
    • <span style="color: green;">🧮 v8.9.0 FRICTION STRESS LAB:</span> σ₀ derived from context via HP(d), HP(λ), solver-law inversion, P–N+Taylor floor — ▼ consensus.<br>
    • <span style="color: green;">🔬 v9.0.0 PHYSICS-GROUNDED ρ₀:</span> snippet-enhanced NER ∧ three-factor (synthesis, architecture, λ) regime classifier ∧ physics-regime LatentMoE expert (w=0.25).<br>
    • <span style="color: green;">⚡ v9.1.0 THEORY-AWARE γ̇₀:</span> Johnson-Cook vs CPFEM vs DDD vs MD disambiguation via `theory` tagging ∧ theory-regime LatentMoE expert (w=0.25).<br>
    ...
    with st.sidebar:
        ...
        if st.button("🔄 Reset to Defaults", type="secondary"):
            ...
            st.rerun()
        st.markdown("---")
        with st.expander("🧠 AI Plasticity Recommender v9.1.0", expanded=False):
...
def _regression_test_v883() -> None:
    ...
    assert 'derived' in PROVENANCE_MARKERS
...
def _regression_test_v890() -> None:
    """v8.9.0 — G=0 rejection + derived consensus smoke test."""
    bad_table = (
        "| Field         | Value    |\n"
        "| Shear modulus | 0.000    |\n"
        "| Shear modulus | 0 | GPa |\n"
    ...
    zero_frac = [{"value": 0.0, "unit": "GPa", "confidence": 0.9,
                  "property_label": "shear modulus", "source": "test"}]
    assert gatekeep(zero_frac, "shear_modulus_G", provenance="llm") == [], \
    ...
    ctx = {
        'sigma_y': CtxField(value=300.0, conf=0.9, provenance='llm'),
        'd':       CtxField(value=1000.0, conf=0.7, provenance='heuristic'),
        'lam':     CtxField(value=20.0, conf=0.9, provenance='llm'),
        'b':       CtxField(value=0.256, conf=0.9, provenance='llm'),
        'nu':      CtxField(value=0.34, conf=0.9, provenance='llm'),
        'M':       CtxField(value=3.06, conf=0.9, provenance='llm'),
        'k_y':     CtxField(value=0.11, conf=0.9, provenance='llm'),
...
def _regression_test_v90() -> None:
    """v9.0 — physics-regime ρ₀ extraction regression."""
    # 1. Regime classifier monotone in λ within a fixed regime
    ...
    # 2. Architecture axis: equiaxed ≫ columnar within a synthesis
    ...
    # 3. Snippet extractor captures the three factors
    ...
    # 4. PhysicsRegimeExpert rewards in-regime, penalises out
    ...
    # 5. Scorer weight switch fires only for rho0
    ...
    extractions = [
        ...
        {"param": "rho0", "value": 1e11, "unit": "m^-2",
         ...
         "_source_title": "regression", "_provenance": "llm"},
    ]
    ...
    assert rank_ctx <= rank_no, \
        "context-aware regime expert should not demote the in-regime candidate"
...
def _regression_test_v91() -> None:
    """v9.1 — theory-aware γ̇₀ extraction regression."""
    # 1. Classifier — distinct regimes per theory
    ...
    # 2. Snippet extractor captures theory tag
    ...
    # 3. TheoryRegimeExpert — same value, different theory tags
...
# _regression_test_v890()
# _regression_test_v90()
# _regression_test_v91()
