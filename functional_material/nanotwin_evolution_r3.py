...
class NanotwinnedCuSolver:
    ...
    def evolve_twin_field(self, sxx, syy, sxy, eps_p_mag):
        ...
        try:
            ...
            # Apply periodic boundary conditions (already inherent in numba functions)
...
class SimulationMonitor:
    ...
    def create_field_comparison_plot(initial, final, N, dx):
        ...
        axes[0, 1].imshow(initial['sigma_eq']/1e9, cmap='hot', vmin=0, vmax=np.percentile(initial['sigma_eq'], 95)/1e9)
        ...
        axes[1, 1].imshow(final['sigma_eq']/1e9, cmap='hot', vmin=0, vmax=np.percentile(final['sigma_eq'], 95)/1e9)
        ...
        axes[2, 2].set_title('Δ Twin Spacing (nm)')
        for ax in axes.flat:
            ax.set_xticks([])
            ax.set_yticks([])
        plt.tight_layout()
...
# ============================================================================
# STREAMLIT APPLICATION WITH ERROR HANDLING AND PLOTLY VISUALIZATIONS
# ============================================================================
def main():
    st.set_page_config(
    ...
    • Comprehensive error handling and monitoring with Plotly 3D visualizations
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.sidebar.button("🚀 Initialize Simulation", type="primary", use_container_width=True):
        # Validate parameters
        params = {
            'N': N,
            'dx': dx,
    ...
    if 'initialized' in st.session_state and st.session_state.initialized:
        ...
        tab1, tab2, tab3, tab4 = st.tabs(["📐 Initial Geometry", "▶️ Run Simulation", "📊 Results & Visualization", "📤 Export"])
        ...
        with tab1:
            ...
            with col2:
                ...
                # Compute statistics
                valid_h = h[(h > 5) & (h < 50)]
                avg_spacing = np.mean(valid_h) if len(valid_h) > 0 else 0
                ...
                st.write(f"Grid size: {N}×{N}")
                st.write(f"Domain size: {N*dx:.1f}×{N*dx:.1f} nm²")
            ...
            if st.checkbox("Show 3D Visualization"):
                ...
                fig_3d.update_layout(
                    title='3D Gradient Magnitude (Twin Boundary Indicator)',
                    scene=dict(
                        xaxis_title='x (nm)',
                        yaxis_title='y (nm)',
                        zaxis_title='|∇φ|'
        ...
        with tab3:
            if 'results_history' in st.session_state:
                ...
                # NEW: Interactive Plotly 3D Visualizations
                ...
                with col1:
                    ...
                    fig_phi = go.Figure(data=[go.Surface(
                        ...
                        colorbar=dict(title="φ"),
                        contours={
                            "z": {"show": True, "size": 0.2, "color": "white"}
                        }
                    )])
                    fig_phi.update_layout(
                        title="Twin Order Parameter (3D)",
                        scene=dict(
                ...
                with col2:
                    # 3D von Mises stress - FIXED WITH ADAPTIVE COLORMAP
                    stress_gpa = results['sigma_eq'] / 1e9
                    vmax_3d = np.percentile(stress_gpa, 98)
                    vmin_3d = np.percentile(stress_gpa, 2)
                    
                    fig_stress = go.Figure(data=[go.Surface(
                        z=stress_gpa,
                        colorscale='Hot',
                        cmin=vmin_3d,
                        cmax=vmax_3d,
                    ...
                    fig_stress.update_layout(
                        title=f"Von Mises Stress (3D)<br><sup>Range: [{vmin_3d:.2f}, {vmax_3d:.2f}] GPa</sup>",
                        scene=dict(
                            xaxis_title='X (grid points)',
                            yaxis_title='Y (grid points)',
                ...
                # Cross-section plots
                st.subheader("Field Cross-Sections (Mid-line)")
                mid_idx = params['N'] // 2
                
                fig_cross = make_subplots(rows=2, cols=2, 
                                         subplot_titles=("φ (X-cut)", "σ_eq (X-cut)", 
                                                        "Twin Spacing (X-cut)", "Yield Stress (X-cut)"))
                
                fig_cross.add_trace(go.Scatter(y=results['phi'][mid_idx, :], mode='lines', 
                                              name='φ', line=dict(color='blue')), row=1, col=1)
                fig_cross.add_trace(go.Scatter(y=results['sigma_eq'][mid_idx, :]/1e9, mode='lines',
                                              name='σ_eq (GPa)', line=dict(color='red')), row=1, col=2)
                fig_cross.add_trace(go.Scatter(y=results['h'][mid_idx, :], mode='lines',
                                              name='h (nm)', line=dict(color='purple')), row=2, col=1)
                fig_cross.add_trace(go.Scatter(y=results['sigma_y'][mid_idx, :]/1e6, mode='lines',
                                              name='σ_y (MPa)', line=dict(color='green')), row=2, col=2)
                
                fig_cross.update_layout(height=500, showlegend=False, 
                                       margin=dict(l=20, r=20, t=40, b=20))
                fig_cross.update_xaxes(title_text="Y position (grid points)")
                ...
                st.subheader("Evolution Analysis: Initial vs Final State")
        ...
        with col2:
            ...
            <li>Complete physics-based modeling with spectral elasticity solver</li>
            <li>Multiple geometry configurations (standard twins, defects)</li>
            <li>Real-time monitoring and convergence tracking</li>
            <li>Interactive Plotly 3D visualizations for stress and microstructure</li>
            <li>Comprehensive export functionality (ZIP, NPZ, CSV, PyTorch)</li>
            <li>Robust error handling with detailed diagnostics</li>
            </ul>
            </div>
            </div>
            """, unsafe_allow_html=True)
