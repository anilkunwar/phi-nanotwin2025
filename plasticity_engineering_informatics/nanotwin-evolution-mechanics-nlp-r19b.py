        ...
        if b_m > 0 and w_m > 0 and (1.0 - nu_v) > 1e-3:
            ...
            if tau_p_pn > 0:
                ...
                reasoning = (
                    ...
                    f"Step 6: τ_P = {tau_p_pnpn/1e6 if False else tau_p_pn/1e6:.4g} MPa."
                )
...
def render_plasticity_recommender_sidebar(
    ...
    st.subheader("🤖 Intelligent Plasticity Recommender v9.3.1")
    st.caption(
...
def render_recommender_visuals_dashboard():
    ...
    st.caption(
        ...
        "candidates. v9.3.1: fine key `physics_inferred` replaces the "
        "misleading `derived`; the bar-chart tab offers a 🔓 audit "
...
def main():
    ...
    <strong>✅ PURE FFT SPECTRAL + AI PLASTICITY RECOMMENDER v9.3.1:</strong><br>
    ...
    • <span style="color: green;">🔬 v9.3.1 HONEST NAMING:</span> `derived` → `physics_inferred`; coarse label reads "Deterministic (regex · physics-inferred)". <strong>NEW:</strong> 🔓 audit toggle reveals the full competitive landscape without moving the pinned winner.<br>
    ...
    with st.sidebar:
        ...
        if st.button("🔄 Reset to Defaults", type="secondary"):
            ...
            st.rerun()
        st.markdown("---")
        with st.expander("🧠 AI Plasticity Recommender v9.3.1", expanded=False):
...
def _run_regression_suite() -> None:
    """Run the v8.8–v9.3.1 battery.  Names resolve via globals().get()
