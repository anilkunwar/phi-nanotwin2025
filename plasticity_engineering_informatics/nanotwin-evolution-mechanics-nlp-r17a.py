...
# ███ + PLASTICITY PARAMETER INTELLIGENT RECOMMENDER v9.0.1              ███
# ███ PUBLICATION-QUALITY VISUALS DASHBOARD                             ███
# ███ (all previous version banners retained — see history)             ███
# ███                                                                   ███
# ███ v9.0.1 — INVERSE-SRS CONVENTION CLEANUP                           ███
# ███   · `PLASTICITY_ONTOLOGY["srs"]` label rewritten:                  ███
# ███       "Strain-Rate Sensitivity Exponent"                           ███
# ███     → "Inverse Strain-Rate Sensitivity Exponent (1/m_conv)"        ███
# ███     symbol rewritten: `m` → `m_inv`                                ███
# ███   · `PARAM_META["srs"]` symbol + title brought in sync.            ███
# ███   · NEW `_normalize_srs(raw_value)` — convention auto-detector:    ███
# ███       raw < 1.0  → almost certainly conventional m_conv,           ███
# ███                     invert and return (1/raw, "conventional")      ███
# ███       raw ≥ 1.0  → already inverse, return (raw, "inverse")        ███
# ███   · `gatekeep` auto-inverts conventional-form srs values before    ███
# ███     the plausibility check, so a corpus paper reporting textbook   ███
# ███     m_conv = 0.05 (Cu) is no longer silently dropped.              ███
# ███   · `ValueCandidate` gains `convention` field:                     ███
# ███       "inverse" | "conventional_inverted" | "ambiguous" | ""       ███
# ███     carried through gatekeep, surfaced by to_display().            ███
# ███   · Sidebar ρ₀-adjacent caption for the srs card explains the       ███
# ███     convention explicitly, with the Cu example.                     ███
# ███   · Reasoning-expert rewards candidates whose chain cites the       ███
# ███     inversion step ("inverted from m_conv").                        ███
# ███   · NEW `_audit_srs_conventions()` diagnostic — one-shot corpus     ███
# ███     convention audit; callable from a notebook / test.              ███
# ███   · Regression test `_regression_test_v901`.                       ███
# ============================================================================
...
def update_plastic_strain(sigma_eq, sigma_y, eps_p_xx, eps_p_yy, eps_p_xy,
                          gamma0_dot, m_inv, dt):
    """Viscoplastic flow rule in the INVERSE-SRS convention.

        gamma_dot = gamma0_dot * overstress ** m_inv

    where `m_inv` is 1/m_conv (Cu ≈ 20 at 300 K; the conventional SRS
    m_conv = d ln σ / d ln ε̇ ≈ 0.05).  The steep power law is only
    physical if m_inv ≫ 1 — that is the whole reason the ontology
    valid_range is [1.0, 200.0] rather than (0, 1).
    """
    MAX_OVERSTRESS = 1.0
...
PARAM_META = {
    ...
    # v9.0.1 — SRS label clarified to inverse form (see PLASTICITY_ONTOLOGY).
    ...
    'srs':            dict(title='Inverse strain-rate sensitivity exponent',
                           symbol=r'm_{\mathrm{inv}}',              unit=None),
...
# ============================================================================
# ███ v9.0.1 — NER PIPELINE + CASCADE + DERIVED LAB + PHYSICS ρ₀        ███
# ============================================================================
...
PARAM_CANON: Dict[str, Dict[str, Any]] = {
    ...
    # v9.0.1 — SRS is stored in the INVERSE convention. The plausible
    # range [1, 200] excludes the conventional m_conv ∈ (0,1) precisely
    # because a conventional value has no business being reported as-is;
    # `gatekeep` now auto-inverts it before this range check is applied.
    "srs":            dict(aliases=["inverse strain rate sensitivity",
                                    "inverse strain-rate sensitivity",
                                    "strain rate sensitivity exponent",
                                    "rate sensitivity exponent",
                                    "m exponent", "strain-rate sensitivity",
                                    "srs"],
                           unit=None, plausible=(1.0, 200.0)),
...
# ============================================================================
# v8.8.3 — DERIVED-QUANTITY BRIDGE (legacy)
# ============================================================================
...
def build_extraction_prompt(param_key, material, records, max_chars=1200):
    ...
    if param_key == "srs":
        srs_note = (
            "\nIMPORTANT — SRS CONVENTION:\n"
            "The simulator expects the INVERSE SRS exponent m_inv, i.e. the\n"
            "exponent in  γ̇ = γ̇₀ · ((σ − σ_y)/σ_y)^m_inv  with m_inv ≫ 1\n"
            "(Cu at 300 K: m_inv ≈ 20, from conventional m_conv ≈ 0.05).\n"
            "If the paper reports the conventional exponent m_conv < 1,\n"
            "report it VERBATIM with unit 'dimensionless' and note the\n"
            "convention in property_label; the gatekeeper will invert it.\n")
...
def build_prior_inference_prompt(param_key, material, records, max_chars=600):
    ...
    if param_key == "srs":
        srs_note = (
            "\nSRS CONVENTION (mandatory):\n"
            "Return the INVERSE exponent m_inv = 1/m_conv, i.e. the exponent\n"
            "in γ̇ = γ̇₀ · overstress^m_inv. Reference values:\n"
            "  Cu, Al, Ni (FCC, 300 K):  m_inv ≈ 20–33   (m_conv ≈ 0.03–0.05)\n"
            "  Fe, W (BCC, 300 K):       m_inv ≈ 5–20    (m_conv ≈ 0.05–0.2)\n"
            "  Mg, Ti (HCP):             m_inv ≈ 20–50\n"
            "Do NOT emit m_conv < 1 — the gatekeeper will reject it.\n")
...
class ValueCandidate:
    ...
    reasoning: str = ""
    dominant_factor: str = ""
    # v9.0.1 — records which SRS convention the value was reported in:
    #   "inverse"               → already m_inv (the canonical form)
    #   "conventional_inverted" → m_conv was auto-inverted by gatekeep
    #   "ambiguous"             → value ≥ 1.0, assumed inverse
    #   ""                      → non-SRS parameter (field unused)
    convention: str = ""
...
# ============================================================================
# ███ v9.0.1 — INVERSE-SRS CONVENTION BRIDGE                            ███
# ============================================================================
def _normalize_srs(raw_value: float, hint: str = "") -> Tuple[float, str]:
    """Auto-detect which SRS convention a paper used and return the
    simulator-canonical INVERSE form.

    Heuristic:
      raw_value < 1.0  → almost certainly the conventional form
                         m_conv = d ln σ / d ln ε̇.
                         FCC at 300 K: m_conv ≈ 0.005–0.05.
                         BCC at 300 K: m_conv ≈ 0.05–0.2.
                         HCP at 300 K: m_conv ≈ 0.02–0.05.
                         high-T / UFG can push to 0.3–0.5.
                         Anything ≤ 0.6 is unambiguously m_conv → invert.
      raw_value ≥ 1.0  → already the inverse form (Cu ≈ 20, W ≈ 10, …).

    Returns (value_in_inverse_form, convention_tag) where the tag is one of
    {"inverse", "conventional_inverted", "rejected", "ambiguous"}.
    """
    try:
        v = float(raw_value)
    except (TypeError, ValueError):
    ...
    if v <= 0 or not np.isfinite(v):
        return float("nan"), "rejected"
    if v < 1.0:
        # Treat as conventional m_conv; invert.
        if v > 0.6:
            # 0.6 < v < 1.0 — rare; warn but still invert.
            logger.info(
                "_normalize_srs: value %g is in the ambiguous band "
                "(0.6, 1.0); assuming conventional m_conv and inverting",
                v)
        return 1.0 / v, "conventional_inverted"
    return v, "inverse"
...
def gatekeep(items, param_key, provenance="llm", conf_cap=None):
    ...
    v9.0.1: auto-inverts conventional-form SRS values (m_conv < 1)
            before the plausibility check, so a corpus paper reporting
            textbook Cu m_conv = 0.05 is no longer silently dropped.
    """
    canon = PARAM_CANON.get(param_key, {})
    (lo, hi) = canon.get("plausible", (float("-inf"), float("inf")))
    ...
    for it in items:
        ...
        # v9.0.1 — SRS convention bridge.
        conv_tag = ""
        if param_key == "srs":
            v_norm, conv_tag = _normalize_srs(v)
            if conv_tag == "rejected" or not np.isfinite(v_norm):
                logger.info("gatekeep[srs]: rejected non-finite %s", v)
                continue
            if conv_tag == "conventional_inverted":
                logger.info(
                    "gatekeep[srs]: auto-inverted conventional "
                    "m_conv=%g → m_inv=%.4g", v, v_norm)
                # Append the inversion to the reasoning chain if any.
                if not it.get("reasoning"):
                    it["reasoning"] = (
                        f"Step 0 (auto): detected conventional-form SRS "
                        f"m_conv = {v:g}; inverted to m_inv = {v_norm:.4g} "
                        f"to match the simulator's "
                        f"γ̇ = γ̇₀ · overstress^m_inv convention.")
            v = v_norm
        else:
            conv_tag = ""
...
# ============================================================================
# ███ v9.0.1 — SRS CONVENTION AUDIT                                     ███
# ============================================================================
def _audit_srs_conventions(corpus_dir: str = "json_metadatabase",
                           material: str = "Cu",
                           temp_k: float = 300.0,
                           strain_rate: float = 1e-3,
                           verbose: bool = True) -> List[Dict[str, Any]]:
    """One-shot diagnostic: scan the corpus and report every SRS value
    found, its raw form, and its normalized inverse form.

    Useful when you suspect the corpus mixes m_conv and m_inv conventions
    and want to know how many of each are present without relying on the
    silent-rejection behaviour of gatekeep.

    Returns a list of {source, raw, normalized, convention, evidence} dicts.
    """
    rows: List[Dict[str, Any]] = []
    try:
        corpus = PlasticityCorpus(corpus_dir).load()
    except Exception as e:
        logger.warning("_audit_srs_conventions: corpus load failed: %s", e)
        return rows

    extractor = PlasticityParameterExtractor(client=None)
    for doc in corpus:
        try:
            findings = extractor._heuristic(
                doc["text"], material=material, temp_k=temp_k,
        ...
        for f in findings:
            if f.get("param") != "srs":
                continue
            raw = float(f["value"])
            norm, conv = _normalize_srs(raw)
            row = dict(
                source=doc.get("source", ""),
                raw=raw,
                normalized=norm,
                convention=conv,
                evidence=str(f.get("evidence", ""))[:90],
            )
            rows.append(row)
            if verbose:
                print(f"{row['source'][:40]:40s}  "
                      f"raw={raw:<10.4g}  →  m_inv={norm:<10.4g}  "
                      f"[{conv:22s}]  {row['evidence']}")

    if verbose and rows:
        n_inv = sum(1 for r in rows if r["convention"] == "inverse")
        n_cnv = sum(1 for r in rows
                    if r["convention"] == "conventional_inverted")
        print(f"\nSummary: {n_inv} inverse, {n_cnv} conventional "
              f"(auto-inverted), {len(rows)} total.")
    return rows
...
class MaterialProperties:
    ...
    def get_cu_properties():
        return {
            ...
            # NOTE: 'm' below is the INVERSE SRS exponent m_inv ≈ 20 for Cu
            # at 300 K.  See `update_plastic_strain` and module banner.
...
class NanotwinnedCuSolver:
    ...
    def compute_plastic_strain(self, sigma_eq, sigma_y):
        try:
            ...
            # `m` in the plasticity dict is the INVERSE SRS exponent m_inv.
            m_inv = int(plastic_params['m'])
            eps_p_xx_new, eps_p_yy_new, eps_p_xy_new = update_plastic_strain(
                sigma_eq, sigma_y,
                self.eps_p_xx, self.eps_p_yy, self.eps_p_xy,
                gamma0_dot, m_inv, self.dt)
...
PLASTICITY_ONTOLOGY: Dict[str, Dict[str, Any]] = {
    ...
    # ── v9.0.1 — SRS label clarified as INVERSE exponent ─────────────────
    # The solver's flow rule is γ̇ = γ̇₀ · overstress^m with m ≫ 1, so the
    # stored quantity is m_inv ≡ 1/m_conv (Cu ≈ 20 ⇔ m_conv ≈ 0.05).
    # `gatekeep` auto-inverts conventional-form values (raw < 1.0) before
    # the range check, and tags the resulting candidate with
    # convention="conventional_inverted" so the UI can display a 🔄 glyph.
    "srs": {
        "label": "Inverse Strain-Rate Sensitivity Exponent (1/m_conv)",
        "symbol": "m_inv",
        "aliases": PARAM_CANON["srs"]["aliases"] + [
            "inverse strain rate sensitivity",
            "inverse strain-rate sensitivity",
            "strain rate sensitivity", "srs", "m exponent",
            "stress exponent", "rate sensitivity",
            "strain-rate sensitivity", "rate sensitivity exponent",
            "n exponent", "viscous exponent",
            "m_inv", "m_inverse",
        ],
        ...
        "convention": "inverse",
        "convention_note": (
            "Stored as the *inverse* SRS exponent.  The simulator's flow "
            "rule is  γ̇ = γ̇₀ · ((σ − σ_y)/σ_y)^m_inv  with m_inv ≫ 1.  "
            "Conventional m_conv = d ln σ / d ln ε̇ = 1/m_inv "
            "(Cu at 300 K: m_conv ≈ 0.05 ⇒ m_inv ≈ 20).  Papers reporting "
            "m_conv < 1 are auto-inverted by the gatekeeper."
        ),
    },
    "sigma0_fric": {
...
_PARAM_ALIASES: Dict[str, str] = {
    ...
    "m_inv": "srs", "m_inverse": "srs", "inv_srs": "srs",
    "inverse_srs": "srs", "inverse_strain_rate_sensitivity": "srs",
...
ASCII strings — do NOT use symbols or synonyms):
  - "rho0"        (initial dislocation density, m^-2)
  - "mu"          (shear modulus, Pa or GPa)
  - "gamma0_dot"  (reference strain rate, s^-1)
  - "srs"         (INVERSE strain-rate sensitivity exponent m_inv = 1/m_conv,
                   dimensionless; Cu ≈ 20, W ≈ 10, Mg ≈ 30)
...
IMPORTANT — SRS convention: the simulator uses the INVERSE form
    γ̇ = γ̇₀ · overstress^m_inv   with m_inv ≫ 1.
If a paper reports the conventional exponent m_conv < 1 (typical FCC
value 0.005–0.05), still report it VERBATIM and let the downstream
gatekeeper invert it. Do NOT pre-invert in the output.
...
these ASCII strings (character-for-character):
    ...
    "mu"            — shear modulus                          [Pa]
    "gamma0_dot"    — reference strain rate                  [s^-1]
    "srs"           — INVERSE SRS exponent m_inv (1/m_conv)  [dimensionless]
...
REASONING CHAINS (use these formulas, do NOT just look up a constant):
  ...
  srs (m_inv, material, T):
    🚨 CRITICAL — RETURN THE INVERSE EXPONENT 🚨
    The simulator flow rule is  γ̇ = γ̇₀ · overstress^m_inv  with
    m_inv ≫ 1, so m_inv = 1/m_conv where m_conv = d ln σ / d ln ε̇.
    Reference m_inv values (all ≫ 1):
      FCC 300 K (Cu, Al, Ni):   m_inv ≈ 20-33
      BCC 300 K (Fe, W, Mo):    m_inv ≈ 5-20
      HCP 300 K (Mg, Ti):       m_inv ≈ 20-50
      High-T (>0.5 T_m):        m_inv ≈ 5-10
    Do NOT emit m_conv < 1.
...
class PlasticityLatentMoEScorer:
    ...
    def _reasoning_expert(reasoning: str, method: str) -> float:
        ...
        cites_formula = any(k in reasoning for k in
                            ["mu(T)", "sigma0", "rho_0", "gamma0_dot",
                             ...
                             # v9.0.1 — reward candidates that cite the
                             # SRS inversion step explicitly.
                             "inverted", "m_conv", "m_inv",
                             "conventional", "overstress^"])
    ...
    def score(self, extractions, target_material, target_temp,
        ...
        regime_expert = PhysicsRegimeExpert()
        regime_active = any([
        ...
        for ext in extractions:
            ...
            s_reg  = (regime_expert.score(v_si_clamped, synthesis,
                                          architecture, twin_spacing)
                      if p == "rho0" and regime_active else 0.0)
            ...
            buckets[p].append(PlasticityCandidate(
                ...
                clamped=was_clamped,
                convention=str(ext.get("_convention", "")),
            ))
        ...
        for doc in corpus[:max_docs]:
            ...
            for ext in extractions:
                ...
                try:
                    ...
                    continue
                # v9.0.1 — normalize SRS into the inverse convention before
                # it enters the prior-learning histogram.
                if p == "srs":
                    v_si, _ = _normalize_srs(v_si)
                    if not np.isfinite(v_si):
                        continue
        ...
        for i, param in enumerate(PARAM_ORDER):
            ...
            for c in cands:
                ...
                extractions.append({
                    ...
                    "_convention": getattr(c, "convention", ""),
                })
...
def _plr_render_parameter_selector(param: str,
    ...
    # v9.0.1 — SRS convention explainer caption under the card title.
    if param == "srs":
        st.caption(
            "ℹ️ **SRS is stored as the *inverse* exponent** — the power-law "
            "exponent in γ̇ = γ̇₀ · ((σ − σ_y)/σ_y)^m_inv.  Conventional "
            "m_conv = d ln σ / d ln ε̇ = 1/m_inv "
            "(Cu at 300 K: m_conv ≈ 0.05 ⇒ m_inv ≈ 20).  "
            "Papers reporting m_conv < 1 are **auto-inverted** 🔄 by the "
            "gatekeeper before the plausibility check.")
    ...
    if best is not None:
        ...
        conv_tag = ""
        if getattr(best, "convention", "") == "conventional_inverted":
            conv_tag = " 🔄 inv-from-m_conv"
        elif getattr(best, "convention", "") == "inverse":
            conv_tag = " ✓ m_inv"
        options.append((
            f"⭐ Recommended: {_pl_fmt(param, best.value_si)} "
            f"(score {best.score:.2f} · {prov_tag}{conv_tag})",
            best.value_si, best))
    for i, c in enumerate(bundle.candidates.get(param, [])[1:], start=1):
        c_conv = ""
        if getattr(c, "convention", "") == "conventional_inverted":
            c_conv = " 🔄"
        options.append((
            f"   Alt {i}: {_pl_fmt(param, c.value_si)} "
            f"(score {c.score:.2f}){c_conv}",
            c.value_si, c))
    ...
    if chosen is not None:
        conv_disp = ""
        if getattr(chosen, "convention", ""):
            conv_disp = f" | convention={chosen.convention}"
        st.caption(
...
def render_plasticity_recommender_sidebar(
    ...
    st.subheader("🤖 Intelligent Plasticity Recommender v9.0.1")
    st.caption(
        "**v9.0.1 Inverse-SRS convention cleanup:** the dashboard label "
        "for the strain-rate-sensitivity exponent now reads "
        "*'Inverse Strain-Rate Sensitivity Exponent (1/m_conv)'* with "
        "symbol `m_inv`.  Corpus papers reporting the conventional form "
        "(m_conv < 1, textbook Cu ≈ 0.05) are now **auto-inverted** 🔄 by "
        "the gatekeeper instead of being silently dropped, and the "
        "resulting candidate is tagged `convention=conventional_inverted` "
        "so the UI shows the inversion glyph."
    )
    st.caption(
        ...
        "**v8.9.0 Friction & Lattice Stress Lab** derives σ₀ via a "
        "weighted-median consensus of HP(d), HP(λ), solver-law inversion, "
        "and P–N+Taylor floor."
    )
    ...
    if run_btn:
        ...
        try:
            ...
            srs_cands = bundle.candidates.get("srs", [])
            if srs_cands:
                n_inv = sum(1 for c in srs_cands
                            if getattr(c, "convention", "") == "inverse")
                n_cnv = sum(1 for c in srs_cands
                            if getattr(c, "convention", "")
                            == "conventional_inverted")
                st.caption(
                    f"🔄 **SRS convention:** {n_inv} inverse, {n_cnv} "
                    f"auto-inverted (m_conv → m_inv).  SRS stored as "
                    f"m_inv = 1/m_conv.")
        except Exception as e:
            progress.empty()
    ...
    st.metric("5-target coverage",
              ...
              help="ρ₀, μ, γ̇₀, m_inv, σ₀ — the five solver-consumed parameters")
        ...
        for k, v in overrides.items():
            ...
            if k == "_tau_p_passthrough":
                try:
                    ...
                    pass
                continue
            # v9.0.1 — 'm' override is already in the INVERSE convention
            # because the sidebar stores the post-gatekeep m_inv value.
            cleaned[k] = v
...
def render_recommender_bars_pub(bundle, style):
    ...
    # v9.0.1 — SRS convention reminder above the bar chart for srs.
    if selected_param == "srs":
        st.caption(
            "ℹ️ **Inverse SRS convention** — stored as m_inv = 1/m_conv.  "
            "Bars annotated with 🔄 are values that were auto-inverted "
            "from a conventional m_conv < 1 reported in the corpus.")
...
def render_recommender_treemap_pub(bundle, style):
    ...
    for p_idx, p in enumerate(PARAM_ORDER):
        ...
        for j, c in enumerate(bundle.candidates.get(p, [])):
            ...
            ids.append(c_id)
            inv_glyph = " 🔄" if getattr(c, "convention", "") == \
                        "conventional_inverted" else ""
            labels.append(f"{_pl_fmt(p, c.value_si)}<br><i>{c.method}{inv_glyph}</i>"
...
def render_recommender_visuals_dashboard():
    ...
    st.caption(
        ...
        "candidates. v9.0.1 clarifies the SRS axis as the **inverse** "
        "exponent m_inv; auto-inverted candidates carry a 🔄 glyph.")
...
def main():
    ...
    <strong>✅ PURE FFT SPECTRAL + AI PLASTICITY RECOMMENDER v9.0.1:</strong><br>
    ...
    • <span style="color: green;">🔄 v9.0.1 INVERSE-SRS CLEANUP:</span> the SRS card is now labelled <em>Inverse Strain-Rate Sensitivity Exponent (1/m_conv)</em>; conventional-form m_conv &lt; 1 values are auto-inverted by the gatekeeper and tagged with a 🔄 glyph.<br>
    • <span style="color: green;">🎯 5/5 target coverage:</span> ρ₀, μ, γ̇₀, m_inv, σ₀ close automatically.<br>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        if st.button("🔄 Reset to Defaults", type="secondary"):
            ...
            st.rerun()
        st.markdown("---")
        with st.expander("🧠 AI Plasticity Recommender v9.0.1", expanded=False):
...
def _regression_test_v884() -> None:
    ...
    assert "convention" in names, \
        "ValueCandidate missing v9.0.1 convention field"
    assert names.index("reasoning") < names.index("dominant_factor")
    assert names.index("dominant_factor") < names.index("convention")
    ...
    assert getattr(stale, "convention", "") == ""
...
def _regression_test_v890() -> None:
    ...
    # σ_y = 300 MPa, k_y = 0.11 MPa·m^½, λ = 20 nm
    #   obstacle term k/√λ = 0.11/√(20e-9) ≈ 778 MPa > σ_y
    # → HP-λ route is degenerate (negative σ₀) with these inputs.
    assert hp_lam.status == 'degenerate', \
        (f"HP-λ route expected degenerate for σ_y=300 MPa, λ=20 nm "
         f"(obstacle ≈ 778 MPa > σ_y); got status={hp_lam.status}")
    ...
    logger.info(
        ...
        "σ₀_consensus = %.1f MPa  HP-λ status = %s",
        w_default * 1e9, pn.tau_P_MPa, cons['value_MPa'], hp_lam.status)
...
def _regression_test_v901() -> None:
    """v9.0.1 — inverse-SRS convention cleanup regression."""
    # 1. Label/symbol rewritten in the ontology
    srs_spec = PLASTICITY_ONTOLOGY["srs"]
    assert "inverse" in srs_spec["label"].lower(), \
        f"SRS label not clarified: {srs_spec['label']!r}"
    assert "m_inv" in srs_spec["symbol"] or "inv" in srs_spec["symbol"], \
        f"SRS symbol not clarified: {srs_spec['symbol']!r}"
    assert "convention_note" in srs_spec
    assert "inverse" in srs_spec["convention"]

    # 2. _normalize_srs inverts conventional values, passes inverse ones
    v_conv, tag_conv = _normalize_srs(0.05)   # textbook Cu m_conv
    assert tag_conv == "conventional_inverted"
    assert abs(v_conv - 20.0) < 1e-9

    v_inv, tag_inv = _normalize_srs(20.0)     # already m_inv
    assert tag_inv == "inverse"
    assert abs(v_inv - 20.0) < 1e-9

    v_bad, tag_bad = _normalize_srs(0.0)
    assert tag_bad == "rejected"
    assert math.isnan(v_bad)

    # 3. gatekeep inverts conventional-form SRS before the range check
    conv_item = [{"value": 0.05, "unit": "", "confidence": 0.9,
                  "property_label": "m_conv", "source": "test",
                  "reasoning": ""}]
    out = gatekeep(conv_item, "srs", provenance="llm")
    assert out, "gatekeep rejected the textbook Cu m_conv = 0.05"
    assert abs(out[0].value - 20.0) < 1e-9, \
        f"gatekeep failed to invert: got {out[0].value}"
    assert out[0].convention == "conventional_inverted"
    assert "inverted" in out[0].reasoning.lower()

    # 4. gatekeep passes inverse-form SRS untouched
    inv_item = [{"value": 20.0, "unit": "", "confidence": 0.9,
                 "property_label": "m_inv", "source": "test",
                 "reasoning": ""}]
    out2 = gatekeep(inv_item, "srs", provenance="llm")
    assert out2 and abs(out2[0].value - 20.0) < 1e-9
    assert out2[0].convention == "inverse"

    # 5. Sanity: negative m_inv is rejected
    bad_item = [{"value": -20.0, "unit": "", "confidence": 0.9,
                 "property_label": "bad", "source": "test",
                 "reasoning": ""}]
    out3 = gatekeep(bad_item, "srs", provenance="llm")
    assert out3 == [], f"gatekeep accepted negative SRS: {out3}"

    # 6. BCC-convention double: m_conv = 0.2 → m_inv = 5.0 (Fe at 300 K)
    v_fe, tag_fe = _normalize_srs(0.2)
    assert tag_fe == "conventional_inverted"
    assert abs(v_fe - 5.0) < 1e-9

    logger.info(
        "v9.0.1 regression test: PASS  "
        "label=%r symbol=%r  "
        "0.05→%.4g (%s)  20.0→%.4g (%s)  0.2→%.4g (%s)",
        srs_spec["label"], srs_spec["symbol"],
        v_conv, tag_conv, v_inv, tag_inv, v_fe, tag_fe)
