...
# ███   · Tier 1  grounded LLM  (verbatim-only contract — UNCHANGED)    ███
# ███   · Tier 2  deterministic regex (sci-notation + param-aware units) ███
# ███   · Tier 3  LLM PRIOR INFERENCE (separate prompt + separate        ███
# ███     contract; confidence capped at 0.5; bounded by PARAM_CANON)    ███
# ███   · PROVENANCE_MARKERS extended to three glyphs ('D' / '^' / 'o')  ███
# ███   · Sci-notation scanner: 6.0e13, 1.0×10^13, 1.0 x 10 ** 13         ███
# ███   · Param-aware unit tokens: m^-2, s^-1, nm — rho0 rows like        ███
# ███     '1.0×10¹³ m⁻²' are now reachable by the heuristic tier.         ███
# ███   · Retrieval relaxation for sparse parameters (sigma0 case)        ███
# ███   · Optional prior-inference toggle in the sidebar                   ███
# ███   · 5/5 coverage metric + per-parameter diagnostics dataframe        ███
# ███   · Preserves the v8.7 world-knowledge path WITHOUT loosening the   ███
# ███     Tier-1 verbatim-only contract.                                  ███
# ███ FIX (v8.8.3): PYRAMIDAL SANITY — prior-inferred sigma0/mu rows that  ███
# ███   inadvertently leak through Tier-2 prose regex are now tagged      ███
# ███   'heuristic' rather than 'llm', preserving provenance integrity.   ███
# ███ NEW (v8.8.3): FRICTION STRESS (σ₀) + PEIERLS–NABARRO (τ_P)          ███
# ███   FIRST-CLASS NER TARGETS                                          ███
# ███   · GAZETTEER extended with grain_size / twin_thickness /           ███
# ███     hall_petch_k / burgers_vector / poisson_ratio / taylor_factor /  ███
# ███     dislocation_core_width / resolved_shear_stress entity types      ███
# ███   · PARAM_CANON split: sigma0_fric / sigma0_yield / tau_p + seven   ███
# ███     side-note context fields (grain_size_d, twin_thickness_lambda,   ███
# ███     burgers_vector_b, poisson_ratio_nu, taylor_factor_M,             ███
# ███     hall_petch_k, core_width_w, shear_modulus_G)                     ███
# ███   · UNIT_TOKENS extended with μm / Å / MPa·m^(1/2)                    ███
# ███   · SIDE-NOTE TABLE PARSER — recognises markdown pipe tables         ███
# ███   · DERIVED-QUANTITY BRIDGE — Hall–Petch σ₀ = σ_y − k_y·d^(−1/2);    ███
# ███     Taylor τ_P ≈ σ₀/M; Peierls–Nabarro τ_P = 2G/(1−ν)·exp(−2πw/b)     ███
...
# ============================================================================
# v8.8.3: PROVENANCE_MARKERS now carries FOUR glyphs — 'D' for verbatim
# grounded LLM extraction, '^' for deterministic-regex fallback, 'o' for
# the Tier-3 prior-inference path, and 'v' (triangle-down) for the new
# derived-quantity bridge (Hall–Petch σ₀, Taylor τ_P, P–N τ_P).
# ============================================================================
PROVENANCE_MARKERS = {
    ...
    # v8.8.3 — fourth glyph: derived from a side-note cluster via
    # Hall–Petch / Taylor / Peierls–Nabarro formulas.  Uses 'v'
    # (triangle-down) to avoid colliding with '^' (heuristic).
    'derived':   dict(marker='v', ms=7.0, label='Derived (side-note cluster)'),
}


def _norm_provenance(p):
    """Collapse any provenance string into one of the four canonical keys.

    v8.8.3: 'derived' joins the family.  Ordering matters — the 'prior'
    test must precede the 'llm' test; the 'deriv' test must precede the
    'heuristic' fall-through (otherwise 'derived' would be classified as
...
GAZETTEER: Dict[str, List[str]] = {
    ...
    # ── side-note context fields needed to DERIVE σ₀ / τ_P ─────────
...
PARAM_CANON: Dict[str, Dict[str, Any]] = {
    ...
    # ─── side-note context fields (v8.8.3) ─────────────────────────
...
def score_record(text, param_key, material="Cu", value_hints=None):
    ...
    # v8.8.3: extended entity weighting includes side-note context fields
...
# ============================================================================
# v8.8.2 — PARAM-AWARE SCI-NOTATION SCANNER
# ============================================================================
UNIT_TOKENS = {
    "GPa":  r"gpa",
    "MPa":  r"mpa",
...
def unit_regex_for(param_key: str) -> Optional[str]:
    """Return a regex fragment matching any unit acceptable for `param_key`."""
    ...
    # v8.8.3 — length family: any of nm / μm / Å
...
# ---- number scanner: 47.19 | 6.0e13 | 1.0×10^13 | 5.0 x 10**15 ------------
_NUM_ANY = re.compile(
    r"(?<![a-z0-9.])"
    r"(?P<val>\d+(?:\.\d+)?(?:[eE][+-]?\d+)?|\.\d+)"
...
def find_value_after(text: str, pos: int, param_key: str, window: int = 90):
    """Param-aware proximity search (v8.8.2: was stress-units-only)."""
...
def _value_from_cell(key: str, val: str, param_key: str) -> Tuple[Optional[float], Optional[str]]:
    """v8.8.2 replacement for the old _NUM_UNIT_RE / _BARE_NUM_RE pair."""
    kt = norm_text(key)
    vt = norm_text(val)
    ure = unit_regex_for(param_key)
    best = None
...
def heuristic_extract(records, param_key, target_unit=None):
    """Three passes:
       (a) JSON key match
       (b) prose proximity (param-aware units + sci-notation)
       (c) side-note markdown-table row
    """
    canon = PARAM_CANON.get(param_key, {})
    ...
    for rec in records:
        text = rec["text_norm"]
        raw  = rec.get("text", text)          # v8.8.3: raw text for tables

        # ── (a) key-position match ─────────────────────────────────
...
def parse_side_note_table(text: str) -> List[Dict[str, Any]]:
    ...
    Runs against the RAW record text (not text_norm) because the markdown
    pipe characters must survive intact.
    """
    out: List[Dict[str, Any]] = []
...
# ============================================================================
# v8.8.3 — DERIVED-QUANTITY BRIDGE FOR σ₀ / τ_P
# ============================================================================
def _pick_best(candidates: List["ValueCandidate"]) -> Optional["ValueCandidate"]:
...
def derive_sigma0_and_tau_p(
    ...
    def best(param_key: str) -> Optional["ValueCandidate"]:
        return _pick_best(cands_by_param.get(param_key, []))
    ...
    if w_v  is None: w_v  = float(_default("core_width_w")) * 1e-9
    if G_v  is None: G_v  = float(_default("mu")) * 1e9

    # ─────────────────────────────────────────────────────────────
    # PATH A — Hall–Petch intercept: σ₀ = σ_y − k_y · d^(−1/2)
    # ─────────────────────────────────────────────────────────────
    ...
    # ─────────────────────────────────────────────────────────────
    # PATH B — Taylor cross-conversion: τ_P ≈ σ₀ / M
    # ─────────────────────────────────────────────────────────────
    ...
    # ─────────────────────────────────────────────────────────────
    # PATH C — Classical P–N formula: τ_P = 2G/(1−ν) · exp(−2π w/b)
    # ─────────────────────────────────────────────────────────────
...
# ---- 9. GROUNDED PROMPT (Tier 1 — verbatim-only contract) ------------------
...
# ============================================================================
# v8.8.3 — TIER 3 PRIOR-INFERENCE PROMPT (SEPARATE CONTRACT, + SIDE-NOTE)
# ============================================================================
...
def parse_llm_json(raw: str) -> list:
    """Tolerant parser: strips CoT blocks, code fences, prose wrappers."""
...
# ============================================================================
# v8.8.3 — THREE-TIER CASCADE ORCHESTRATOR
# ============================================================================
def recommend_param_values(param_key, material="Cu", value_hints=None,
    ...
    """Full three-tier retrieve-then-extract pipeline for a single parameter.

    Returns
    -------
    (candidates, records, diagnostics)
    """
    ...
    # ── retrieval + one relaxation step for sparse parameters ───────────────
    ...
    # ── Tier 2: deterministic regex (sci-notation + unit-aware) ─────────────
    ...
    # ── Tier 3: LLM PRIOR INFERENCE — separate prompt, separate contract ────
...
def get_retriever(folder: str = "json_metadatabase",
    ...
    """Cached singleton. Purged by st.cache_resource.clear() on
    'Force reload corpus'."""
...
PLASTICITY_ONTOLOGY: Dict[str, Dict[str, Any]] = {
    ...
    # ── v8.8.3 — friction stress, yield stress, Peierls–Nabarro ─────
    "sigma0_fric": {
    ...
    # ── v8.8.3 — side-note context fields ──────────────────────────
    "grain_size_d": {
        "label": "Grain Size",
        "symbol": "d",
...
# v8.8.3: PARAM_ORDER now includes the split σ₀ / τ_P targets and the
# side-note context fields so the cascade sweeps all of them.
PARAM_ORDER: List[str] = [
    # ── classic plasticity parameters (solver consumption order) ──
    "rho0", "mu", "gamma0_dot", "srs",
    # ── friction stress family ────────────────────────────────────
    "sigma0_fric", "sigma0_yield", "tau_p",
    # ── side-note context fields (feed the derived-quantity bridge) ─
...
SOLVER_KEY_MAP: Dict[str, str] = {
    ...
    "srs":                     "m",
    # ── solver-visible plasticity slots ───────────────────────────
    "sigma0_fric":             "sigma0",          # direct injection
    # The following are NOT consumed verbatim by the solver; the
    # `apply_plasticity_overrides` hook converts them appropriately.
    "sigma0_yield":            "_yield_passthrough",
    "tau_p":                   "_tau_p_passthrough",
    # Context fields are ignored by the solver.
...
_PARAM_ALIASES: Dict[str, str] = {
    ...
    "sigma0_fric": "sigma0_fric",
    "sigma0_f": "sigma0_fric",
    "friction_lattice_stress": "sigma0_fric",
    "athermal_stress": "sigma0_fric",
    "sigma_0": "sigma0_fric", "σ₀": "sigma0_fric", "σ0": "sigma0_fric",
    "sigma0": "sigma0_fric",

    "yield_stress": "sigma0_yield",
    "sigma_y": "sigma0_yield",
    "yield_strength": "sigma0_yield",
    "sigma0_yield": "sigma0_yield",
    "sy": "sigma0_yield",
    "σy": "sigma0_yield",
    "0.2%_yield_stress": "sigma0_yield",
    "offset_yield_stress": "sigma0_yield",

    "peierls_stress": "tau_p",
    "peierls_nabarro_stress": "tau_p",
    "peierls-nabarro_stress": "tau_p",
    "pn_stress": "tau_p",
    "tau_p": "tau_p",
    "tau_p_": "tau_p",
    "τ_p": "tau_p",
    "intrinsic_lattice_friction": "tau_p",
    "lattice_resistance": "tau_p",
    "peierls": "tau_p",

    # ── v8.8.3: side-note context fields ───────────────────────────
    "grain_size": "grain_size_d",
    "grain_diameter": "grain_size_d",
    "d_grain": "grain_size_d",
    "d50": "grain_size_d",
    "crystallite_size": "grain_size_d",
    "mean_grain_size": "grain_size_d",
    ...
    "burgers_vector": "burgers_vector_b",
    "burgers_b": "burgers_vector_b",
    "burgers_vector_b": "burgers_vector_b",
    "b_vector": "burgers_vector_b",
    "b": "burgers_vector_b",
    ...
    "taylor_M": "taylor_factor_M",
    "taylor_factor": "taylor_factor_M",
...
def _canonicalize_param(raw_p: str) -> Optional[str]:
    ...
    # Longest-prefix match against canonical names first, to avoid
    # "sigma0" swallowing "sigma0_fric".
...
def _normalize_unit(value: float, unit: str, param: str) -> float:
    """Convert a raw (value, unit) pair into the canonical SI unit of `param`.

    SI targets:
      mu / sigma0_fric / sigma0_yield / tau_p / shear_modulus_G  → Pa
      rho0                                                        → m^-2
      gamma0_dot                                                  → s^-1
      grain_size_d / twin_thickness_lambda / burgers_vector_b
      / core_width_w                                              → m
      hall_petch_k                                                → MPa·m^(1/2)
      poisson_ratio_nu / taylor_factor_M / srs                    → unchanged
    """
    ...
    if param in ("mu", "sigma0_fric", "sigma0_yield", "tau_p",
        ...
        if "pa"  in u: return v
        return v  # assume Pa if unit is missing
    ...
    # ── length family → canonical SI metres ────────────────────────
    if param in ("grain_size_d", "twin_thickness_lambda",
                 "burgers_vector_b", "core_width_w"):
        if "nm" in u or "nanometer" in u:
            return v * 1e-9
        if "um" in u or "micrometer" in u or "micron" in u:
            return v * 1e-6
        if "angstrom" in u or (u.startswith("a") and len(u) <= 3):
            return v * 1e-10
        if u.startswith("mm"):
            return v * 1e-3
        if u.startswith("m") and "pa" not in u:
            return v
        # no explicit unit — param-specific default
        return v * 1e-9 if param in ("grain_size_d", "twin_thickness_lambda",
                                     "core_width_w") else v * 1e-9

    # ── Hall–Petch coefficient → MPa·m^(1/2) ──────────────────────
    if param == "hall_petch_k":
        if "um" in u or "μm" in u:
            return v * 1e3
        return v

    # ── dimensionless (poisson, taylor, srs) ──────────────────────
    return v
...
# ----------------------------------------------------------------------------
# LLM PROMPTS (v8.7 legacy dual-mode prompts preserved)
# ----------------------------------------------------------------------------
_EXTRACT_SCHEMA = (
...
variant. Use the exact ASCII strings above. The parser is strict.
...
PATH B — REASONED INFERENCE (only if Path A does not apply)
  ...
  method="LLM_inferred" and confidence between 0.4 and 0.6.
...
# ----------------------------------------------------------------------------
# LLM + HEURISTIC EXTRACTION (v8.7 legacy two-tier path — retained)
# ----------------------------------------------------------------------------
        ...
        if "gamma0_dot" not in have:
            llm_out.append({
                ...
                "reasoning": (
                    ...
                    f"({strain_rate} s^-1) as the reference rate.\n"
...
class PlasticityLatentMoEScorer:
    ...
    v8.8.3: prior-inferred candidates surface as method='prior inference'
    (canonicalised below to 'llm_prior' for provenance reporting), and are
    deliberately given a lower _method_expert weight so the MoE cannot
    outrank verbatim-grounded hits.  Derived candidates (Hall–Petch /
    Taylor / P–N) get intermediate weights.
    """
...
class PlasticityRecommendationBundle:
    ...
    def coverage(self) -> int:
        """Number of parameters that produced at least one candidate."""
        return sum(1 for p in PARAM_ORDER if self.candidates.get(p))


class PlasticityRecommender:
    """End-to-end: hybrid retrieve → three-tier cascade → gatekeep →
    LatentMoE rank → derived-quantity bridge → priors.
    ...
    def recommend_grounded(self, material: str, temp_k: float,
        ...
        """Run the three-tier cascade for each parameter, then the derived
        bridge for σ₀/τ_P, then LatentMoE-rank the union."""
        if self.hybrid is None:
            st.warning("Hybrid retriever unavailable — falling back to the "
                       "v8.7 pipeline via recommend().")
        ...
        for i, param in enumerate(PARAM_ORDER):
            if progress_callback:
                progress_callback(i + 1, len(PARAM_ORDER),
                                  f"cascade[{param}]")
            ...
            if self.debug_llm:
                logger.info(
                    "cascade[%s]: tier=%s n_llm=%d n_heuristic=%d "
                    "n_prior=%d records=%d reason=%s",
                    param, diag["tier"], diag["n_llm"], diag["n_heuristic"],
                    diag["n_prior"], diag["records"], diag["reason"],
                )
        ...
        context_keys = ("sigma0_yield", "grain_size_d",
                        ...
                        "burgers_vector_b", "taylor_factor_M",
                        "core_width_w")
        ...
        derived = derive_sigma0_and_tau_p(context_bucket,
        ...
        for c in derived:
            ...
            diagnostics.append(dict(
                param=target, records=0, n_llm=0,
                n_heuristic=0, n_prior=0,
                tier="derived",
                reason=f"derived via {c.method} from side-note cluster",
            ))

        # ── gamma0_dot default fallback (v8.4.0 contract, unchanged) ────
        ...
        if "gamma0_dot" not in present:
            ...
            diagnostics.append(dict(param="gamma0_dot", records=0,
                                    ...
                                    reason="no cascade candidate → "
                                           "default_fallback injection"))
    ...
    # ------------------------------------------------------------------
    # Legacy v8.7 pathway — retained as explicit fallback.
    # ------------------------------------------------------------------
...
def _plr_render_parameter_selector(param: str,
    ...
    if best is not None:
        ...
        options.append((
            ...
            f"(score {best.score:.2f} · {prov_tag})",
            best.value_si, best,
        ))
...
def render_plasticity_recommender_sidebar(
    ...
    st.caption(
        ...
        "classical Peierls–Nabarro formula (τ_P = 2G/(1−ν)·exp(−2πw/b)).  "
        "Look for the ▼ marker in the bar chart."
    )
    st.caption(
        "**v8.8.2 three-tier cascade:** Tier 1 verbatim-grounded LLM · "
        "Tier 2 deterministic regex · Tier 3 LLM PRIOR INFERENCE "
        "(confidence-capped at 0.5, bounded by PARAM_CANON plausible ranges)."
    )
    ...
    allow_prior = st.checkbox(
        ...
        "(restores full 5-parameter coverage)",
        value=True,
        key=f"{_PLR}allow_prior",
        help=("When enabled, parameters whose evidence records lack explicit "
              "values fall through to Tier-3 prior inference, which is prompted "
              "separately with a capped confidence (≤0.5) and bounded by the "
              "PARAM_CANON plausible range. This restores the v8.7 coverage "
              "WITHOUT loosening the Tier-1 verbatim-only contract."),
    )
    ...
    show_boolean_debug = st.checkbox(
        "🔎 Show generated Boolean/NL queries for the current hints",
        value=False,
        key=f"{_PLR}show_bool_debug",
    )

    if show_boolean_debug:
        qs = build_boolean_queries(material or "Cu", "mu",
                                   value_hints=value_hints)
        st.markdown("**Generated Boolean queries (for external search APIs):**")
        st.code("\n".join(qs), language="text")
        st.markdown("**Generated natural-language queries (dense channel):**")
        st.code("\n".join(build_query_texts(material or "Cu", "mu",
                                            value_hints=value_hints)),
                language="text")
    ...
    st.metric("Parameters with candidates",
              f"{bundle.coverage()}/{len(PARAM_ORDER)}")

    if bundle.diagnostics:
        with st.expander("🔎 Per-parameter extraction diagnostics",
                         expanded=False):
            diag_df = pd.DataFrame(bundle.diagnostics)[
                [c for c in ["param", "records", "tier", "n_llm",
                             "n_heuristic", "n_prior", "reason"]
                 if c in pd.DataFrame(bundle.diagnostics).columns]
            ]
            st.dataframe(diag_df, use_container_width=True, hide_index=True)
            st.caption(
                "Tier legend · **llm** = Tier-1 verbatim grounded  ·  "
                "**heuristic** = Tier-2 deterministic regex  ·  "
                "**llm_prior** = Tier-3 prior inference (confidence capped "
                "at 0.5)  ·  **derived** = σ₀ / τ_P from side-note cluster  ·  "
                "**default_fallback** = gamma0_dot user-input guarantee."
            )
    ...
    if st.button("✅ Apply selected values to solver",
        ...
        # Persist taylor factor for τ_P → σ₀ fallback conversion
...
def apply_plasticity_overrides(solver) -> None:
    """Apply user-selected plasticity values to the solver's mat_props.

    v8.8.3: SOLVER_KEY_MAP may deliver pseudo-keys beginning with `_`.
    These are interpreted as follows:
      * `_ignore`               — discarded
      * `_yield_passthrough`    — held; contributes to the σ₀ fallback
      * `_tau_p_passthrough`    — held; τ_P × M becomes the σ₀ fallback
    Explicit `sigma0` (from `sigma0_fric`) takes precedence.
    """
    try:
        ...
        # σ₀ fallback chain: explicit σ₀ > τ_P × M > σ_y × 0.2
...
class RecommenderVisualStyle:
    ...
    def apply_plotly_layout(self, fig, title: str = "", xlabel: str = "",
                            ylabel: str = ""):
        fig.update_layout(
...
def render_recommender_bars_pub(bundle: PlasticityRecommendationBundle,
    ...
    with st.expander("🧭 NER debug: retrieved records + Boolean queries",
        ...
        st.markdown(f"**Parameter:** `{selected_param}` · "
                    f"**Material:** `{mat}`")
        try:
            retr = get_retriever()
            top_recs = retr.search(selected_param, mat, k=5)
            if top_recs:
                for r in top_recs:
                    with st.container():
                        st.caption(f"`{r['id']}` — source `{r['source']}`")
                        st.code(json.dumps(r["data"], indent=2,
                                           ensure_ascii=False,
                                           default=str)[:800],
                                language="json")
            else:
                st.caption("No records retrieved from the hybrid retriever "
                           "(corpus may be empty).")
        except Exception as e:
            st.caption(f"Retriever unavailable: {e}")

        st.markdown("**Generated Boolean queries:**")
        st.code("\n".join(build_boolean_queries(mat, selected_param)),
                language="text")
        st.markdown("**Generated NL queries (dense channel):**")
    ...
    with st.expander("🎨 Colormap & Coloring Options", expanded=True):
        ...
        with col_a:
            ...
            chosen_cmap = st.selectbox(
                "Colormap", all_cmap_names,
                index=default_idx, key="rec_bar_cmap_select",
            )
    ...
    with st.expander("✍️ Typography & Labels", expanded=True):
        ...
        with t_col2:
            ...
            show_offset_hint = st.checkbox(
                ...
                value=False, key="rec_bar_headroom_hint",
            )
            if show_offset_hint:
...
def render_recommender_style_controls() -> RecommenderVisualStyle:
    ...
    with st.expander("🌈 Colormap", expanded=True):
        ...
        with c2:
            style.cmap_reverse = st.checkbox("Reverse",
                                             value=style.cmap_reverse,
                                             key="rec_cmap_rev")
        try:
            cmap = style.get_cmap()
            gradient = np.linspace(0, 1, 256).reshape(1, -1)
    ...
    with col_chart:
        ...
        if chart_bytes is None:
            st.caption(
                ...
                "Install `kaleido` (plotly) and/or use Matplotlib-based "
                "charts for full PNG/SVG/PDF export."
            )
        else:
...
def main():
    ...
    • <span style="color: green;">🎯 TIER 1:</span> grounded LLM under a verbatim-only JSON contract.<br>
    • <span style="color: green;">📐 TIER 2:</span> deterministic regex with sci-notation scanner and param-aware unit tokens.<br>
    • <span style="color: green;">🧠 TIER 3:</span> LLM PRIOR INFERENCE — separate prompt, confidence-capped at 0.5.<br>
        ...
        if operation_mode == "Run New Simulation":
            ...
            if not simulations:
                ...
                if comparison_type == "Overlay Line Profiles":
                    ...
                    position_ratio = st.slider("Position Ratio", 0.0, 1.0,
                                               0.5, 0.1)
                    ...
                    if profile_direction == "Custom":
                        custom_angle = st.slider("Custom angle (deg)",
                                                 -180, 180, 45, 5)
                    else:
                        custom_angle = 45
                if st.button("🔬 Run Comparison", type="primary"):
                    comparison_config = {
                        'sim_ids': [sim_options[name]
                                    for name in selected_sim_ids],
            ...
            col1, col2 = st.columns(2)
            with col1:
                N_sweep = st.slider("Grid size N", 64, 256, 128, 32,
                                    key="sweep_N")
                dx_sweep = st.slider("dx (nm)", 0.2, 1.0, 0.5, 0.1,
                                     key="sweep_dx")
                dt_sweep = st.slider("dt (ns)", 1e-5, 1e-2, 1e-3, 1e-5,
                                     format="%.5f", key="sweep_dt")
                W_sweep = st.slider("W (J/m³)", 0.5, 5.0, 2.0, 0.1,
                                    key="sweep_W")
            with col2:
    ...
    if operation_mode == "Compare Saved Simulations" and \
       'comparison_config' in st.session_state:
            ...
            if config['type'] == "Side-by-Side Heatmaps":
                    ...
                    for idx, sim_idx in enumerate(valid_indices):
                        ...
                        if field in frame:
                            ...
                            im = ax.imshow(data, extent=extent, cmap='viridis',
                                           origin='lower')
                            ax.set_title(sim_names[sim_idx][:30] + "...",
                                         fontsize=8)
                            ax.set_xlabel('x (nm)')
                            ax.set_ylabel('y (nm)')
                ...
                for sim_idx, sim in enumerate(simulations):
                    ...
                    if field in ['sigma_eq', 'sigma_h']:
                        ...
                        ylabel = field
                    fig.add_trace(go.Scatter(x=distance, y=profile,
                                             mode='lines',
                ...
                for sim in simulations:
                    ...
                    if hist:
                        ...
                        row = {
                            ...
                            'σ_app (MPa)':
                                params.get('applied_stress', 0) / 1e6,
                            'θ (deg)': params.get('applied_stress_angle', 0),
                            'W (J/m³)': params.get('W', 0),
                            'Avg σ_eq (GPa)':
                                last.get('avg_stress', 0) / 1e9,
                            'Max σ_eq (GPa)':
                                last.get('max_stress', 0) / 1e9,
                            'Avg h (nm)': last.get('avg_spacing', 0),
                            'Plastic Work (J)':
                                last.get('plastic_work', 0),
                ...
                for sim in simulations:
                    ...
                    if hist and 'convergence' in hist[-1]:
                        ...
                        row = {
                            ...
                            'W': params.get('W', 0),
                            'avg_stress':
                                conv.get('avg_stress', 0) / 1e9,
                            'max_stress':
                                conv.get('max_stress', 0) / 1e9,
        ...
        if sim_data:
            ...
            with col1:
                st.metric("λ (twin spacing)",
                          f"{params.get('twin_spacing', 0):.1f} nm")
            with col2:
                stress_mag = params.get('applied_stress', 0) / 1e6
                angle = params.get('applied_stress_angle', 0)
                st.metric("σ_app / θ",
                          f"{stress_mag:.0f} MPa / {angle:.0f}°")
            with col3:
                st.metric("W (well depth)",
                          f"{params.get('W', 0):.2f} J/m³")
            with col4:
            ...
            if history:
                ...
                fig = visualizer.create_multi_field_comparison(
                    results, style_params)
                if fig:
                    st.pyplot(fig)
                    plt.close(fig)
                if st.button("🗑️ Delete This Simulation",
                             key=f"delete_{sim_id}"):
            ...
            if st.button("▶️ Start Evolution", type="secondary",
                ...
                with st.spinner("Running phase-field simulation (FFT)..."):
                    try:
                        ...
                        st.success(
                            f"✅ Simulation completed! Generated "
            ...
            if 'results_history' in st.session_state:
                ...
                if 'solver' in st.session_state and \
                   st.session_state.solver.history['phi_norm']:
                    full_timesteps = (
                        np.arange(len(st.session_state.solver.history['phi_norm']))
                        * params['dt'])
        ...
        with tabs[3]:
            if 'results_history' in st.session_state:
                ...
                for idx, ptype in enumerate(internal_types):
                    ...
                    ax.set_title(
                        f'{ptype.replace("_", " ").title()} Profile')
        ...
        with tabs[4]:
            if 'results_history' in st.session_state:
                ...
                frame_idx_plotly = st.slider("Frame", 0,
                                             len(results_history) - 1,
                                             len(results_history) - 1,
                                             key="plotly_2d_frame")
                ...
                with col1:
                    profile_type_plotly = st.selectbox(
                        "Profile direction",
                        ["Horizontal", "Vertical", "Diagonal",
                         "Anti-Diagonal"], key="plotly_2d_profile")
                with col2:
                    position_ratio_plotly = st.slider("Position ratio", 0.0,
                                                      1.0, 0.5, 0.05,
                                                      key="plotly_2d_pos")
                profile_type_mapping = {
                    "Horizontal": "horizontal", "Vertical": "vertical",
                ...
                fig_line = visualizer.create_plotly_line_profiles(
                    results, plotly_field, [internal_pt],
                    position_ratio_plotly)
        ...
        with tabs[5]:
            ...
            if 'results_history' in st.session_state:
                ...
                frame_idx_3d = st.slider("Frame", 0,
                                         len(results_history) - 1,
                                         len(results_history) - 1,
                                         key="3d_frame")
        ...
        with tabs[6]:
            ...
            if 'results_history' in st.session_state and \
               st.session_state.results_history:
                ...
                sim_name = build_sim_name(params, sim_id)
                sim_data = {
                    'metadata': MetadataManager.create_metadata(params,
                                                                results_history),
                    'params': params}
...
def _regression_test_v882() -> None:
    """Regression test for the v8.8.2 three-tier cascade."""
    ...
    assert any(abs(_num_value(m) - 6.0e13) < 1e6 for m in hits), \
        "sci-notation '6.0e13' not matched"
    ure = unit_regex_for("rho0")
    assert ure is not None and re.search(ure, "m^-2"), "m^-2 unit token failed"
    assert re.search(ure, "m-2"), "m-2 unit token failed"
    ...
    assert prior_cands and prior_cands[0].confidence == 0.5, \
        "conf_cap not applied to prior-inferred candidate"
    llm_cands = gatekeep(fake, "mu", provenance="llm")
    assert llm_cands and llm_cands[0].confidence == 0.9, \
        "unexpected confidence drift on verbatim candidate"

    assert _norm_provenance("llm_prior") == "llm_prior"
    assert _norm_provenance("prior inference") == "llm_prior"
    assert _norm_provenance("LLM_inferred") == "llm"
    assert _norm_provenance("heuristic") == "heuristic"

    logger.info("v8.8.2 regression test: PASS  "
                "sci-notation ✓  param-units ✓  conf_cap ✓  marker ✓")


def _regression_test_v883() -> None:
    """v8.8.3 — derived σ₀ / τ_P bridge regression."""
    fake_cands = {
        "sigma0_yield": [ValueCandidate(
            value=1.0e9, unit="Pa", provenance="llm",
            property_label="yield stress", method="explicit",
            evidence="σ_y = 1.0 GPa", source="test", confidence=0.9,
        )],
        "twin_thickness_lambda": [ValueCandidate(
            value=15e-9, unit="m", provenance="llm",
            property_label="twin thickness λ", method="explicit",
            evidence="λ = 15 nm", source="test", confidence=0.9,
        )],
        "hall_petch_k": [ValueCandidate(
            value=0.12, unit="MPa·m^(1/2)", provenance="llm",
            property_label="k_λ", method="explicit",
            evidence="k_λ = 0.12 MPa·m^(1/2)", source="test",
            confidence=0.9,
        )],
        "mu": [ValueCandidate(
            value=45e9, unit="Pa", provenance="llm",
            property_label="shear modulus G", method="explicit",
            evidence="G = 45 GPa", source="test", confidence=0.9,
        )],
        "poisson_ratio_nu": [ValueCandidate(
            value=0.34, unit="", provenance="llm",
            property_label="ν", method="explicit",
            evidence="ν = 0.34", source="test", confidence=0.9,
        )],
        "burgers_vector_b": [ValueCandidate(
            value=0.256e-9, unit="m", provenance="llm",
            property_label="b", method="explicit",
            evidence="b = 0.256 nm", source="test", confidence=0.9,
        )],
    }

    derived = derive_sigma0_and_tau_p(fake_cands, "Cu")
    assert derived, "bridge produced zero derived candidates"

    sigma0 = next((c for c in derived
                   if c.method == "hall_petch_intercept"), None)
    assert sigma0 is not None, "Hall–Petch path did not fire"
    ...
    assert PROVENANCE_MARKERS['derived']['marker'] == 'v'

    table_text = (
        "| Field            | Value            |\n"
        "| Grain size d     | 50 nm            |\n"
        "| Yield stress σ_y | 1.0 GPa          |\n"
        "| k_λ              | 0.12 MPa·m^(1/2) |\n"
    )
    rows = parse_side_note_table(table_text)
    assert rows, "side-note table parser returned nothing"
    params = {r["param"] for r in rows}
    assert "grain_size_d" in params, f"missing grain_size_d in {params}"
    assert "sigma0_yield" in params, f"missing sigma0_yield in {params}"
    assert "hall_petch_k" in params, f"missing hall_petch_k in {params}"

    logger.info(
        "v8.8.3 regression test: PASS  σ₀=%.2f MPa  "
        "τ_P(P-N)=%.3g MPa  τ_P(σ₀/M)=%.3f MPa  side-note rows=%d",
        sigma0.value / 1e6, tau_p_pn.value / 1e6,
        tau_p_taylor.value / 1e6, len(rows),
    )
