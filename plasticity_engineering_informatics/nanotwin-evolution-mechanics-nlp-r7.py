...
def render_plasticity_recommender_sidebar(
    ...
    if refresh_btn:
        ...
        try:
            idx = os.path.join(PlasticityFAISSRetriever.CACHE_DIR,
                               PlasticityFAISSRetriever.INDEX_FILE)
...
class RecommenderVisualStyle:
    ...
    def apply_plotly_layout(self, fig, title: str = "", xlabel: str = "", ylabel: str = ""):
        ...
        if self.grid:
            # NOTE: Plotly.py property names are ALL-LOWERCASE.
            #   ❌ gridDash  → ✅ griddash
            #   ❌ gridColor → ✅ gridcolor
            #   ❌ gridWidth → ✅ gridwidth
...
def render_recommender_histograms_pub(
    ...
    for idx, p in enumerate(available):
        ...
        # Recolor each patch by its value on the colormap
...
def render_recommender_visuals_dashboard():
    ...
    # ----- Layout: style controls on left, chart on right -----
    ...
    with col_chart:
        ...
        if chart_bytes is None:
            ...
            with ec2:
                # Only Matplotlib charts can also emit SVG/PDF directly
...
def main():
    ...
    <strong>✅ PURE FFT SPECTRAL + AI PLASTICITY RECOMMENDER v8.1:</strong><br>
