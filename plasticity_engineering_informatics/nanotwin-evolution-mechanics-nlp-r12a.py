...
def plot_candidate_scores(candidates, scores, provenance, best_idx=None, *,
    ...
    C_BAR, C_BEST, C_EDGE = '#BBBBBB', '#0072B2', 'black'   # Okabe–Ito

    # ---- bars: colour encodes SELECTION only --------------------------------
...
PLASTICITY_ONTOLOGY: Dict[str, Dict[str, Any]] = {
    ...
    "gamma0_dot": {
        ...
        "symbol": "γ̇₀",
        # v8.4.0: extended alias list with space/hyphen variants and
        # contextual terms frequently used in the corpus.
        "aliases": [
...
class PlasticityParameterExtractor:
    ...
    def _heuristic(cls, text: str, material: str, temp_k: float,
        ...
        # If the standard alias scan above did not produce a gamma0_dot
        # candidate, try the contextual pattern: "strain rate" / "deformation
        # rate" / "loading rate" followed (within ~60 chars) by a number with
        # an s^-1 (or /s, per second) unit. This catches the common corpus
        # phrasing that lacks the word "reference".
        if not any(e["param"] == "gamma0_dot" for e in out):
            m = cls._CTX_RATE_RE.search(text)
            if m:
                num_str = m.group(1).replace(" ", "")
                # Handle "1.5 × 10^4" style within the captured group
                sci = re.match(
                    r"(\d+(?:\.\d+)?)(?:[×xX\*]10\^?\{?([+-]?\d+)\}?)?",
                    num_str,
                )
...
def render_plasticity_recommender_sidebar(
    ...
    st.subheader("🤖 Intelligent Plasticity Recommender v8.4.0")
    st.caption(
...
def main():
    ...
    <strong>✅ PURE FFT SPECTRAL + AI PLASTICITY RECOMMENDER v8.4.0:</strong><br>
    ...
    with st.sidebar:
        ...
        with st.expander("🧠 AI Plasticity Recommender v8.4.0", expanded=False):
