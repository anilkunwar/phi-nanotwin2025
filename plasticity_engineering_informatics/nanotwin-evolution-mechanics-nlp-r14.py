...
# ███ NEW (v8.8.0): VRH DERIVATION LAYER + FUNDAMENTALS RETRIEVAL      ██████
# ███   · The LLM is now asked to extract C11/C12/C44 VERBATIM as       ███
# ███     separate extraction items (param="C11"|"C12"|"C44").           ███
# ███   · A pure-Python layer (derive_shear_modulus_vrh) converts those  ███
...
# NOTE: r'\rho_0' uses a RAW string with a SINGLE backslash.
PARAM_META = {
...
PLASTICITY_ONTOLOGY: Dict[str, Dict[str, Any]] = {
    ...
    "mu": {
        "label": "Shear Modulus",
        "symbol": "μ",
        # v8.8.0: c44 / c_44 REMOVED from this alias list.  C44 is the
        # {001}<100> shear elastic constant — NOT the polycrystalline
        # shear modulus.  Treating them as synonyms was the aliasing bug
        # that filled the μ bucket with elastic-constant values and hid
        # the VRH derivation route from the recommender.
        "aliases": [
...
# ----------------------------------------------------------------------------
# v8.8.0: FUNDAMENTAL PARAMETERS (elastic constants)
# ----------------------------------------------------------------------------
# C11 / C12 / C44 are NOT plastic parameters — they are the raw inputs from
# which μ is derived.  They live outside the 5-param ontology and outside the
# bucket scoring system.  The extractor produces them; the derivation layer
# consumes them; the μ bucket gets the results.
FUNDAMENTAL_PARAMS: set = {'C11', 'C12', 'C44'}

_FUNDAMENTAL_ALIASES: Dict[str, str] = {
    # C11
    "c11": "C11", "c_11": "C11", "c₁₁": "C11", "c11_(gpa)": "C11",
    "elastic constant c11": "C11", "elastic constant c_11": "C11",
    "c11gpa": "C11",
    # C12
    "c12": "C12", "c_12": "C12", "c₁₂": "C12", "c12_(gpa)": "C12",
    "elastic constant c12": "C12", "elastic constant c_12": "C12",
    # C44
    "c44": "C44", "c_44": "C44", "c₄₄": "C44", "c44_(gpa)": "C44",
    "elastic constant c44": "C44", "elastic constant c_44": "C44",
    "shear constant c44": "C44",
}

# ----------------------------------------------------------------------------
# v8.8.0: second retrieval pass — target the fundamental sources directly
# ----------------------------------------------------------------------------
# The μ-alias query never retrieves chunks that contain "C11 = 168.4 GPa"
# because the words "shear modulus" are usually absent from the same
# sentence.  We run a second pass with these queries and merge the results.
FUNDAMENTAL_QUERIES: Dict[str, List[str]] = {
    'mu': [
        "single-crystal elastic constants C11 C12 C44",
        "elastic stiffness coefficients copper aluminium nickel GPa",
        "Voigt Reuss Hill average shear modulus from elastic constants",
        "Zener anisotropy ratio cubic elastic constants",
        "ab initio elastic constants C11 C12 C44 temperature dependence",
    ],
}

# ----------------------------------------------------------------------------
# v8.8.0: dimensionless-context veto
# ----------------------------------------------------------------------------
# Phase-field and MD papers commonly write "we set μ = 1" (reduced units).
# Retrieval matches "shear modulus" verbatim; the LLM attaches the unit from
# PARAM_META['mu']['unit'] = 'GPa' because nothing told it not to.  The
# resulting 1.00 GPa candidate then gets high MoE score because the corpus
# consensus is 1.00.  This veto purges those candidates before scoring.
DIMENSIONLESS_CONTEXT: Tuple[str, ...] = (
    'nondimensional', 'non-dimensional', 'dimensionless',
    'reduced units', 'reduced unit', 'in reduced',
    'normalized units', 'normalised units',
    'scaled units', 'in units of', 'model units',
    'lattice units', 'normalized to', 'normalised to',
    'arbitrary units', 'a.u.', 'in a.u.',
)


def is_dimensionless_context(text_window: Optional[str]) -> bool:
    if not text_window:
        return False
    low = text_window.lower()
    return any(k in low for k in DIMENSIONLESS_CONTEXT)


# ----------------------------------------------------------------------------
# PARAM ALIAS MAP — v8.8.0: c44 / c_44 removed from the mu list
# ----------------------------------------------------------------------------
...
def _canonicalize_param(raw_p: str) -> Optional[str]:
    """Map whatever the LLM emits to a canonical key.

    Extended in v8.8.0 to also canonicalize C11 / C12 / C44 into the
    FUNDAMENTAL_PARAMS namespace (returned verbatim, uppercase).
    """
    if raw_p is None:
        return None
    p = str(raw_p).strip()
    ...
    if p in PLASTICITY_ONTOLOGY:
        return p
    if p in FUNDAMENTAL_PARAMS:
        return p

    if p in _PARAM_ALIASES:
        return _PARAM_ALIASES[p]
    if p in _FUNDAMENTAL_ALIASES:
        return _FUNDAMENTAL_ALIASES[p]

    low = p.lower().replace(" ", "_").replace("-", "_")
    if low in PLASTICITY_ONTOLOGY:
        return low
    if low in FUNDAMENTAL_PARAMS:
        return low
    if low in _PARAM_ALIASES:
        return _PARAM_ALIASES[low]
    if low in _FUNDAMENTAL_ALIASES:
        return _FUNDAMENTAL_ALIASES[low]

    for canon in PLASTICITY_ONTOLOGY:
        if canon in low or low in canon:
            return canon
    # Fundamental params: only match full token to avoid 'c11' matching inside
    # unrelated words.
    for canon in FUNDAMENTAL_PARAMS:
        if low == canon.lower() or low.startswith(canon.lower() + '_'):
            return canon
    for alias, canon in _PARAM_ALIASES.items():
        if alias and (alias in low or low in alias):
            return canon
    for alias, canon in _FUNDAMENTAL_ALIASES.items():
        if alias and (alias in low or low in alias):
            return canon
    return None
...
def _pl_fmt(param: str, si_value: float) -> str:
    ...
    # v8.8.0: omit the unit glyph when it is a placeholder for dimensionless.
    _dim_placeholders = ('–', '-', 'dimensionless', '')
    ui_unit = spec.get('ui_unit', '')
    suffix = f" {ui_unit}" if ui_unit not in _dim_placeholders else ""
    if param in ("rho0", "gamma0_dot"):
        return f"{ui_val:.2e}{suffix}"
    if param == "mu":
        return f"{ui_val:.2f}{suffix}"
    if param == "sigma0":
        return f"{ui_val:.1f}{suffix}"
    return f"{ui_val:.2f}{suffix}"
...
def _cij_in_gpa(value: float, unit: str) -> float:
    ...
    because published Cij tables are almost always printed in GPa."""
    u = (unit or "").strip().lower().replace("μ", "u").replace("µ", "u")
    v = float(value)
    if 'tpa' in u:
        return v * 1e3
            ...
            for spec in PLASTICITY_ONTOLOGY.values():
                for alias in spec["aliases"]:
                    if alias in low:
                        score += 1.0
                        break
            # v8.8.0: boost docs that mention the fundamental constants
            for kk in ('c11', 'c12', 'c44', 'elastic constant', 'voigt',
                       'reuss', 'hill', 'zener'):
                if kk in low:
                    score += 1.5
    ...
    def search(self, query: str, k: int = 15,
        ...
        for sim, idx in zip(sims, idxs):
            ...
            if material_hint and material_hint.lower() in doc["text"].lower():
                bonus += 0.05
            # v8.8.0: small boost for documents that look like elastic-constant
            # sources — helps the second (fundamental) retrieval pass surface
            # useful chunks.
            low = doc["text"].lower()
            if any(k in low for k in ('c11', 'c12', 'c44',
                                       'elastic constant', 'voigt',
                                       'reuss', 'hill')):
                bonus += 0.03
...
and unambiguously stated as numerical values in the text. Do NOT infer. Do
NOT fill in missing parameters.

Parameters of interest (the `param` field MUST be exactly one of these
eight ASCII strings — do NOT use symbols or synonyms):
  ...
  - "C11"         (cubic elastic constant, GPa)
  - "C12"         (cubic elastic constant, GPa)
  - "C44"         (cubic elastic constant, GPa)

⚠️ CRITICAL RULES — VIOLATIONS WILL BE REJECTED:

  · "C11", "C12", "C44" are the SINGLE-CRYSTAL ELASTIC CONSTANTS. They are
    NOT the polycrystalline shear modulus μ. NEVER place an elastic-constant
    value in the "mu" field and NEVER place a μ value in a C11/C12/C44 field.

  · Report values VERBATIM. Do NOT calculate, average, convert, or infer.
    If the text says "C11 = 168.4 GPa", write 168.4 with unit "GPa". Do not
    write 168.4e9 unless the text uses 168.4e9.

  · If a value is described as nondimensional, dimensionless, reduced, or
    normalized units, set is_dimensionless=true and leave unit empty. DO NOT
    attach GPa. In particular, "we set μ = 1" in a phase-field paper means
    is_dimensionless=true, unit="" — NOT "1 GPa".

  · Omit any parameter whose value is not explicitly present. Do not guess.

For each parameter you find, quote the exact sentence as evidence.
Set method="explicit" and confidence >= 0.8.
...
⚠️ The `param` field MUST be EXACTLY one of these eight ASCII strings:
  ...
  ELASTIC-CONSTANT SET (cubic symmetry — single-crystal):
    "C11"         — cubic elastic constant                 [GPa]
    "C12"         — cubic elastic constant                 [GPa]
    "C44"         — cubic elastic constant                 [GPa]

Do NOT use "rho_0", "ρ₀", "γ̇₀", "m", "σ₀", "shear_modulus", or any other
variant. The parser is strict.

🚨 CRITICAL — READ TWICE:
  · C11, C12, C44 are NOT synonyms of μ. C44 is the {001}<100> shear
    elastic constant of a single crystal; μ is the polycrystalline shear
    modulus. A paper that says "C44 = 75.4 GPa" is reporting a fundamental
    constant, not μ. Put 75.4 in the "C44" field, NEVER in the "mu" field.
  · Report every value VERBATIM. Do NOT calculate the Voigt/Reuss/Hill
    average yourself. Do NOT convert GPa to Pa. Do NOT average.
  · If the text says "we set μ = 1" or uses "reduced units", set
    is_dimensionless=true and leave unit empty. Do NOT write "1 GPa".
...
PATH B — REASONED INFERENCE (only if Path A does not apply AND the
parameter is one of the five PLASTIC parameters).
  ...
  ⚠️ Do NOT use Path B for C11 / C12 / C44. Those are ONLY reported when
  explicitly present. If the text does not give them, omit them.
...
REASONING CHAINS (use these formulas, do NOT just look up a constant):
  ...
  mu (T, material, phase state):
    ...
    Step 2: If the text supplies single-crystal elastic constants
            (C11, C12, C44 or equivalent), DO NOT compute μ yourself.
            Return the C11/C12/C44 items as separate Path-A extractions.
            The caller will derive μ from them. For the μ field itself,
            leave value=null.

    Step 3: Otherwise, use these room-T polycrystalline VRH shear-modulus
            baselines (GPa):
    ...
    Step 5: Multiplicative defect / alloying corrections -- apply ONLY
            the corrections explicitly supported by the text. Do NOT stack
            corrections the text does not mention.
              * Solid-solution hardening: +3 to +8 % per 5 at.% solute.
  ...
  gamma0_dot (T, strain_rate, material):
    ...
    Step 4: Emit ONE JSON object with param="gamma0_dot", value=<number>,
            unit="s^-1", method="LLM_inferred" (or "explicit" if Path A).
...
CRITICAL RULES:
- Return the five PLASTIC parameters for every document, including
  gamma0_dot.
- Return C11 / C12 / C44 ONLY when the text explicitly states them.
  NEVER fabricate them.
- The `param` field MUST be exactly "rho0", "mu", "gamma0_dot", "srs",
  "sigma0", "C11", "C12", or "C44".
...
- If the text contains NO information at all, still return Path B for
  all 5 plastic parameters with the simplest possible chain and
...
class PlasticityParameterExtractor:
    ...
    # v8.8.0: dedicated pattern for C11 / C12 / C44 tables. Matches
    # "C11 = 168.4 GPa", "C_12 : 121.4", "c44 ≈ 75.4 GPa", "C44=75.4".
    _CIJ_RE = re.compile(
        r"\bc\s*_?\s*(11|12|44)\s*(?:[=:≈]|\bis\b)\s*"
        r"([+-]?\d+(?:\.\d+)?(?:\s*[×xX\*]\s*10\s*\^?\s*\{?[+-]?\d+\}?)?)"
        r"\s*(GPa|MPa|TPa|Pa)?",
        re.I,
    )
    ...
    def _validate(raw: Any) -> List[Dict[str, Any]]:
        ...
        for item in raw:
            ...
            # v8.8.0: carry the dimensionless flag through.
            is_dimless = item.get("is_dimensionless", False)
            if isinstance(is_dimless, str):
                is_dimless = is_dimless.strip().lower() in ('true', '1', 'yes')
            else:
                is_dimless = bool(is_dimless)

            out.append({
                "param": p,
                "value": float(v),
                "unit": str(unit or ""),
                "is_dimensionless": is_dimless,
    ...
    def _heuristic(cls, text: str, material: str, temp_k: float,
        ...
        # ---- v8.8.0: C11/C12/C44 heuristic detection ----------------------
        for m in cls._CIJ_RE.finditer(text):
            idx_str = m.group(1)
            try:
                mantissa = float(m.group(2))
            except (ValueError, TypeError):
                continue
            unit = (m.group(3) or "GPa").strip()
            try:
                value = _cij_in_gpa(mantissa, unit)
            except Exception:
                continue
            # Reject any candidate outside physical bounds — the C11 heuristic
            # is allowed to be conservative because we only feed plausible
            # triples to the derivation layer anyway.
            ...
            out.append({
                ...
                "method": "heuristic",
                "confidence": 0.35,
                "evidence": text[max(0, m.start() - 40): m.end() + 40].strip(),
                "reasoning": "",
            })
        # -------------------------------------------------------------------
        ...
        for param, spec in PLASTICITY_ONTOLOGY.items():
            for alias in spec["aliases"]:
                ...
                try:
                    ...
                    continue
                # v8.8.0: check for dimensionless context
                is_dimless = is_dimensionless_context(window)
                out.append({
                    "param": param,
                    "value": value,
                    "unit": unit,
...
class PlasticityCandidate:
    ...
    reasoning: str = ""
    clamped: bool = False
    is_dimensionless: bool = False   # v8.8.0
...
class PlasticityLatentMoEScorer:
    ...
    def score(self, extractions, target_material, target_temp,
        ...
        for ext in extractions:
            p = ext.get("param")
            # v8.8.0: fundamental params (C11/C12/C44) are handled by the
            # derivation layer, not by the bucket system.
            if p in FUNDAMENTAL_PARAMS:
                continue
            if p not in buckets:
                continue
            ...
            buckets[p].append(PlasticityCandidate(
                ...
                clamped=was_clamped,
                is_dimensionless=bool(ext.get("is_dimensionless", False)),
            ))
    ...
    # v8.8.0: exposed for the derivation layer so it can compute a score
    # for the synthesized candidate using the same 6-expert scheme.
    def score_derived(self, method: str, material: str, temp_k,
                      target_material: str, target_temp: float,
                      reasoning: str, confidence: float = 0.7) -> float:
        s_mat = self._material_expert(material, target_material)
        s_temp = self._thermal_expert(temp_k, target_temp)
        s_strain = 0.5
        s_method = self._method_expert(method)
        s_conf = float(confidence)
        s_reason = self._reasoning_expert(reasoning, method)
        return (self.w_material * s_mat
                + self.w_thermal * s_temp
                + self.w_strain * s_strain
                + self.w_method * s_method
                + self.w_confidence * s_conf
        ...
        for doc in corpus[:max_docs]:
            ...
            for ext in extractions:
                ...
                if (ext.get("method") or "").lower() == "default_fallback":
                    continue
                # v8.8.0: skip dimensionless-flagged entries for the prior
                # pool (they are not GPa / Pa values).
                if ext.get("is_dimensionless"):
                    continue
                try:
                    v_si = _normalize_unit(ext["value"], ext.get("unit", ""), p)
                except Exception:
...
class PlasticityRecommendationBundle:
    ...
    llm_used: bool
    n_derived_mu: int = 0
    n_dimensionless_excluded: int = 0
...
class PlasticityRecommender:
    ...
    def recommend(self, material: str, temp_k: float, strain_rate: float = 1e-3,
        ...
        # ── 1. Standard retrieval + extraction ──────────────────────────
        if self.retriever._docs:
        ...
        for i, doc in enumerate(docs):
            if progress_callback:
                progress_callback(i + 1, total_docs + 15, doc.get("title", "")[:60])
        ...
        # ── 2. v8.8.0: second retrieval pass for fundamentals (Cij) ─────
        # The μ-alias query never retrieves chunks that contain "C11 = 168.4"
        # because "shear modulus" is usually absent from the same sentence.
        fundamental_docs: List[Dict[str, Any]] = []
        if self.retriever._docs:
            seen_keys = {(d.get("source", ""), d.get("title", "")) for d in docs}
            for q in FUNDAMENTAL_QUERIES.get('mu', []):
                hits = self.retriever.search(q, k=8, material_hint=material)
                for h in hits:
                    key = (h.get("source", ""), h.get("title", ""))
                    if key in seen_keys:
                        continue
                    seen_keys.add(key)
                    fundamental_docs.append(h)

        for j, doc in enumerate(fundamental_docs):
            if progress_callback:
                progress_callback(total_docs + j + 1, total_docs + 15,
                                  f"[Cij] {doc.get('title', '')[:50]}")
            ext = self.extractor.extract(
                doc["text"], material, temp_k, strain_rate,
                use_llm=self.llm_available,
                mode="reasoned_inference",
        ...
        # ── 3. Score the standard parameters ────────────────────────────
        ...
        return PlasticityRecommendationBundle(
            ...
            n_derived_mu=n_derived,
            n_dimensionless_excluded=n_dimless,
        )
...
def _plr_render_parameter_selector(param: str,
    ...
    if best is not None:
        ...
        options.append((
            ...
            f"(score {best.score:.2f}){_prov_tag}",
            best.value_si, best,
        ))
    ...
    for i, c in enumerate(bundle.candidates.get(param, [])[1:], start=1):
        ...
        options.append((
            f"   Alt {i}: {_pl_fmt(param, c.value_si)} (score {c.score:.2f}){_prov_tag}",
            c.value_si, c,
        ))
    ...
    if chosen is not None:
        ...
        if chosen.reasoning:
            with st.expander("🧠 Derivation / reasoning chain", expanded=False):
...
def render_plasticity_recommender_sidebar(
    ...
    st.caption(
        ...
        f"🔬 VRH-derived μ candidates: **{bundle.n_derived_mu}** · "
        f"🚫 dimensionless excluded: **{bundle.n_dimensionless_excluded}**"
    )

    if bundle.n_derived_mu > 0:
        st.success(
    ...
    if bundle.n_dimensionless_excluded > 0:
        st.info(
            f"🚫 {bundle.n_dimensionless_excluded} reduced-units / "
            "dimensionless μ candidate(s) excluded from the pool. "
            "These were probably 'μ = 1' entries from phase-field papers; "
            "they are diagnostic noise, not GPa values."
        )
        ...
        with col_b:
            ...
            show_cbar = st.checkbox(
                ...
                help=("Adds a colourbar showing the score→colour mapping. "
                      "Only available in 'score' colour mode.")
        ...
        with t_col1:
            ...
            ann_offset = st.slider(
                ...
                help="Gap in POINTS between the top of each bar and its value "
                     "label.  This is a POSITION, not a size.",
            )
