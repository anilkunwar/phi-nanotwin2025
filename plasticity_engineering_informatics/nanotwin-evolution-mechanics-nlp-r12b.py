...
# ███ + PLASTICITY PARAMETER INTELLIGENT RECOMMENDER v8.4.0              ███
...
# ███ NEW (v8.6.0): CONFIGURABLE / RESPONSIVE BAR COLOURS             ████
# ███   · color_mode = 'highlight' | 'score' | 'cycle'                  ███
# ███   · palette: None | 'journal' | palette-name | colormap-name |    ███
# ███              explicit list of colours                              ███
# ███   · 'highlight': tinted-family accent colour (no more flat grey)   ███
# ███   · 'score':     bar fill = f(score) via any colormap              ███
# ███   · 'cycle':     bar fill cycles through a categorical palette     ███
# ███   · Winner is ALWAYS flagged by heavier outline, so the selection  ███
# ███     channel is redundant with (not overwritten by) the colour      ███
# ███     channel; greyscale-safe and colour-blind-safe in every mode.   ███
# ============================================================================
...
import matplotlib.colors as mcolors              # v8.6.0: _to_hex / _to_rgb
    ...
    def get_journal_styles():
        return {
            ...
            'custom': {
                ...
                'color_cycle': [mcolors.to_hex(c) for c in plt.cm.Set2(np.linspace(0, 1, 10))]
            }
        }
...
# ============================================================================
# LATENTMoE CANDIDATE-SCORE BAR CHART — v8.5.0 / v8.5.1 / v8.6.0
# ============================================================================
...
# ----------------------------------------------------------------------------
# v8.6.0: CONFIGURABLE BAR COLOURS
# ----------------------------------------------------------------------------
def _to_hex(c):
    """Safely convert any matplotlib colour spec (hex / name / RGBA tuple)
    to a hex string. Falls back to a neutral grey on failure."""
    if c is None:
        return '#BBBBBB'
    try:
...
def _tint(color, amount: float = 0.72) -> str:
    """Mix `color` with white -> cohesive family instead of flat grey.
    amount = 0 → unchanged; amount = 1 → pure white."""
    try:
        rgb = mcolors.to_rgb(color)
    except Exception:
        rgb = (0.73, 0.73, 0.73)
    return _to_hex(tuple(ch + (1.0 - ch) * amount for ch in rgb))


def _resolve_palette(palette, journal: str = 'nature'):
    """Resolve a `palette` argument to a list of hex colours.
       None / 'default' / 'minimal' → None  (meaning: keep the minimal scheme)
       'journal'                    → the active journal preset's colour_cycle
       a palette name               → PUBLICATION_PALETTES[name]
       a list/tuple                 → each entry coerced to hex
       anything else                → None
    """
    if palette is None:
        return None
    if isinstance(palette, str):
        if palette in ('', 'default', 'minimal'):
            return None
        if palette in PUBLICATION_PALETTES:
            return list(PUBLICATION_PALETTES[palette])
        if palette == 'journal':
            styles = JournalTemplates.get_journal_styles()
            cyc = styles.get(journal, styles['nature'])['color_cycle']
            return [_to_hex(c) for c in cyc] or None
        # unknown string → treat as no palette
        return None
    if isinstance(palette, (list, tuple)):
        hexes = [_to_hex(c) for c in palette if c is not None]
        return hexes or None
    return None


def _score_mapped_faces(scs, cmap_name='viridis', lo: float = 0.25, hi: float = 0.92):
    """Bar fill = f(score) through a colormap, sampled strictly inside [lo, hi]
    so the near-black/near-white extremes — where black edges and value labels
    lose contrast — are never hit.
    """
    if isinstance(cmap_name, mcolors.Colormap):
        cm = cmap_name
    else:
        try:
            cm = plt.get_cmap(str(cmap_name))
        except Exception:
            cm = plt.get_cmap('viridis')
    smin, smax = float(min(scs)), float(max(scs))
    rng = (smax - smin) or 1.0
    return [_to_hex(cm(lo + (hi - lo) * (s - smin) / rng)) for s in scs]
...
def plot_candidate_scores(candidates, scores, provenance, best_idx=None, *,
                          ...
                          score_std=None,
                          color_mode: str = 'highlight',
                          palette: Union[None, str, List, Tuple] = None):
    """
    ...
    score_std : optional per-candidate score uncertainty
    color_mode: 'highlight' | 'score' | 'cycle'
    palette   : None | 'journal' | palette-name | colormap-name | list of colours
    """
    n = len(candidates)
    assert n == len(scores) == len(provenance), 'length mismatch'

    param_symbol = normalize_tex(param_symbol)
    color_mode = (color_mode or 'highlight').lower()
    if color_mode not in ('highlight', 'score', 'cycle'):
    ...
    # ---------------------------------------------------------------------
    # v8.6.0: CONFIGURABLE / RESPONSIVE BAR COLOURS
    # ---------------------------------------------------------------------
    C_EDGE = 'black'
    if color_mode == 'score':
        faces = _score_mapped_faces(scs, palette or 'viridis')
    elif color_mode == 'cycle':
        pal = _resolve_palette(palette, journal) \
              or list(PUBLICATION_PALETTES['okabe_ito'])
        faces = [pal[i % len(pal)] for i in range(n)]
    else:  # 'highlight'
        pal = _resolve_palette(palette, journal)
        c_best = pal[0] if pal else '#0072B2'
        c_bar = _tint(c_best, 0.72) if pal else '#BBBBBB'
        faces = [c_best if i == best_x else c_bar for i in range(n)]

    # winner ALWAYS flagged by a heavier outline — the selection channel
    # survives even in 'score' / 'cycle' modes where colour does not
    # identify the winner.
    edge_lws = [1.8 if i == best_x else 0.8 for i in range(n)]

    ax.bar(xs, scs, width=bar_width, facecolor=faces,
           edgecolor=C_EDGE, linewidth=edge_lws, zorder=3)
    ...
    # ---- value labels: padded in POINTS, white halo, bold winner --------------
    ...
    parts = [p for p in (param_title, f'${param_symbol}$' if param_symbol else '') if p]
    xlabel = ' '.join(parts)
    if unit:
    ...
    if best_x is not None:
        if color_mode == 'highlight':
            handles.append(Patch(facecolor=faces[best_x], edgecolor=C_EDGE,
                                 linewidth=1.8, label='Best match'))
        else:
            handles.append(Patch(facecolor='white', edgecolor=C_EDGE,
                                 linewidth=1.8, label='Best match'))
...
# ----------------------------------------------------------------------------
# STREAMLIT WIRING — screen render + hi-res download.
# ----------------------------------------------------------------------------
@handle_errors
    ...
    def learn(self, corpus, material: str = "?", temp_k: float = 300.0,
        ...
        for doc in corpus[:max_docs]:
            ...
            extractions = self.extractor.extract(
                text, material=material, temp_k=temp_k, strain_rate=strain_rate,
                use_llm=use_llm, mode="strict_extract", debug_llm=debug_llm,
            )
            for ext in extractions:
        ...
        if build_priors:
            try:
                priors_df = self.prior_learner.learn(
                    ...
                    max_docs=min(150, len(corpus)), debug_llm=self.debug_llm,
                )
            except Exception as e:
...
def render_plasticity_recommender_sidebar(
    ...
    st.caption(
        ...
        "dual-mode prompts · param-alias canonicalization · Chain-of-Thought "
        "surfaced in UI · **v8.6.0: configurable bar colours (highlight / "
        "score-mapped / cycle)**."
    )
    ...
    def apply_plotly_layout(self, fig, title: str = "", xlabel: str = "", ylabel: str = ""):
        fig.update_layout(
            ...
            font=dict(family=self.font_family,
                      size=self.font_size_axis, color='#1e293b'),
...
# ----------------------------------------------------------------------------
# BAR CHART — v8.6.0 configurable colours
# ----------------------------------------------------------------------------
_COLOR_MODE_LABELS = {
    'highlight': 'Highlight best match',
    'score': 'Score-mapped colormap',
    'cycle': 'Categorical palette',
}

_SCORE_CMAPS = ['viridis', 'plasma', 'inferno', 'magma',
                'cividis', 'YlOrRd', 'Blues', 'turbo']


def _render_latentmoe_color_controls():
    """v8.6.0: Renders the colour-configuration expander for the bar chart.
    Returns (color_mode:str, palette: Optional[str]).
    """
    with st.expander("🎨 Candidate-Score Chart Colours", expanded=False):
        m_col, p_col = st.columns(2)
        color_mode = m_col.selectbox(
            "Colour mode",
            ['highlight', 'score', 'cycle'],
            index=0,
            key='reco_color_mode',
            format_func=lambda m: _COLOR_MODE_LABELS.get(m, m),
            help=("highlight = best vs rest (default, journal-defensible)\n"
                  "score = bar colour = LatentMoE score through a colormap\n"
                  "cycle = categorical palette per candidate (avoid for score data)"),
        )
        palette = None
        if color_mode == 'highlight':
            choice = p_col.selectbox(
                "Accent palette",
                ['default', 'journal'] + list(PUBLICATION_PALETTES),
                index=0, key='reco_accent',
                help=("'journal' follows the currently selected journal "
                      "preset (Nature blue, Science blue, PRL black, …)."),
            )
            palette = None if choice == 'default' else choice
        elif color_mode == 'score':
            palette = p_col.selectbox(
                "Score colormap",
                _SCORE_CMAPS,
                index=0, key='reco_score_cmap',
                help=("Sequential maps are recommended (viridis / plasma / "
                      "inferno / magma / cividis / YlOrRd / Blues / turbo). "
                      "Diverging maps put a false midpoint at the centre of "
                      "the score range."),
            )
        else:  # cycle
            palette = p_col.selectbox(
                "Categorical palette",
                list(PUBLICATION_PALETTES),
                index=0, key='reco_cat_palette',
            )
...
def render_recommender_bars_pub(bundle: PlasticityRecommendationBundle,
    ...
    v8.5.0 fixes:
      1. Labels no longer collide with bar tops/spines.
      2. Mathtext scientific notation with the unit stated once.
      3. LLM/heuristic provenance via marker shape + above-axes legend.

    v8.5.1 hardening:
      · PARAM_META symbols were r'\\\\rho_0' → mathtext ParseException.
        Fixed to r'\\rho_0' + safe_mathtext() everywhere.

    v8.6.0 colours:
      · color_mode = 'highlight' | 'score' | 'cycle', palette from UI.
      · Winner flagged by heavier outline so the selection channel
        survives regardless of colour mode.
    """
    ...
    # v8.6.0: expose colour controls
    color_mode, palette = _render_latentmoe_color_controls()
    ...
    # Small diagnostic so the user can immediately confirm the toggle reached
    # the chart — remove if you find it noisy.
    st.caption(f"🎨 color_mode = `{color_mode}` · palette = `{palette}`")
...
# ----------------------------------------------------------------------------
# PUBLICATION HISTOGRAMS (Matplotlib)
# ----------------------------------------------------------------------------
...
def render_recommender_style_controls() -> RecommenderVisualStyle:
    ...
    with st.expander("🖼️ Figure & DPI", expanded=False):
        ...
        with c1:
            ...
            style.line_width = st.slider(
                "Line width", 0.5, 5.0, float(style.line_width), 0.1, key="rec_lw")
        with c2:
            style.figure_height = st.slider(
                "Figure height (in)", 3.0, 16.0, float(style.figure_height), 0.5,
                key="rec_fh")
            style.marker_size = st.slider(
                "Marker size", 2.0, 20.0, float(style.marker_size), 0.5, key="rec_ms")
        with c3:
    ...
    with st.expander("🎯 Labels, Titles & Legend", expanded=False):
        ...
        style.xlabel_override = st.text_input(
            "X label override", value=style.xlabel_override, key="rec_xlabel_ovr",
        )
        style.ylabel_override = st.text_input(
            "Y label override", value=style.ylabel_override, key="rec_ylabel_ovr",
        )
        c1, c2 = st.columns(2)
        with c1:
            ...
            if style.show_legend:
                legend_choices = ['best', 'upper right', 'upper left', 'lower right',
                                  'lower left', 'center right', 'center left',
                                  'upper center', 'lower center', 'center']
                style.legend_position = st.selectbox(
                    "Legend position", legend_choices,
                    index=legend_choices.index(style.legend_position)
                    if style.legend_position in legend_choices else 0,
                    key="rec_legend_pos",
                )
        with c2:
    ...
    with st.expander("🧱 Backgrounds & Spines", expanded=False):
        ...
        with c1:
            style.background_color = st.color_picker(
                "Figure background", style.background_color, key="rec_bg_color")
            style.plot_background_color = st.color_picker(
                "Plot background", style.plot_background_color, key="rec_plot_bg")
        ...
        with c2:
            style.spine_color = st.color_picker(
                "Spine / tick color", style.spine_color, key="rec_spine_color")
            style.spine_width = st.slider(
                "Spine width", 0.5, 4.0, float(style.spine_width), 0.1,
                key="rec_spine_w")
            style.grid = st.checkbox("Show grid", value=style.grid, key="rec_grid")
            if style.grid:
...
def main():
    ...
    • <span style="color: green;">🎨 CONFIGURABLE BAR COLOURS (v8.6.0):</span> color_mode = <code>highlight</code> / <code>score</code> / <code>cycle</code>; palette from the new "🎨 Candidate-Score Chart Colours" expander; winner always flagged by a heavier outline (greyscale-safe).<br>
