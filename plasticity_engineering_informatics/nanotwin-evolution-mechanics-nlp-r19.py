...
# ███ + PLASTICITY PARAMETER INTELLIGENT RECOMMENDER v9.2.2              ███
...
# ============================================================================
# ███ v9.2.2 — 3-BUCKET DISPLAY TAXONOMY + PLOTTING                    ███
# ============================================================================
...
def derive_sigma0_and_tau_p(
    ...
    # ── v9.2.1 sign-off: derived / regime_prior are REJECTED as w sources.
...
def _pick_best(cands):
    if not cands:
        return None
    # v9.2.1 — trust keyed on FINE provenance keys.
    trust = {
...
def build_derived_routes(ctx, mu_GPa=None, w_mode='auto') -> List[DerivedRoute]:
    ...
    # v9.2.1 sign-off: only llm_extract / llm_reasoned / regex_ner are
    # trusted as captured-w sources.
    _W_TRUSTED_FINE = ('llm_extract', 'llm_reasoned', 'regex_ner')
...
def gatekeep(items, param_key, provenance="regex_ner", conf_cap=None):
    ...
    v9.2.2: provenance is now a FINE key.  Confidence caps keyed on fine
    provenance; regime_prior behaves like llm_prior (0.5), derived at 0.75.
    """
    canon = PARAM_CANON.get(param_key, {})
    (lo, hi) = canon.get("plausible", (float("-inf"), float("inf")))
...
def recommend_param_values(param_key, material="Cu", value_hints=None,
    ...
    if use_llm and records:
        try:
            ...
            cands = gatekeep(raw_items, param_key, provenance="llm_extract")
        except Exception as e:
    ...
    # ── Tier 2 — heuristic / regex NER (M1-corrected count) ───────────
    diag.setdefault("n_heuristic", 0)
    if not cands and records:
        hits = heuristic_extract(records, param_key)
        for _h in hits:
            if isinstance(_h, dict):
                _stamp_provenance(_h, 'regex_ner')
        cands = gatekeep(hits, param_key, provenance="regex_ner")
    ...
    if not cands and use_llm and allow_prior_inference:
        try:
            ...
            cands = gatekeep(raw_items, param_key,
    ...
    if records:
        try:
            ...
            for _it in parsed_items:
                if isinstance(_it, dict):
                    _stamp_provenance(_it, 'llm_extract')
            cands = gatekeep(parsed_items, "gamma0_dot",
                             provenance="llm_extract")
            # Re-attach theory tags if the gatekeeper dropped them in
            # any cascaded path.  M2-corrected: γ̇₀ is never in GPa, so we
            # read `value` not `value_gpa`.
            for c in cands:
    ...
    if not cands and target_theory:
        try:
            ...
            for _it in raw_items:
                if isinstance(_it, dict):
                    _stamp_provenance(_it, 'llm_prior')
            cands = gatekeep(raw_items, "gamma0_dot",
                             provenance="llm_prior", conf_cap=0.5)
...
# ════════════════════════════════════════════════════════════════════════
# ███ v9.2.2 — PlasticityCandidate (B1: provenance field appended)     ███
# ════════════════════════════════════════════════════════════════════════
@dataclass
class PlasticityCandidate:
    ...
    provenance: str = ""          # ── v9.2.2: fine key, fed by ext["_provenance"]
    ...
    def score(self, extractions, target_material, target_temp,
        ...
        for ext in extractions:
            ...
            cand_kwargs = dict(
                ...
                clamped=was_clamped,
                provenance=str(ext.get("_provenance", "")),   # ← B2
            )
            if p == "gamma0_dot":
...
class PlasticityRecommender:
    ...
    def recommend_grounded(self, material, temp_k, strain_rate=1e-3,
        ...
        for i, param in enumerate(PARAM_ORDER):
            ...
            # ── v9.2.2 §E4: read-side contract — one-line `_provenance` addition.
            for c in cands:
...
def render_plasticity_recommender_sidebar(
    ...
    st.subheader("🤖 Intelligent Plasticity Recommender v9.2.2")
    st.caption(
        "**v9.2.2 Honesty pass:** fine provenance keys (llm_extract / "
        "llm_reasoned / regex_ner / regime_prior / llm_prior / derived) "
        "drive the ledger; the reader-facing chart rolls them up to a "
        "3-bucket legend (LLM grounded / LLM prior / Deterministic). "
        "`llm_reasoned` is a *reserved* key — it stays unwired until a "
        "real chain-of-thought prompt path exists, so the present-only "
        "legend never promises more than the pipeline delivers."
    )
    st.caption(
        "**v9.1.0 theory-aware γ̇₀:** Johnson-Cook (≈1e-3 s⁻¹) vs CPFEM "
        "(≈1e-3 s⁻¹) vs DDD (≈5e3 s⁻¹) vs MD (≈1e7 s⁻¹) disambiguation. "
        "**v9.0.0 physics-grounded ρ₀:** (synthesis, architecture, λ) "
        "regime classifier. **v8.9.0 Friction Stress Lab:** HP(d), HP(λ), "
        "solver-law inversion, P–N+Taylor floor."
    )
...
def render_friction_stress_lab():
    ...
    # ── v9.2.2 §D3: text consumer → Unicode glyphs.
...
def render_recommender_bars_pub(bundle, style):
    ...
    # ── v9.2.2 §D4: legend granularity radio + counts checkbox ────────
    ...
    # ── v9.2.2 §B3: provenance list prefers c.provenance or c.method.
...
def render_recommender_visuals_dashboard():
    ...
    st.caption(
        ...
        "candidates. v9.2.2 rolls fine provenance keys up to a 3-bucket "
        "reader legend; `llm_reasoned` stays reserved and silent until a "
        "real CoT prompt path exists.")
...
def main():
    ...
    <strong>✅ PURE FFT SPECTRAL + AI PLASTICITY RECOMMENDER v9.2.2:</strong><br>
    ...
    • <span style="color: green;">🎯 v9.2.2 3-BUCKET LEGEND:</span> coarse display rollup (LLM grounded / LLM prior / Deterministic) over 6 fine keys. `llm_reasoned` is a RESERVED fine key — unwired until a genuine CoT prompt path exists.<br>
    ...
    with st.sidebar:
        ...
        if st.button("🔄 Reset to Defaults", type="secondary"):
            ...
            st.rerun()
        st.markdown("---")
        with st.expander("🧠 AI Plasticity Recommender v9.2.2", expanded=False):
...
def _regression_test_v921() -> None:
    ...
    # fill semantics pinned at the data level (matplotlib-version-safe)
    ...
    assert all(m['fill'] for m in LEGEND_MARKERS.values())  # coarse: shape only
    fig = plot_candidate_scores([1e14, 1e15], [0.8, 0.9],
                                ['heuristic', 'llm_prior'],
                                legend_granularity='fine')
    plt.close(fig)   # smoke: renders without error
...
def _run_regression_suite() -> None:
    """Run the v8.8–v9.2.2 battery.  Names resolve via globals().get()
