...
class InitialGeometryVisualizer:
    ...
    def create_twin_grain_geometry(self, twin_spacing=20.0, grain_boundary_pos=0.0, gb_width=3.0,
                                   buffer_width=5.0, gb_profile='plane', gb_curvature=0.0):
        """
        Create initial fields for a twinned grain with a buffer zone and curved GB.
        ...
        buffer_width : float
            Thickness of the twin‑free region adjacent to the GB (nm).
        gb_profile : str
        ...
        def gb_x_func(y):
            if gb_profile == 'plane':
                ...
                # Concave: curves into the twinned grain (negative deviation at center)
                # Use Gaussian bump for smooth decay to zero at edges
                sigma = (self.N * self.dx) / 4.0  # characteristic width
                deviation = -gb_curvature * np.exp(-y**2 / (2 * sigma**2))
                return grain_boundary_pos + deviation
            elif gb_profile == 'convex':
                # Convex: curves away from the twinned grain (positive deviation at center)
        ...
        # Assign twin order parameter φ (only inside twinned grain, beyond buffer zone)
        for i in range(self.N):
            for j in range(self.N):
                if eta1[i, j] > 0.5:  # inside the twinned grain
                    y_val = self.Y[i, j]
                    gb_x = gb_x_func(y_val)
                    # Horizontal distance to GB curve
                    dist_from_gb = abs(self.X[i, j] - gb_x)
                    if dist_from_gb > buffer_width:
                        phase = 2 * np.pi * self.Y[i, j] / twin_spacing
                        phi[i, j] = np.tanh(np.sin(phase) * 3.0)
                    else:
                        # Twin‑free buffer: set φ to a constant (variant +1)
                        phi[i, j] = 1.0
    ...
    def create_defect_geometry(self, twin_spacing=20.0, defect_type='dislocation', defect_pos=(0, 0), defect_radius=10.0,
                               grain_boundary_pos=0.0, gb_width=3.0, buffer_width=5.0,
                               gb_profile='plane', gb_curvature=0.0):
        """
        Create initial fields with a defect, using a curved GB and buffer zone.
        """
        ...
        phi, eta1, eta2 = self.create_twin_grain_geometry(
            ...
            buffer_width, gb_profile, gb_curvature
        )
    ...
    def initialize_fields(self):
        ...
        if geom_type == 'defect':
            ...
            return self.geom_viz.create_defect_geometry(
                ...
                gb_pos, gb_width, buffer_width, gb_profile, gb_curvature
            )
        else:
...
def main():
    ...
    • <span style="color: green;">BUFFER ZONE:</span> User‑controlled twin‑free region near the grain boundary (0–20 nm).<br>
    • <span style="color: green;">CURVED GB:</span> Choose plane, concave, or convex grain boundary profiles with adjustable amplitude.<br>
    • <span style="color: green;">INTEGRATED:</span> Full compatibility with existing analysis, database, and export.
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        if operation_mode == "Run New Simulation":
            ...
            # --- NEW: Buffer zone and curved GB parameters ---
            buffer_width = st.slider("Twin‑free buffer near GB (nm)", 0.0, 20.0, 5.0, 0.5,
                                     help="Twins start only beyond this distance from the GB.")
            ...
            if st.button("🚀 Initialize Simulation", type="primary", use_container_width=True):
                params = {
                    ...
                    'gb_width': 3.0,                     # fixed transition width
                ...
                if errors:
                    ...
                    if geometry_type == "Twin Grain with Defect":
                        phi, eta1, eta2 = geom_viz.create_defect_geometry(
                            ...
                            grain_boundary_pos, 3.0, buffer_width, gb_profile.lower(), gb_curvature
                        )
                    else:
                        phi, eta1, eta2 = geom_viz.create_twin_grain_geometry(
                            twin_spacing, grain_boundary_pos, 3.0,
                            buffer_width, gb_profile.lower(), gb_curvature
                        )
