...
# ███ + PLASTICITY PARAMETER INTELLIGENT RECOMMENDER v8.7.0              ███
...
# ███   · Prompt: explicit "MUST output gamma0_dot as separate object"  ███
# ███   · Contextual heuristic: "strain rate" + s^-1 unit near number   ███
# ███   · Guaranteed fallback: inject user-supplied strain_rate         ███
# ███   · Extended alias list: deformation/loading rate variants        ███
...
# ███   · plot_candidate_scores() now accepts bar_colormap, color_by    ███
# ███     ('score' | 'index' | 'uniform'), best_color_override,         ███
# ███     cmap_vmin / cmap_vmax, and show_colorbar.                     ███
# ███   · Adaptive text halo — value labels auto-flip between black on  ███
# ███     light bars and white on dark bars using WCAG-style luminance. ███
# ███   · 60+ curated publication-friendly colormaps across 5 categories.███
# ███   · Live colormap preview + 5 one-click presets.                  ███
# ███   · Preset buttons use a safe *pending-preset* mechanism so they  ███
# ███     never trigger StreamlitAPIException on widget-key mutation.   ███
# ███ NEW (v8.7.0): BAR-CHART TYPOGRAPHY CUSTOMIZATION                 ████
# ███   · Four independent typography knobs promoted to first-class     ███
# ███     arguments all the way from the widget to the matplotlib       ███
# ███     artist: annotation_fontsize, legend_fontsize,                 ███
# ███     colorbar_label_fontsize, colorbar_tick_fontsize.              ███
# ███   · Fixes the old bug where the "Annotation size" slider was      ███
# ███     wired to label_pad (a POINT OFFSET) instead of a font size,   ███
# ███     so the slider never actually resized the text.                ███
# ███   · New Annotation-offset slider does the position job.           ███
# ███   · Auto-raised y-axis head-room via _compute_min_safe_headroom() ███
# ███     so a 20 pt label can never collide with the top spine.        ███
# ███   · Colorbar label/tick sliders auto-disable when color_by is not  ███
# ███     'score', since no colorbar is drawn in that mode.             ███
# ============================================================================
...
from matplotlib.cm import ScalarMappable             # v8.6.0: colorbar for score->color map
from matplotlib.colors import Normalize, to_rgb      # v8.6.0: ditto + hex->rgb helper
...
# v8.6.0: curated, categorised bar-chart colormaps for the recommender visuals.
# Sequential = score gradients; Diverging = low↔high; Cyclic = perceptual;
# Categorical = distinct hues; Classic = greyscale & legacy.
BAR_CMAP_CATEGORIES = {
...
#
# v8.6.0 colormap engine — the bar chart is no longer locked to grey:
#   · color_by='score'   → bar color encodes the LatentMoE score as a gradient
#   · color_by='index'   → each bar gets a distinct color from the colormap
#   · color_by='uniform' → legacy grey (backward-compatible)
#   · best_color_override forces a specific hex for the winner; if None, the
#     auto path picks a high-contrast color by sampling the opposite end of
#     the colormap (score mode) or Okabe-Ito blue (index/uniform modes).
#   · Value labels automatically flip between black and white based on the
#     bar's relative luminance (WCAG-style: 0.299R + 0.587G + 0.114B).
#   · Optional colorbar legend shows the score→color mapping.
#
# v8.7.0 typography — the "Annotation size" slider was previously wired to
# `label_pad` (a POINT OFFSET), so it only nudged the label up/down without
# ever resizing it. The bar chart now exposes four independent typography
# controls (annotation, legend, colorbar label, colorbar tick) as
# first-class arguments all the way from the widget to the matplotlib
# artist, plus a separate annotation OFFSET control that does exactly what
# its name says. Y-axis head-room is auto-raised when the annotation font
# grows so a 20 pt label can never collide with the top spine.
# ============================================================================

# ---- provenance -> marker SHAPE (independent of colour => colour-blind safe)
PROVENANCE_MARKERS = {
    'llm':       dict(marker='D', ms=6.0, label='LLM-inferred'),
    'heuristic': dict(marker='^', ms=7.5, label='Heuristic fallback'),
}


def _norm_provenance(p):
    p = str(p).strip().lower()
    return 'llm' if any(k in p for k in ('llm', 'inferred', 'model', 'ai')) else 'heuristic'


# ---- 1.0e+13  ->  $1.0\times10^{13}$ ---------------------------------------
def sci_tex(value, unit=None, decimals=1):
    r"""sci_tex(2.5e13, 'm^{-2}') -> r'$2.5\times10^{13}\,\mathrm{m^{-2}}$'
    Values with 0.1 <= |v| < 10^4 render as plain decimals."""
    v = float(value)
    if v == 0:
        s = '0'
    else:
...
PARAM_META = {
    ...
    'mu':             dict(title='Shear modulus',                    symbol=r'\mu(T)',                unit='GPa'),
...
# ----------------------------------------------------------------------------
# v8.6.0: LUMINANCE-AWARE TEXT-HALO HELPER
# ----------------------------------------------------------------------------
def _color_to_rgb(c: Any) -> Tuple[float, float, float]:
    """Best-effort conversion of a matplotlib color (RGBA tuple, hex string,
    or named color) to an (R, G, B) tuple in [0,1]. Returns a mid-grey on
    failure — this is a best-effort readability helper, never a hard error."""
    if c is None:
        return (0.7, 0.7, 0.7)
    # RGBA / RGB tuple or numpy array
    ...
    if isinstance(c, str):
        try:
            ...
            pass
    # Last-resort: let matplotlib try
    try:
        return to_rgb(str(c))
    except Exception:
        return (0.7, 0.7, 0.7)
...
def _relative_luminance(rgb: Tuple[float, float, float]) -> float:
    """WCAG-style relative luminance (sRGB, gamma-uncompensated for speed).
    Bars are already visually flat-coloured so the simple weighted sum is
    adequate for choosing a label colour, and it is far cheaper than the
    full linearisation."""
    r, g, b = rgb
...
# ----------------------------------------------------------------------------
# v8.7.0: BAR-CHART TYPOGRAPHY RESOLUTION HELPERS
# ----------------------------------------------------------------------------
def _resolve_bar_typography(style: Dict[str, Any],
    ...
    """Resolve the four typography knobs against the journal style dict.

    Any ``None`` falls back to the journal preset's ``font_size_small``
    (which is the historical hard-coded value).  The colorbar tick size
    defaults to *one point smaller than the label* so the two stay visually
    coupled when the user only tweaks the label.
    """
...
def _compute_min_safe_headroom(fig_height_in: float,
    ...
    """Return the smallest y-axis head-room factor that still guarantees the
    top value label clears the top spine.

    The clearance consumed above the tallest bar (in points) is
        label_pad + 1.5 * fontsize
    (the 1.5 accounts for the round-padded text bbox plus descender slack).
    Converted to a fraction of the axes height, we solve
        headroom * (1 - clearance_frac) >= 1
    for headroom.
    """
    clearance_pt = float(label_pad_pt) + 1.5 * float(annotation_fontsize_pt)
    axes_pt = max(1.0, axes_fraction * float(fig_height_in) * 72.0)
    clearance_frac = min(0.85, clearance_pt / axes_pt)   # never allow /0
    return 1.0 / max(1e-6, 1.0 - clearance_frac)


# ----------------------------------------------------------------------------
# v8.5.0 / v8.5.1 / v8.6.0 / v8.7.0: plot_candidate_scores
# ----------------------------------------------------------------------------
def plot_candidate_scores(candidates, scores, provenance, best_idx=None, *,
    ...
    """
    label_pad : padding (POINTS) between bar top and its value label
    headroom  : y-axis head-room factor so labels clear the top spine
    best_idx  : index into the ORIGINAL `candidates` list
    score_std : optional per-candidate score uncertainty (e.g. per-expert
                disagreement from the MoE) — rendered as error bars

    v8.6.0 colormap arguments
    -------------------------
    bar_colormap : str | matplotlib.colors.Colormap
        Any matplotlib colormap name or object.
    color_by : {'score', 'index', 'uniform'}
        'score'   → bar colour encodes LatentMoE score (gradient)
        'index'   → each bar gets a distinct colour from the colormap
        'uniform' → legacy grey (#BBBBBB)
    best_color_override : str | None
        Hex/named colour for the best-match bar. When None, auto-derived:
          · score mode    → the OPPOSITE end of the colormap (high contrast)
          · index/uniform → Okabe–Ito blue (#0072B2)
    cmap_vmin, cmap_vmax : float | None
        Manual normalisation range for score colouring. None → auto.
    show_colorbar : bool
        Add a score→colour legend (only meaningful in score mode).

    v8.7.0 typography arguments
    ---------------------------
    annotation_fontsize, legend_fontsize,
    colorbar_label_fontsize, colorbar_tick_fontsize : float | None
        Font sizes (in POINTS) for the four text artists that used to be
        locked to the journal preset's ``font_size_small``.  ``None`` →
        journal default (backward compatible).
    colorbar_label : str
        Text drawn next to the colorbar (default 'Score').
    """
    n = len(candidates)
    ...
    provs = [_norm_provenance(provenance[i]) for i in order]
    ...
    # ── v8.7.0: resolve typography BEFORE building any artists ──────────
    ...
    # ── v8.7.0: auto-expand head-room when the annotation font is large ─
    # Without this, a 16–20 pt label would crowd / overlap the top spine
    # even at the journal default head-room of 1.22.
    ...
    # make mathtext match the surrounding typeface (STIX ~ Times-like)
    mpl.rcParams['mathtext.fontset'] = (
        'stix' if 'times' in style['font_family'].lower() else 'dejavusans')

    C_EDGE = 'black'                                   # uniform bar outline
    ...
    if isinstance(bar_colormap, str):
        ...
        cmap = bar_colormap                             # assume Colormap-like

    # ── Compute per-bar face colours ────────────────────────────────────
    ...
    if color_by == 'score':
        ...
        # Categorical: sample N evenly-spaced colours from the colormap
        indices = np.linspace(0.05, 0.95, max(n, 1))
        faces = [cmap(float(i)) for i in indices]
    else:  # 'uniform' → legacy grey
        faces = ['#BBBBBB'] * n
    ...
    if best_x is not None:
        if best_color_override:
            ...
            if color_by == 'score':
                # Sample the OPPOSITE end of the colormap → maximal contrast
                # against the score-gradient baseline.
                faces[best_x] = cmap(1.0 - norm_scores[best_x])
            else:
                # Okabe–Ito blue is distinguishable on any categorical palette
                faces[best_x] = '#0072B2'

    # ── Draw bars ───────────────────────────────────────────────────────
    ax.bar(xs, scs, width=bar_width, facecolor=faces,
           edgecolor=C_EDGE, linewidth=0.8, zorder=3)

    # ── Bonus: per-expert score disagreement as uncertainty ─────────────
    if stds is not None:
        ax.errorbar(xs, scs, yerr=stds, fmt='none', ecolor='black',
                    elinewidth=0.8, capsize=3, zorder=5)

    # ── Provenance markers (colour-independent → colour-blind safe) ─────
    ...
    # ── Value labels: offset-in-points, adaptive halo, configurable size ─
    # v8.7.0: `fontsize=ann_fs` (was style['font_size_small']).
    ...
    # ── Head-room: no label ever touches the top spine ──────────────────
    ax.set_ylim(0, max(scs) * effective_headroom)          # ← v8.7.0
    ...
    # ── Optional colorbar (score mode only) ─────────────────────────────
    # v8.7.0: label and tick sizes are user-configurable.
    if show_colorbar and color_by == 'score':
        sm = ScalarMappable(cmap=cmap, norm=Normalize(vmin=vmin, vmax=vmax))
        sm.set_array([])
        cbar = fig.colorbar(sm, ax=ax, fraction=0.04, pad=0.02,
                            orientation='vertical')
        cbar.set_label(colorbar_label, fontsize=cbar_fs)     # ← v8.7.0
        cbar.ax.tick_params(labelsize=ctick_fs)              # ← v8.7.0

    # ── Legend from PROXY ARTISTS: shapes + best-match patch ────────────
    # v8.7.0: font size configurable; anchor & spacing also parametrized.
    ...
    ax.legend(handles=handles, loc='lower left',
              ...
              fontsize=leg_fs,                            # ← v8.7.0
              columnspacing=legend_columnspacing,
              handletextpad=legend_handletextpad,
              borderaxespad=legend_borderaxespad)

    if title:
        # Title pad scales with the legend font so a large legend still clears.
...
# ----------------------------------------------------------------------------
# STREAMLIT WIRING — screen render + hi-res download. Always savefig with
# bbox_inches='tight' + pad_inches so the above-axes provenance legend is
# included in the exported PNG.
# ----------------------------------------------------------------------------
@handle_errors
def render_candidate_score_chart(param_key, candidates, scores, provenance,
                                 ...
                                 # ── v8.7.0: typography pass-through ──
                                 annotation_fontsize=None,
                                 legend_fontsize=None,
                                 colorbar_label_fontsize=None,
                                 colorbar_tick_fontsize=None,
                                 annotation_offset=None,
                                 # ── v8.6.0: colormap pass-through ───
                                 bar_colormap='viridis',
                                 color_by='score',
                                 best_color_override=None,
                                 cmap_vmin=None,
                                 cmap_vmax=None,
                                 show_colorbar=False):
    """Publication-quality candidate-score chart for the dashboard.

    v8.7.0: every typography and colormap knob is now forwarded to
    ``plot_candidate_scores``.  Previously this wrapper only forwarded the
    four mandatory arguments, making the entire v8.6.0 colormap engine and
    the journal font sizes dead code from the UI's perspective.
    """
    ...
    # v8.5.1: normalize at the source so nothing downstream can propagate
    # a doubled backslash into a mathtext f-string.
    ...
    # v8.7.0: `label_pad` is now decoupled from font size.  When the caller
    # does not supply an explicit offset we keep the historical default.
...
# Quick preview reproducing the original rho0.png (standalone use only):
#   fig = plot_candidate_scores(
#       candidates=[5e12, 1.0e13, 2.0e13, 5.0e13, 1.0e14],
#       scores=[0.612, 0.820, 0.758, 0.694, 0.577],
...
#       param_title='Initial dislocation density',
#       param_symbol=r'\rho_0', unit='m^{-2}',
#       bar_colormap='plasma', color_by='score',
#       show_colorbar=True)
#   fig.savefig('rho0_scores_plasma.png', dpi=600,
#               bbox_inches='tight', pad_inches=0.05)
#
# v8.7.0 typography preview:
#   fig = plot_candidate_scores(
#       candidates=[5e12, 1.0e13, 2.0e13, 5.0e13, 1.0e14],
#       scores=[0.612, 0.820, 0.758, 0.694, 0.577],
#       provenance=['llm', 'llm', 'heuristic', 'heuristic', 'llm'],
...
#       bar_colormap='plasma', color_by='score',
#       show_colorbar=True,
#       annotation_fontsize=14.0, legend_fontsize=10.0,
#       colorbar_label_fontsize=11.0, colorbar_tick_fontsize=9.0)
#   fig.savefig('rho0_scores_typo.png', dpi=600,
#               bbox_inches='tight', pad_inches=0.05)
#
...
# ███  heuristic fallback + value coercion + UI debug toggle            ██████
# ███  v8.4.0: gamma0_dot — prompt disambiguation + contextual          ██████
# ███          heuristic + guaranteed fallback injection                ██████
# ███  v8.6.0: colormap-driven bar chart (score | index | uniform)      ██████
# ███  v8.7.0: full typography customization for the bar chart          ██████
# ███████████████████████████████████████████████████████████████████████████
# ============================================================================
...
PLASTICITY_ONTOLOGY: Dict[str, Dict[str, Any]] = {
    ...
    "gamma0_dot": {
        ...
        "aliases": [
            ...
            "reference shear rate", "reference shear strain rate",
            "gamma0_dot", "gamma_dot_0", "gamma_0_dot", "γ̇₀", "γ0",
            "gamma dot 0", "gamma-dot-0",
...
_PARAM_ALIASES: Dict[str, str] = {
    ...
    "gamma dot 0": "gamma0_dot",
    "gamma-dot-0": "gamma0_dot",
    "gamma_0": "gamma0_dot",
    "reference_rate": "gamma0_dot",
...
REASONING CHAINS (use these formulas, do NOT just look up a constant):
  ...
  mu (T, material, phase state):
    ...
    Step 2: If the text supplies single-crystal elastic constants, compute
            the Voigt-Reuss-Hill (VRH) polycrystalline shear modulus and
            skip directly to Step 5.

            Cubic symmetry (needs C11, C12, C44):
              G_V    = (C11 - C12 + 3*C44) / 5
              S11    = (C11 + C12) / [(C11 - C12)*(C11 + 2*C12)]
              S12    = -C12 / [(C11 - C12)*(C11 + 2*C12)]
              S44    = 1 / C44
              1/G_R  = (4/5)*(S11 - S12) + (3/5)*S44
              G_VRH  = (G_V + G_R) / 2

            Hexagonal symmetry (needs C11, C12, C13, C33, C44):
              C66    = (C11 - C12) / 2
              G_V    = (1/15)*(2*C11 + C33 - C12 - 2*C13
                               + 6*C44 + 3*C66)
              [S]    = inverse of the 6x6 stiffness matrix
              1/G_R  = (1/15)*(8*S11 + 4*S33 - 4*S12 - 8*S13
                               + 6*S44 + 3*S66)
              G_VRH  = (G_V + G_R) / 2

            If only two of the three cubic constants are given, do NOT
            substitute a default for the missing one -- fall through to
            Step 3 instead.

    Step 3: If no elastic constants are given, use these room-T
            polycrystalline VRH shear-modulus baselines (GPa):
              FCC:   Cu 48,  Ag 30,  Au 27,  Al 26,  Ni 76,
                     Pd 44,  Pt 61,  Pb 6,   gamma-Fe 82, Rh 150, Ir 224
              BCC:   alpha-Fe 82, W 161, Mo 120, Cr 115, V 47,
                     Nb 38,  Ta 69,  beta-Ti 37
              HCP:   Mg 17,  Ti 44,  Zn 43,  Zr 33,  Co 75,
                     Cd 19,  Be 132, Ru 173, Hf 56
              HEA:   CoCrFeNi 80, CoCrFeMnNi 78, AlCoCrFeNi 85,
                     Al0.3CoCrFeNi 88
              Amorphous / metallic glass:
                     approx 30-40 % of the crystalline counterpart
                     (e.g. Cu50Zr50 MG ~= 33 GPa)

    Step 4: Temperature correction. Use the Koster-Varshni softening form
            whenever parameters are available:
              G(T) = G_0 - delta_G * exp(-T_0 / T)
            Default parameters for common metals (G_0 and delta_G in GPa,
            T_0 in K) -- calibrated so G(300 K) matches the Step-3 baseline:
              Cu:  G_0 = 55.0, delta_G = 36.0, T_0 = 490   -> G(300)=48.0
              Al:  G_0 = 31.5, delta_G = 16.0, T_0 = 400   -> G(300)=26.3
              Ni:  G_0 = 90.0, delta_G = 40.0, T_0 = 480   -> G(300)=76.1
              Fe:  G_0 = 95.0, delta_G = 40.0, T_0 = 500   -> G(300)=83.1
              Mg:  G_0 = 21.0, delta_G = 12.0, T_0 = 380   -> G(300)=17.2
              Ti:  G_0 = 50.0, delta_G = 20.0, T_0 = 420   -> G(300)=44.9
            If no parameters are available, fall back to the linear form
              G(T) = G_300 * [1 - a*(T - 300)]
            with a ~= 3.0e-4 K^-1 for FCC, 2.5e-4 K^-1 for BCC,
            4.0e-4 K^-1 for HCP. This is valid up to T ~= 0.6*T_m.
            Above 0.6*T_m, cap the softening at G(T_m) ~= 0.35*G_300.

    Step 5: Multiplicative defect / alloying corrections -- apply ONLY
            the corrections that are explicitly supported by the text.
            Do NOT stack corrections the text does not mention.

            * Solid-solution hardening (substitutional, e.g. Cu-Ni,
              Cu-Zn, Cu-Al, Al-Mg): +3 to +8 % per 5 at.% solute.

            * Precipitation strengthening (rule of mixtures):
                  G_eff = G_matrix*(1 - f) + G_precip*f
              with G_precip ~= 1.2-2.0 x G_matrix depending on phase.

            * Cold work / irradiation hardening: +2 to +5 %.

            * Dilute spherical porosity phi
              (Hashin-Shtrikman upper bound):
                  G_eff = G_0 * exp(-3.5 * phi)

            * Nanograin softening for grain size d < 50 nm:
                  G_eff ~= G_0 * [1 - (d_0 / d)],  d_0 ~= 5-10 nm

    Step 6: Report the final value in Pa. In the `reasoning` field, write
            a 4-6 step numbered chain that names each formula used
            (VRH, Koster-Varshni, rule of mixtures, ...) and lists the
            numerical inputs at every step. If Path A (explicit
            extraction) applies, skip Steps 2-5 and copy the value
            verbatim.
  ...
  gamma0_dot (T, strain_rate, material):
    🚨 IMPORTANT — DO NOT OMIT THIS PARAMETER 🚨
    ...
    Never assume "the value is already known from context, so I can skip
    emitting it." The caller parses the JSON array, not the context block.

    Step 1: Check whether the text explicitly states a reference strain
            rate (typically written as "reference strain rate",
            "reference shear rate", or "γ̇₀"). If yes, extract it (Path A).
    Step 2: If not, infer from the physical regime:
            - Quasi-static loading (default): 1e-3 s^-1
            - Dynamic / shock loading: 1e3 to 1e6 s^-1
            - Creep / very slow: 1e-8 to 1e-6 s^-1
            - Molecular dynamics (MD) simulations: typically 1e6 to 1e8 s^-1
    Step 3: If the text gives no regime hint, default to 1e-3 s^-1
            (standard quasi-static rate for the constitutive law).
    Step 4: Emit ONE JSON object with:
            param="gamma0_dot", value=<number>, unit="s^-1",
            method="LLM_inferred" (or "explicit" if Path A),
            reasoning="<your 3-5 step chain>".
    Step 5: Output value in s^-1. NEVER omit this parameter.
...
CRITICAL RULES:
- Return ALL FIVE parameters for every document, including gamma0_dot.
...
# ----------------------------------------------------------------------------
# LLM + HEURISTIC EXTRACTION (DUAL-MODE + GATEKEEPER FIX + v8.4.0)
# ----------------------------------------------------------------------------
class PlasticityParameterExtractor:
    ...
    v8.4.0 additions:
      · Contextual heuristic for gamma0_dot: "strain rate" / "deformation
        rate" / "loading rate" near a number with s^-1 unit is treated as
        a low-confidence gamma0_dot candidate.
      · Guaranteed fallback injection: if gamma0_dot is STILL missing after
        LLM + heuristic merge, it is injected using the user-provided
        strain_rate with method="default_fallback" and confidence=0.3.
      · Extended gamma0_dot alias list (space/hyphen variants, MD-scale
        aliases like "pre-exponential factor").
    """
    ...
    # v8.4.0: contextual alias regex for gamma0_dot detection. Matches
    # "strain rate" / "deformation rate" / "loading rate" adjacent to a
    # number with an s^-1 or /s unit. Requires the unit to avoid false
    # positives from plain "strain rate = 0.05" (which is dimensionless).
    _CTX_RATE_RE = re.compile(
        ...
        r"\s*(?:s\^?-?1|/\s*s|per\s+second|s⁻¹|s-1)",
        re.I,
    )
    ...
    def extract(self, text: str, material: str, temp_k: float,
        ...
        # ---- Per-parameter heuristic merge (v8.3.0) ------------------------
        ...
        # ---- v8.4.0: guaranteed gamma0_dot fallback ------------------------
        # If gamma0_dot is STILL missing (both LLM and heuristic failed to
        # surface it — which happens systematically when the LLM reads the
        # context-variable phrasing "use strain_rate as gamma0_dot"), inject
        # a low-confidence default built from the user-supplied strain rate.
        if "gamma0_dot" not in have:
            llm_out.append({
                ...
                "reasoning": (
                    ...
                    "Step 2: Heuristic scan for 'strain rate' near s^-1 unit "
                    "returned no candidate.\n"
                    "Step 3: Falling back to the user-supplied strain rate "
                    f"({strain_rate} s^-1) as the reference rate for the "
                    "constitutive law, since it is the natural choice for "
                    "quasi-static loading.\n"
                    "Step 4: Default value 1e-3 s^-1 applies if user value is "
                    "unavailable."
                ),
            })
            have.add("gamma0_dot")
            logger.info("Injected gamma0_dot default_fallback = %s s^-1",
                        strain_rate)
        # --------------------------------------------------------------------
    ...
    def _heuristic(cls, text: str, material: str, temp_k: float,
        ...
        # ---- v8.4.0: contextual heuristic for gamma0_dot -------------------
                ...
                if sci:
                    try:
                        ...
                        out.append({
                            ...
                            "reasoning": (
                                f"Contextual heuristic: matched '{m.group(0)[:60]}' "
                                f"→ value {value:.3e} s^-1 as candidate "
                                f"reference strain rate."
                            ),
                        })
...
class PlasticityLatentMoEScorer:
    ...
    def _reasoning_expert(reasoning: str, method: str) -> float:
        ...
        # v8.4.0: treat default_fallback like llm_inferred (both are gaps
        # that were filled by reasoning, just from different sources).
        ...
        for doc in corpus[:max_docs]:
            ...
            for ext in extractions:
                ...
                if p not in PLASTICITY_ONTOLOGY:
                    continue
                # v8.4.0: skip default_fallback in prior statistics — priors
                # are supposed to reflect literature, not defaults.
...
def render_plasticity_recommender_sidebar(
    ...
    st.subheader("🤖 Intelligent Plasticity Recommender v8.7.0")
    st.caption(
        "FAISS + SentenceTransformer retrieval · Ollama NER · LatentMoE scoring · "
        "dual-mode prompts (strict priors vs reasoned inference) · "
        "param-alias canonicalization · Chain-of-Thought surfaced in UI · "
        "**v8.4.0: guaranteed gamma0_dot + contextual heuristic** · "
        "**v8.6.0: colormap-driven bar chart** · "
        "**v8.7.0: full typography control**."
    )
    ...
    if not bundle.priors_df.empty:
        ...
        with st.container():
            st.caption(
                ...
                "learning, and default_fallback values are excluded."
            )
...
# ███  v8.6.0: full colormap control for the LatentMoE bar chart        ██████
# ███  v8.7.0: full typography control for the LatentMoE bar chart      ██████
# ███████████████████████████████████████████████████████████████████████████
# ============================================================================
...
class RecommenderVisualStyle:
    ...
    # ── v8.6.0: bar-chart-specific defaults ─────────────────────────────
    bar_colormap: str = 'viridis'
    bar_color_mode: str = 'score'          # 'score' | 'index' | 'uniform'
    bar_best_color: Optional[str] = None   # None = auto-contrast
    bar_show_colorbar: bool = False

    # ── v8.7.0: bar-chart typography ────────────────────────────────────
    # None → fall back to the journal preset's font_size_small
    ...
    bar_annotation_offset: float = 9.0     # points (data-independent)
    bar_headroom: float = 1.22             # min y-axis head-room factor
...
# ----------------------------------------------------------------------------
# BAR CHART — publication quality (v8.5.0 rebuild on plot_candidate_scores)
# v8.6.0: full colormap control panel + one-click presets.
# v8.7.0: full typography control panel.
# ----------------------------------------------------------------------------
# Implementation notes on the preset buttons:
#   Streamlit forbids writing to a widget's session_state key AFTER the
#   widget has been instantiated in the current run. The preset buttons
#   therefore stash the desired (colormap, color-mode) pair under a
#   dedicated 'pending' key and call st.rerun(). On the next pass,
#   _apply_pending_bar_preset() runs BEFORE the selectbox/radio are
#   created, so we can legally seed their keys and have the widgets
#   render with the preset already selected.
# ----------------------------------------------------------------------------
...
def _apply_pending_bar_preset() -> None:
    ...
    Must be called at the very top of render_recommender_bars_pub()."""
...
def render_recommender_bars_pub(bundle: PlasticityRecommendationBundle,
    ...
    v8.7.0 fixes the bug where the "Annotation size" slider was wired to
    `label_pad` (a POINT OFFSET) instead of a font size, so the slider only
    moved the label up/down without ever resizing it.  The bar chart now
    exposes four independent typography controls — annotation font size,
    legend font size, colorbar label size, colorbar tick size — plus a
    separate annotation OFFSET control that does exactly what the name says.
    """

    # ── v8.6.0: apply any queued one-click preset BEFORE widgets are made ─
    ...
    # ── v8.6.0: colormap control panel ──────────────────────────────────
    with st.expander("🎨 Colormap & Coloring Options", expanded=True):
        col_a, col_b, col_c = st.columns(3)

        with col_a:
            all_cmap_names: List[str] = list(BAR_CMAP_FLAT)
            preferred = (style.bar_colormap
                         if style.bar_colormap in all_cmap_names
                         else style.colormap
                         if style.colormap in all_cmap_names
                         else 'viridis')
        ...
        with col_b:
            color_mode = st.radio(
                ...
                format_func=lambda x: {
                    ...
                    'uniform': '⬜ Uniform (legacy grey)',
                }[x],
                index=(
                    ['score', 'index', 'uniform'].index(style.bar_color_mode)
            ...
            # v8.7.0: the colorbar is intentionally score-only, so disable
            # the checkbox and explain the situation when the user is in
            # index/uniform mode.
            ...
            show_cbar = st.checkbox(
                ...
                key="rec_bar_show_cbar",
                disabled=not colorbar_available,
                help=("Adds a colourbar showing the score→colour mapping. "
                      "Only available in 'score' colour mode — a score "
                      "colorbar would be meaningless when bars are coloured "
                      "by position or uniformly.")
                if colorbar_available else
                "⚠️ Colorbar is only available when 'Color bars by' = score.",
            )
            if not colorbar_available:
                # Streamlit won't let us mutate `show_cbar` in place, but we
                # can force the effective value we pass downstream.
                show_cbar = False
        ...
        with col_c:
            ...
            best_color_name = st.selectbox(
                "Best-match bar colour",
                list(best_color_options.keys()),
                index=default_best_idx,
                key="rec_bar_best_color",
                help="Forces the winner bar to a specific accent colour. "
                     "'Auto' picks the opposite end of the colormap in score "
                     "mode (maximal contrast) or Okabe–Ito blue otherwise.",
            )
            ...
            if color_mode == 'score':
                with st.expander("⚙️ Advanced normalisation", expanded=False):
                    use_auto_vmin = st.checkbox(
                        ...
                        help="Uncheck to pin the score range used by the "
                             "colormap (useful for comparing runs across "
                             "sessions).",
                    )
                    if use_auto_vmin:
                        cmap_vmin, cmap_vmax = None, None
                        st.caption("Using min(score) … max(score) across "
                                   "candidates.")
                    else:
                        c1, c2 = st.columns(2)
                        with c1:
                        ...
                        if cmap_vmax <= cmap_vmin:
                            st.warning("vmax must exceed vmin — falling back "
                                       "to auto range.")
                            cmap_vmin, cmap_vmax = None, None
            else:
                cmap_vmin, cmap_vmax = None, None

    # ── v8.7.0: typography control panel ────────────────────────────────
    # Four independent font-size sliders + one offset slider.  Each slider
    # feeds a distinct kwarg on `plot_candidate_scores`, so every control
    # now does exactly what its label says.
    with st.expander("✍️ Typography & Labels", expanded=True):
        t_col1, t_col2 = st.columns(2)

        # Resolve defaults from the style dataclass, then the journal preset.
        ...
        with t_col1:
            ann_fs = st.slider(
                ...
                step=0.5,
                key="rec_bar_ann_fs",
                help="Size of the '0.820'-style value labels above each bar. "
                     "This replaces the old behaviour where the slider was "
                     "wired to the label OFFSET instead of its size.",
            )
            leg_fs = st.slider(
                "Legend font size (pt)",
                min_value=4.0, max_value=24.0,
                ...
                step=0.5,
                key="rec_bar_leg_fs",
                help="Size of the legend entries (LLM-inferred / Heuristic / "
                     "Best match) drawn above the axes.",
            )
            ann_offset = st.slider(
                ...
                step=0.5,
                key="rec_bar_ann_offset",
                help="Gap in POINTS between the top of each bar and its value "
                     "label.  This is a POSITION, not a size — for the size "
                     "use 'Annotation font size' above.",
            )

        with t_col2:
            cbar_fs = st.slider(
                "Colorbar label size (pt)",
                min_value=4.0, max_value=24.0,
                ...
                help=("Font size of the colorbar's axis label.  Inactive in "
                      "'index' / 'uniform' colour modes because the colorbar "
                      "is not drawn there.")
                if color_mode == 'score' else
                "⚠️ Only active in 'score' colour mode.",
            )
            ctick_fs = st.slider(
                "Colorbar tick size (pt)",
                min_value=4.0, max_value=24.0,
                value=float(style.bar_colorbar_tick_fontsize
                            if style.bar_colorbar_tick_fontsize is not None
                            else max(_base_small - 1.0, 4.0)),
                step=0.5,
                key="rec_bar_ctick_fs",
                disabled=(color_mode != 'score'),
                help=("Font size of the numeric ticks on the colorbar.  "
                      "Inactive in 'index' / 'uniform' colour modes.")
                if color_mode == 'score' else
                "⚠️ Only active in 'score' colour mode.",
            )
            show_offset_hint = st.checkbox(
                "Preview head-room advisory",
                value=False,
                key="rec_bar_headroom_hint",
                help="Shows the minimum safe y-axis head-room factor implied "
                     "by the current annotation size + offset.",
            )
            if show_offset_hint:
                # Mirror of _compute_min_safe_headroom(), surfaced for the UI.
                ...
                st.caption(
                    f"Minimum safe head-room factor: **{_min_safe:.3f}** "
                    f"(auto-applied if it exceeds the default "
                    f"{style.bar_headroom:.2f})."
                )

    # ── v8.6.0: one-click presets ───────────────────────────────────────
    ...
    fig = plot_candidate_scores(
        ...
        best_idx=0,                    # candidates are already score-sorted
        ...
        # v8.7.0: offset is now a SEPARATE widget from the font size.
        label_pad=float(ann_offset),
        headroom=float(style.bar_headroom),
        despine=True,
        # ── v8.6.0 colormap arguments ───────────────────────────────────
        bar_colormap=chosen_cmap,
        color_by=color_mode,
        best_color_override=best_color_override,
        cmap_vmin=cmap_vmin,
        cmap_vmax=cmap_vmax,
        ...
        # ── v8.7.0 typography arguments ─────────────────────────────────
        annotation_fontsize=float(ann_fs),
        legend_fontsize=float(leg_fs),
        colorbar_label_fontsize=float(cbar_fs),
...
def render_recommender_style_controls() -> RecommenderVisualStyle:
    ...
    # ── v8.6.0 → v8.7.0: bar-chart defaults (persisted to the dataclass) ─
    with st.expander("📊 Bar-Chart Defaults (v8.6.0 → v8.7.0)", expanded=False):
        st.caption(
            "Set the bar chart's default colormap, colour-mapping mode, and "
            "typography.  All values can still be overridden per-render "
            "inside the **Bar Chart** tab."
        )
        c1, c2 = st.columns(2)
        with c1:
            style.bar_colormap = st.selectbox(
                "Default bar colormap",
                BAR_CMAP_FLAT,
        ...
        style.bar_show_colorbar = st.checkbox(
            ...
            value=style.bar_show_colorbar,
            key="rec_style_bar_cbar",
        )

        # ── v8.7.0: typography defaults ─────────────────────────────────
        st.markdown("**✍️ Bar-chart typography defaults**")
        st.caption(
            "Leave any slider at 0 to fall back to the journal preset's "
            "smallest font size (Nature ≈ 7 pt, Science ≈ 8 pt)."
        )
        ...
        with tc1:
            ...
            _ann_new = st.slider(
                ...
                value=float(_ann_default), step=0.5,
            ...
            _leg_new = st.slider(
                ...
                min_value=0.0, max_value=24.0,
                value=float(_leg_default), step=0.5,
            ...
            style.bar_annotation_offset = st.slider(
                "Annotation offset (pt)",
                min_value=0.0, max_value=30.0,
                value=float(style.bar_annotation_offset), step=0.5,
                key="rec_style_bar_ann_offset",
                help="Gap between bar top and value label, in POINTS.",
            )
        with tc2:
            ...
            _cbfs_new = st.slider(
                ...
                min_value=0.0, max_value=24.0,
                value=float(_cbfs_default), step=0.5,
            ...
            _ctick_new = st.slider(
                ...
                min_value=0.0, max_value=24.0,
                value=float(_ctick_default), step=0.5,
            ...
            style.bar_headroom = st.slider(
                "Minimum head-room factor",
                min_value=1.05, max_value=1.80,
                value=float(style.bar_headroom), step=0.01,
                key="rec_style_bar_headroom",
                help="Minimum y-axis head-room above the tallest bar. "
                     "Automatically raised if the annotation font is large.",
            )
...
def render_recommender_visuals_dashboard():
    ...
    st.caption(
        ...
        "**v8.6.0** adds full colormap control to the bar chart; "
        "**v8.7.0** adds full typography control."
    )
...
def main():
    ...
    <strong>✅ PURE FFT SPECTRAL + AI PLASTICITY RECOMMENDER v8.7.0:</strong><br>
    ...
    • <span style="color: green;">🧠 CHAIN-OF-THOUGHT:</span> auditable reasoning chains for inferred parameters.<br>
    ...
    • <span style="color: green;">🎨 COLORMAP ENGINE (v8.6.0):</span> 60+ colormaps · score/index/uniform modes · adaptive text halo · one-click presets · optional colorbar.<br>
    • <span style="color: green;">✍️ TYPOGRAPHY ENGINE (v8.7.0):</span> independent annotation/legend/colorbar-label/colorbar-tick font sizes + separate annotation offset; auto-raised y-axis head-room keeps labels clear of the top spine.<br>
    ...
    with st.sidebar:
        ...
        with st.expander("🧠 AI Plasticity Recommender v8.7.0", expanded=False):
    ...
    # ========================================================================
    # AI RECOMMENDER VISUALS DASHBOARD
    # ========================================================================
