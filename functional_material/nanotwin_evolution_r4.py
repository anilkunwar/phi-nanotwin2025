...
class EnhancedSpectralSolver:
    ...
    def __init__(self, N, dx, elastic_params):
        ...
        # Green's function components - FIXED DIVISION BY ZERO
        denom = mu_2d * (lambda_2d + 2*mu_2d) * self.k2
        denom[denom < 1e-15] = 1e-15  # Prevent division by zero
    ...
    def solve(self, eigenstrain_xx, eigenstrain_yy, eigenstrain_xy, applied_stress=0):
        """Solve mechanical equilibrium with enhanced error handling"""
        try:
            # Check input shapes
            ...
            if np.max(sigma_eq) > 10e9:  # > 10 GPa - unrealistic for Cu
                st.warning(f"⚠️ Unrealistic stress detected: {np.max(sigma_eq)/1e9:.1f} GPa")
                sigma_eq = np.clip(sigma_eq, 0, 2e9)  # Clip to 2 GPa
            
            return sigma_eq, sxx, syy, sxy, eps_xx, eps_yy, eps_xy
            
        except Exception as e:
            st.error(f"❌ Error in spectral solver: {str(e)}")
            # Return realistic zeros instead of complete failure
            zeros = np.zeros((self.N, self.N))
            # Add some small noise to avoid zero stress everywhere
            sigma_eq = np.random.rand(self.N, self.N) * 10e6  # ~10 MPa random stress
            return sigma_eq, zeros, zeros, zeros, zeros, zeros, zeros
...
# ============================================================================
# COMPREHENSIVE VISUALIZATION AND MONITORING - FIXED VERSION
# ============================================================================
class SimulationMonitor:
    ...
    def create_convergence_plots(history, timesteps):
        """Create convergence monitoring plots with length validation - FIXED"""
        # Ensure timesteps matches history length
        if len(timesteps) != len(history['phi_norm']):
            # If mismatch, generate proper timesteps
            if len(timesteps) > 0:
                # Assuming timesteps are for saved points only
                # Create full timesteps for history
                dt = timesteps[1] - timesteps[0] if len(timesteps) > 1 else 1.0
                full_timesteps = np.arange(len(history['phi_norm'])) * dt
                timesteps = full_timesteps
            else:
                # Generate default timesteps
                timesteps = np.arange(len(history['phi_norm']))
        
        fig, axes = plt.subplots(2, 2, figsize=(12, 10))
        
        # Phi norm evolution - FIXED LENGTH MATCHING
        if len(timesteps) == len(history['phi_norm']):
            axes[0, 0].plot(timesteps, history['phi_norm'], 'b-', linewidth=2)
        else:
            min_len = min(len(timesteps), len(history['phi_norm']))
            axes[0, 0].plot(timesteps[:min_len], history['phi_norm'][:min_len], 'b-', linewidth=2)
        ...
        # Energy evolution
        if len(timesteps) == len(history['energy']):
            axes[0, 1].plot(timesteps, history['energy'], 'r-', linewidth=2)
        else:
            min_len = min(len(timesteps), len(history['energy']))
            axes[0, 1].plot(timesteps[:min_len], history['energy'][:min_len], 'r-', linewidth=2)
        ...
        if len(timesteps) == len(history['max_stress']):
            axes[1, 0].plot(timesteps, np.array(history['max_stress'])/1e9, 'g-', linewidth=2)
        else:
            min_len = min(len(timesteps), len(history['max_stress']))
            axes[1, 0].plot(timesteps[:min_len], np.array(history['max_stress'][:min_len])/1e9, 'g-', linewidth=2)
        ...
        if len(timesteps) == len(history['plastic_work']):
            axes[1, 1].plot(timesteps, history['plastic_work'], 'm-', linewidth=2)
        else:
            min_len = min(len(timesteps), len(history['plastic_work']))
            axes[1, 1].plot(timesteps[:min_len], history['plastic_work'][:min_len], 'm-', linewidth=2)
    ...
    @staticmethod
    def create_plotly_visualizations(results, params):
        ...
        fig = make_subplots(
            rows=3, cols=3,
            subplot_titles=('Twin Order Parameter (3D)', 'Von Mises Stress (3D)',
                          'φ Cross-section', 'σ_eq Cross-section (GPa)',
                          'Twin Spacing (3D)', 'Yield Stress (3D)',
                          'h Cross-section', 'σ_y Cross-section (MPa)', 'Plastic Strain'),
            specs=[[{'is_3d': True}, {'is_3d': True}, {'type': 'scatter'}],
                   [{'is_3d': True}, {'is_3d': True}, {'type': 'scatter'}],
                   [{'type': 'scatter'}, {'type': 'scatter'}, {'type': 'scatter'}]]
        )
        
        # 3D Twin Order Parameter
        x = np.arange(N)
        y = np.arange(N)
        X, Y = np.meshgrid(x, y)
        fig.add_trace(
            go.Surface(
                ...
                colorscale='RdBu_r',
                cmin=-1.2, cmax=1.2,
                colorbar=dict(x=0.31, y=0.85, title="φ"),
                showscale=True
            ),
            row=1, col=1
        )
        
        # 3D Von Mises Stress with dynamic scaling
        stress_data = results['sigma_eq'] / 1e9
        vmax_stress = np.percentile(stress_data, 99) if np.percentile(stress_data, 99) > 0 else 0.1
        fig.add_trace(
            go.Surface(
                z=stress_data,
                x=X, y=Y,
                colorscale='Hot',
                cmin=0, cmax=vmax_stress,
                colorbar=dict(x=0.98, y=0.85, title="GPa"),
                showscale=True
            ),
            row=1, col=2
        )
        ...
        fig.add_trace(
            go.Surface(
                ...
                colorscale='Plasma',
                cmin=0, cmax=30,
                colorbar=dict(x=0.31, y=0.15, title="nm"),
                showscale=True
            ),
            row=2, col=1
        )
        ...
        # 3D Yield Stress
        sigma_y_data = results['sigma_y'] / 1e6
        fig.add_trace(
            go.Surface(
                z=sigma_y_data,
                x=X, y=Y,
                colorscale='Viridis',
                cmin=0, cmax=500,
                colorbar=dict(x=0.98, y=0.15, title="MPa"),
                showscale=True
            ),
            row=2, col=2
        )
        ...
        fig.update_layout(
            height=1000,
            showlegend=True,
            title_text="Interactive 3D & Cross-Section Visualizations",
            scene1=dict(
                xaxis_title='X (grid)',
                yaxis_title='Y (grid)',
                zaxis_title='φ'
    ...
    def create_field_comparison_plot(initial, final, N, dx):
        ...
        # Initial stress with dynamic scaling
        init_stress = initial['sigma_eq']/1e9
        vmax_init = np.percentile(init_stress, 99) if np.percentile(init_stress, 99) > 0 else 0.1
        axes[0, 1].imshow(init_stress, cmap='hot', vmin=0, vmax=vmax_init)
        axes[0, 1].set_title(f'Initial σ_eq (GPa) max={np.max(init_stress):.2f}')
        ...
        # Final stress with dynamic scaling
        final_stress = final['sigma_eq']/1e9
        vmax_final = np.percentile(final_stress, 99) if np.percentile(final_stress, 99) > 0 else 0.1
        axes[1, 1].imshow(final_stress, cmap='hot', vmin=0, vmax=vmax_final)
        axes[1, 1].set_title(f'Final σ_eq (GPa) max={np.max(final_stress):.2f}')
...
# ============================================================================
# STREAMLIT APPLICATION WITH ERROR HANDLING - FIXED VERSION
# ============================================================================
def main():
    st.set_page_config(
    ...
    if 'initialized' in st.session_state and st.session_state.initialized:
            ...
            if st.button("▶️ Start Evolution", type="secondary", use_container_width=True):
                with st.spinner("Running phase-field simulation..."):
                    try:
                        ...
                        # Simulation loop - FIXED: Save timesteps properly
        ...
        with tab3:
            if 'results_history' in st.session_state:
                ...
                # Von Mises stress with dynamic scaling
                stress_data = results['sigma_eq']/1e9
                avg_stress = np.mean(stress_data)
                if avg_stress < 0.1:  # < 0.1 GPa
                    st.warning(f"⚠️ Low stress detected ({avg_stress:.2f} GPa). Check elastic parameters.")
                    vmax_stress = np.percentile(stress_data, 99) if np.percentile(stress_data, 99) > 0 else 0.1
                else:
                    vmax_stress = max(2.0, np.percentile(stress_data, 95))
                
                im_stress = axes[0, 2].imshow(stress_data, cmap='hot', vmin=0, vmax=vmax_stress)
                axes[0, 2].set_title(f'Von Mises Stress (GPa)\nmax={np.max(stress_data):.2f}')
                plt.colorbar(im_stress, ax=axes[0, 2], fraction=0.046, pad=0.04)
                ...
                plotly_fig = SimulationMonitor.create_plotly_visualizations(results, params)
                st.plotly_chart(plotly_fig, use_container_width=True)
                
                # Convergence monitoring - FIXED: Use proper timesteps
                st.subheader("📈 Convergence Monitoring")
                if hasattr(st.session_state.solver, 'history'):
                    # Generate full timesteps for history
                ...
                with col1:
                    st.metric("Avg Stress", f"{np.mean(stress_data):.2f} GPa")
                with col2:
                    st.metric("Max Stress", f"{np.max(stress_data):.2f} GPa")
                with col3:
                    st.metric("Avg Twin Spacing", f"{np.mean(results['h'][(results['h']>5)&(results['h']<50)]):.1f} nm")
                with col4:
