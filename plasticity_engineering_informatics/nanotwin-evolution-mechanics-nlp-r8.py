...
def purge_plasticity_caches() -> List[str]:
    """Delete every known plasticity cache:
        ...
        * in-session corpus cache (Streamlit session_state)
        * in-session extractor cache (if a recommender is currently attached)
        * on-disk FAISS index   (``.plasticity_cache/faiss_index.pkl``)
        * on-disk LLM cache     (``.plasticity_cache/llm_cache.json``)
        * any other ``*.json``/``*.pkl`` files inside ``.plasticity_cache``
          that are not the FAISS index (defensive sweep)
    ...
    if os.path.isdir(cache_dir):
        ...
        # 3c) Defensive sweep of any other *.json / *.pkl files
        try:
...
def render_plasticity_recommender_sidebar(
    ...
    # ----------------------------------------------------------------------
    # "Force reload corpus" → clear EVERY cache (session + FAISS + LLM)
    # ----------------------------------------------------------------------
    if refresh_btn:
        try:
    ...
    if run_btn:
        ...
        # Keep a handle so the Refresh button can purge the in-memory cache.
...
def main():
    ...
    <strong>✅ PURE FFT SPECTRAL + AI PLASTICITY RECOMMENDER v8.1.1:</strong><br>
