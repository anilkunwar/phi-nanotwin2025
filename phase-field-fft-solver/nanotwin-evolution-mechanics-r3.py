...
# ---- Seaborn optional colormaps --------------------------------------------
_SEABORN_NAMES = ('rocket', 'mako', 'crest', 'flare', 'icefire', 'vlag')
_SEABORN_AVAILABLE = False
try:
    import seaborn as sns  # noqa: F401  (import registers the colormaps with mpl)
...
class ColormapRegistry:
    """
    Single source of truth for every colormap name the UI can show,
    and for turning a name into a matplotlib Colormap instance.
    """
    ...
    def list_all(cls) -> List[str]:
        ...
        if _SEABORN_AVAILABLE:
            names.extend(_SEABORN_NAMES)
        names.extend(_CUSTOM_COLORMAPS.keys())
    ...
    @classmethod
    def resolve(cls, name: Union[str, Colormap, None]) -> Colormap:
        """
        Resolve a colormap name to a matplotlib Colormap.
        Warns (rather than silently swallowing) if the name is unknown.
        """
        if name is None:
        ...
        if key in _CUSTOM_COLORMAPS:
            return _CUSTOM_COLORMAPS[key]
        if key in _SEABORN_NAMES and _SEABORN_AVAILABLE:
            try:
                return mpl.colormaps[key]
            except KeyError:
                pass
        logger.warning(f'Colormap "{name}" unavailable — falling back to viridis.')
        try:
            st.warning(f'Colormap "{name}" unavailable — using viridis.')
        except Exception:
            pass
        return mpl.colormaps['viridis']

    @classmethod
    def to_plotly(cls, name: Union[str, Colormap, None], n: int = 64):
        """
        Return a Plotly colorscale for the given matplotlib colormap.
        Uses named shortcuts where possible; otherwise samples n RGB stops.
        """
        if name is None:
            return 'Viridis'
        key = str(name)
        ...
        return [[float(ti), f'rgb({r},{g},{b})'] for ti, (r, g, b) in zip(t, rgb)]
...
PLOTLY_NAMED = {
    ...
    'viridis': 'Viridis', 'sigma_eq_pub': 'Viridis',
    'inferno': 'Inferno', 'magma': 'Magma', 'cividis': 'Cividis',
    'gray': 'Greys', 'gist_gray': 'Greys', 'binary': 'Greys',
    'jet': 'Jet', 'turbo': 'Turbo',
    'YlOrRd': 'YlOrRd', 'Reds': 'Reds', 'Blues': 'Blues',
    'Greens': 'Greens', 'Purples': 'Purples', 'Oranges': 'Oranges',
    'Greys': 'Greys', 'YlGnBu': 'YlGnBu', 'YlGn': 'YlGn',
    'twilight': 'Twilight', 'twilight_shifted': 'Twilight',
    'rainbow': 'Rainbow', 'nipy_spectral': 'Jet',
    'coolwarm_enhanced': 'RdBu', 'stress_map': 'Jet',
}


# ---- Field defaults + resolver ---------------------------------------------
FIELD_DEFAULT_CMAPS: Dict[str, str] = {
    'phi':       'RdBu_r',
    'eta1':      'Reds',
    'sigma_eq':  'hot',
    'sigma_h':   'RdBu',
...
FIELD_LABELS: Dict[str, str] = {
    'phi':       'φ (Twin Order)',
    'eta1':      'η₁ (Grain)',
    'sigma_eq':  'σ_eq (Von Mises)',
    'sigma_h':   'σ_h (Hydrostatic)',
    'h':         'h (Twin Spacing)',
    'eps_p_mag': '|ε_p| (Plastic Strain)',
    'sigma_y':   'σ_y (Yield Stress)',
}

# Sentinel marking "use the global choice"
INHERIT = '__INHERIT__'


def resolve_field_colormaps(global_sel: Optional[Dict[str, str]],
                            sim_sel: Optional[Dict[str, str]] = None) -> Dict[str, str]:
    """
    Precedence: simulation-specific (unless INHERIT/None/'') > global > field default.
    Returns a dict keyed by field name.
    """
    resolved: Dict[str, str] = {}
    for field, default in FIELD_DEFAULT_CMAPS.items():
        pick = (sim_sel or {}).get(field)
        if pick in (None, '', INHERIT):
            pick = (global_sel or {}).get(field)
        if pick in (None, '', INHERIT):
            pick = default
        resolved[field] = pick
    return resolved
...
def build_style_params(resolved: Dict[str, str],
                       extra: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    Produce the style_params dict the visualizer consumes.
    Ships BOTH the canonical `colormaps` mapping and the legacy
    `{field}_cmap` keys, so any old code path sees the selection.
    """
    style: Dict[str, Any] = {'colormaps': dict(resolved)}
    for field, cmap in resolved.items():
        style[f'{field}_cmap'] = cmap
    if extra:
        style.update(extra)
    return style
...
class MetadataManager:
    ...
    def create_metadata(sim_params, history, run_time=None, **kwargs):
        ...
        # Snapshot the *resolved* colormaps (canonical namespace only)
        resolved = sim_params.get('resolved_colormaps')
        if not resolved:
            resolved = resolve_field_colormaps(
                {f: sim_params.get(f'global_cmap_{f}') for f in FIELD_DEFAULT_CMAPS},
                {f: sim_params.get(f'cmap_{f}') for f in FIELD_DEFAULT_CMAPS},
            )
    ...
    def validate_metadata(metadata):
        ...
        # Migrate legacy colormap keys to the canonical namespace
        cmaps = metadata.get('colormaps', {}) or {}
        legacy_map = {
            'cmap_phi': 'phi', 'cmap_stress': 'sigma_eq', 'cmap_hydro': 'sigma_h',
            'cmap_h': 'h', 'cmap_eta1': 'eta1',
        }
        for old_key, new_key in legacy_map.items():
            if old_key in cmaps and new_key not in cmaps:
                cmaps[new_key] = cmaps[old_key]
        for f, default in FIELD_DEFAULT_CMAPS.items():
            cmaps.setdefault(f, default)
        metadata['colormaps'] = cmaps
        return metadata
...
class EnhancedTwinVisualizer:
    ...
    def get_colormap(self, cmap_name):
        ...
        Loud, never silently None; `imshow(..., cmap=None)` cannot happen.
        """
    ...
    def create_multi_field_comparison(self, results_dict, style_params=None):
        ...
        fields_to_plot = [
            ('phi',       'Twin Order Parameter φ', [-1.2, 1.2]),
            ('eta1',      'Grain η₁',                [0, 1]),
            ('sigma_eq',  'Von Mises Stress (GPa)',  None),
            ('sigma_h',   'Hydrostatic Stress (GPa)', None),
            ('h',         'Twin Spacing (nm)',       [0, 30]),
            ('eps_p_mag', 'Plastic Strain',          None),
            ('sigma_y',   'Yield Stress (MPa)',      None),
        ]
        available_fields = [(fname, title, vrange)
                            for fname, title, vrange in fields_to_plot
        ...
        for idx, (field_name, title, vrange) in enumerate(available_fields):
            ax = axes[idx]
    ...
    def create_plotly_heatmap(self, results_dict, field_name, frame_idx=0,
        ...
        fig.update_layout(
            title=f"{title} (Frame {frame_idx}) — cmap: {chosen}",
    ...
    def create_plotly_3d_surface(self, results_dict, field_name, frame_idx=0,
        ...
        fig.update_layout(
            title=f"3D Surface: {title} (Frame {frame_idx}) — cmap: {chosen}",
    ...
    def create_animation(self, history, field_name, output_format='gif', fps=5, dpi=150,
        ...
        im = ax.imshow(first_data, extent=self.extent, cmap=cmap,
        ...
        title = ax.set_title(f"{field_name} — t = 0.000 ns  ({chosen})")
        dt_local = self.dt
        ...
        def update_frame(frame_idx):
            ...
            title.set_text(f"{field_name} — t = {time_ns:.3f} ns  ({chosen})")
            return [im, title]
...
class NanotwinnedCuSolver:
    ...
    def evolve_twin_field(self, sxx, syy, sxy, eps_p_mag):
        ...
        nx = float(n_twin[0]); ny = float(n_twin[1])
    ...
    def step(self):
        try:
            ...
            results = {
                'phi': self.phi.copy(), 'eta1': self.eta1.copy(), 'eta2': self.eta2.copy(),
                'sigma_eq': sigma_eq.copy(), 'sigma_h': sigma_h.copy(),
                'sigma_xx': sxx.copy(), 'sigma_yy': syy.copy(), 'sigma_xy': sxy.copy(),
                'h': h.copy(), 'sigma_y': sigma_y.copy(), 'eps_p_mag': eps_p_mag.copy(),
                'eps_xx': eps_xx.copy(), 'eps_yy': eps_yy.copy(), 'eps_xy': eps_xy.copy(),
                'convergence': {
                    'phi_norm': phi_norm, 'energy': total_energy,
...
class DataExporter:
    ...
    def bulk_export_all_simulations(N, dx, extent):
        ...
        with zipfile.ZipFile(bulk_buffer, "w", compression=zipfile.ZIP_DEFLATED) as zf:
            summary = "MULTI-SIMULATION EXPORT SUMMARY\n"
...
def main():
    ...
    <strong>✅ FFT SPECTRAL + UNIFIED COLORMAP CHAIN:</strong><br>
    ...
    with st.sidebar:
        ...
        if operation_mode == "Run New Simulation":
            ...
            with st.expander("Global Colormaps", expanded=False):
                global_cols = st.columns(2)
                global_sel = render_global_colormap_panel_compact(global_cols)
            with st.expander("Simulation-Specific Colormaps", expanded=False):
                sim_cols = st.columns(2)
                sim_sel = render_simulation_colormap_panel_compact(sim_cols)

            with st.expander("Preview the effective colormaps", expanded=False):
                preview = resolve_field_colormaps(global_sel, sim_sel)
                st.json(preview)
            ...
            if not simulations:
                ...
                comparison_cmap = st.selectbox(
                    ...
                    cmap_list,
                    index=cmap_list.index(FIELD_DEFAULT_CMAPS.get(field_key, 'viridis'))
                          if FIELD_DEFAULT_CMAPS.get(field_key, 'viridis') in cmap_list else 0,
                    key="comparison_cmap")
    ...
    if operation_mode == "Compare Saved Simulations" and 'comparison_config' in st.session_state:
        ...
        if sim_data:
            ...
            with st.expander("🎨 Edit colormaps for this simulation", expanded=False):
                st.caption(
                    ...
                    "Persist them into the saved run by clicking the button."
                )
                edit_cols = st.columns(2)
                edit_cmaps: Dict[str, str] = {}
                for i, (field, default) in enumerate(FIELD_DEFAULT_CMAPS.items()):
                    col = edit_cols[i % 2]
                    current = live_cmaps[field]
                    idx = cmap_list.index(current) if current in cmap_list else 0
                    edit_cmaps[field] = col.selectbox(
                        f"{FIELD_LABELS[field]}",
                        cmap_list, index=idx,
                        key=f"edit_cmap_{sim_id}_{field}",
                    )
                if st.button("💾 Persist colormaps to saved run", key=f"persist_{sim_id}"):
                    st.session_state.twin_simulations[sim_id]['metadata']['colormaps'].update(edit_cmaps)
                    for f, v in edit_cmaps.items():
                        st.session_state.twin_simulations[sim_id]['params'][f'cmap_{f}'] = v
                    st.session_state.twin_simulations[sim_id]['params']['resolved_colormaps'] = dict(edit_cmaps)
                    st.success("Colormaps persisted to saved run.")
                    st.rerun()
                live_cmaps = edit_cmaps
            ...
            if history:
                ...
                style_params = build_style_params(
                    live_cmaps,
                    extra={
                ...
                with st.expander("🖼️ Interactive 2D (Plotly)", expanded=False):
                    field_choice = st.selectbox(
                        "Field",
                        list(FIELD_DEFAULT_CMAPS.keys()),
                        key=f"single_field_{sim_id}")
                    chosen = live_cmaps.get(field_choice, FIELD_DEFAULT_CMAPS[field_choice])
                    fig_h = visualizer.create_plotly_heatmap(
                        results, field_choice, frame_idx, cmap_name=chosen)
                    if fig_h:
        ...
        # Compute the effective colormaps once per rerun
        ...
        with tabs[0]:
            ...
            st.caption(f"Effective φ cmap: `{effective_cmaps['phi']}` · "
                       f"η₁: `{effective_cmaps['eta1']}` · h: `{effective_cmaps['h']}`")
            ...
            fig = visualizer.create_multi_field_comparison(
                initial_results,
                build_style_params(effective_cmaps, extra=style_extra))
            if fig:
                st.pyplot(fig)
                plt.close(fig)
        ...
        with tabs[2]:
            if 'results_history' in st.session_state:
                ...
                with st.expander("🎨 Live colormap overrides (this view only)", expanded=False):
                    ...
                    for i, (field, default) in enumerate(FIELD_DEFAULT_CMAPS.items()):
                        col = live_cols[i % 2]
                        idx = cmap_list.index(live_cmaps[field]) if live_cmaps[field] in cmap_list else 0
                        live_cmaps[field] = col.selectbox(
                            FIELD_LABELS[field], cmap_list, index=idx,
                            key=f"runview_cmap_{field}")
                fig = visualizer.create_multi_field_comparison(
                    results,
                    build_style_params(live_cmaps, extra=style_extra))
                if fig:
                    st.pyplot(fig)
                    plt.close(fig)
        ...
        with tabs[4]:
            if 'results_history' in st.session_state:
                ...
                st.caption(
                    "Colormaps here follow the same effective selection as the "
                    "matplotlib figures — edit them on the **Basic Results** tab."
                )
                results_history = st.session_state.results_history
                ...
                chosen_cmap = effective_cmaps.get(plotly_field,
                                                  FIELD_DEFAULT_CMAPS[plotly_field])
                fig_heatmap = visualizer.create_plotly_heatmap(
                    results, plotly_field, frame_idx_plotly, cmap_name=chosen_cmap)
                if fig_heatmap:
        ...
        with tabs[5]:
            ...
            if 'results_history' in st.session_state:
                ...
                chosen_cmap_3d = effective_cmaps.get(field_3d,
                                                     FIELD_DEFAULT_CMAPS[field_3d])
                fig_3d = visualizer.create_plotly_3d_surface(
                    results, field_3d, frame_idx_3d, cmap_name=chosen_cmap_3d)
                if fig_3d:
                    st.plotly_chart(fig_3d, use_container_width=True)
                    st.markdown("""
                    **💡 Interactivity**:
        ...
        with tabs[6]:
            ...
            if 'results_history' in st.session_state and st.session_state.results_history:
                ...
                with col3:
                    ...
                    anim_cmap = st.selectbox(
                        "Animation colormap",
                        cmap_list,
                        index=cmap_list.index(effective_cmaps.get(anim_field, 'viridis'))
                              if effective_cmaps.get(anim_field, 'viridis') in cmap_list else 0,
                        key="anim_cmap")
