...
# ███ + PLASTICITY PARAMETER INTELLIGENT RECOMMENDER v9.3.1              ███
...
def plot_stacked_latentmoe_v10(candidates, param_title, param_symbol,
    ...
    # ── 2. Annotations: total score + ⭐ BEST crown ────────────────────
    ...
    ui_scale_for_log = spec["param"] if hasattr(spec, "param") else \
        cands_sorted[0].param
    _log_display = ui_scale_for_log in ("rho0", "gamma0_dot")
    ...
    # ── 4. Legend — 2 columns, no frame, upper-left of chart area ──────
    if drawn_experts:
        ax.legend(
        ...
        if d_m > 0:
            ...
            if sigma0 > 0:
                ...
                out.append(ValueCandidate(
                    ...
                    unit="Pa",
                    provenance="physics_inferred",   # ── v9.3.1 rename
...
def _pick_best(cands):
    ...
    if not ranked:
        ...
        ranked = list(cands)
    trust = {
        'llm_extract':  3, 'llm_reasoned': 3,        # verbatim corpus anchor
        'regex_ner':    2, 'physics_inferred': 2,    # deterministic / reproducible
        'regime_prior': 1, 'llm_prior':    1,        # parametric / lookup
    }
    def key(c):
...
class ValueCandidate:
    ...
    dominant_factor: str = ""
    theory: str = ""
    context: bool = False        # ── v9.3.0: non-ranked variation candidate
...
class PlasticityCandidate:
    ...
    provenance: str = ""
    context: bool = False
    moe_breakdown: Dict[str, float] = field(default_factory=dict)  # ── v10.0
...
class PlasticityLatentMoEScorer:
    ...
    # Expert colour palette used by plot_stacked_latentmoe_v10 —
    # kept on the class so scorer and visualizer cannot drift.
    ...
    EXPERT_COLORS: Dict[str, str] = {
        'Material':       '#E69F00',   # Okabe–Ito orange
        'Thermal':        '#56B4E9',   # Okabe–Ito sky
        'Strain':         '#009E73',   # Okabe–Ito green
        'Method':         '#CC79A7',   # Okabe–Ito pink
        'Confidence':     '#999999',   # neutral grey
        'Reasoning':      '#F0E442',   # Okabe–Ito yellow
        'Framework (A)':  '#0072B2',   # Okabe–Ito blue    ← theory thread
        'Genesis (B)':    '#D55E00',   # Okabe–Ito vermil. ← regime thread
        'Thermo-Mech (C)': '#000000',  # black             ← srs thread
        'Corpus Density': '#88CCEE',   # light blue        ← provenance
    }

    def __init__(self, thermal_sigma: float = 100.0):
        # ── v10.x rebalanced weights (sum = 1.00) ──────────────────────
        ...
        self.w_framework  = 0.15   # Thread A — mu / gamma0_dot
        self.w_genesis    = 0.15   # Thread B — rho0 / sigma0_fric
        self.w_thermomech = 0.10   # Thread C — srs
        self.w_density    = 0.05   # Thread D — corpus evidence

        self.thermal_sigma = thermal_sigma

        # Regime experts hoisted to __init__ (previously created per-call
        # in v9.3.1 — pure perf win, no behavioural change).
    ...
    def _method_expert(method: str) -> float:
        return {
            ...
            # v9.3.0 σ₀ route disaggregation — preserved for backward compat
    ...
    def _thermomech_expert(p: str, v_si: float,
        ...
        Rationale: FCC metals at T ≲ 300 K typically exhibit m ∈ [15, 75];
        at T ≳ 400 K the exponent drops below ~10 because dislocation
        climb becomes competitive with glide.
        """
        if p != "srs":
            return 0.0
        try:
    ...
    def _density_expert(prov_fine: str) -> float:
        ...
        Grounded verbatim extraction, chain-of-thought reasoning, and
        deterministic regex all yield 1.0.  Physics-inferred rows sit
        at 0.7 (they apply laws to measured context).  Regime-lookup at
        0.4.  Pure parametric priors at 0.2.
        """
        ...
        if prov_fine == "regime_prior":
            return 0.4
        return 0.2   # llm_prior or unknown
    ...
    def recommend_grounded(self, material, temp_k, strain_rate=1e-3,
        ...
        for c in derived_v883:
            ...
            extractions.append({
                ...
                "_source_file": c.source,
                "_source_title": c.property_label,
                "_provenance": "physics_inferred",   # ── v9.3.1
        ...
        try:
            ...
            if cons:
                ...
                derived_cand = PlasticityCandidate(
                    ...
                    provenance='physics_inferred',   # ── v9.3.1
...
def render_recommender_bars_pub(bundle, style):
    ...
    # ════════════════════════════════════════════════════════════════════
    # ███ v10.0.0 — Journal variable hoisted (Change 9)               ███
    # ════════════════════════════════════════════════════════════════════
    ...
    if use_stacked_moe:
        # Context candidates are excluded from the stacked view so the
        # visible ranking always matches the pinned winner from the
        # sidebar.  (They remain in the dataframe views.)
    ...
    # ── v9.3.1: audit toggle overrides the pin-list — vis-only.
    if show_all_competitive:
    ...
    # best_idx: find the pinned best (context excluded) so the yellow bar
    # is always the pinned winner even in audit mode.
    if show_all_competitive:
        # In audit mode: the winner is whichever has the top score.
        best_idx = int(np.argmax(scores))
    else:
        # Standard mode: winner is the top-scoring non-context candidate.
        non_ctx_scores = [
        ...
        if best_idx < 0 or not np.isfinite(non_ctx_scores[best_idx]):
            best_idx = int(np.argmax(scores))  # degenerate fallback
...
def render_recommender_visuals_dashboard():
    ...
    st.caption(
        ...
        "candidates. v10.0.0: bar-chart tab offers an exact-XAI stacked "
        "LatentMoE view (each coloured segment IS one expert's weighted "
        "contribution). v9.3.1: fine key `physics_inferred` replaces the "
        "misleading `derived`; the bar-chart tab also offers a 🔓 audit "
        "toggle to reveal the full competitive ranking without moving "
...
def main():
    ...
    <strong>✅ PURE FFT SPECTRAL + AI PLASTICITY RECOMMENDER v10.0.0:</strong><br>
    ...
    • <span style="color: green;">🌟 v10.0.0 PHYSICS-THREAD MoE:</span> 10-expert ranker (Material/Thermal/Strain/Method/Confidence/Reasoning + Framework A/Genesis B/Thermo-Mech C/Corpus Density) with exact-XAI stacked bar chart — each coloured segment IS one expert's weighted contribution.<br>
...
def _regression_test_v883() -> None:
    ...
    # ── v9.3.1: check the renamed fine key
...
def _regression_test_v921() -> None:
    ...
    # ── v9.3.1 rename: physics_inferred replaces derived.
    ...
    # Legacy 'derived' token still classifies as physics_inferred.
...
def _regression_test_v930() -> None:
    """v9.3.0 — union+context invariants: pinned routes never context;
    non-incumbent routes context; _pick_best excludes context; graph
    renders context without crashing; _is_context pinned-list semantics
    are consistent with PROVENANCE_GROUPS."""
...
def _regression_test_v931() -> None:
    ...
    # ── 1. Fine key rename is total and exhaustive.
    ...
    # ── 2. Legacy 'derived' string still classifies correctly.
    assert _norm_provenance('derived') == 'physics_inferred', \
    ...
    # ── 3. Coarse rollup is unchanged by the rename.
    ...
    # ── 4. gatekeep conf-cap path for physics_inferred.
    ...
    # ── 5. _pick_best still ranks physics_inferred under trust=2.
    ...
    # ── 6. Audit toggle semantics: the flag flip is symmetric.
    # Simulate what render_recommender_bars_pub does.
    ...
    # ── 7. Chart renders under both modes without error.
    fig = plot_candidate_scores(
...
def _regression_test_v10() -> None:
    ...
    # ── 1. New field exists and defaults to {} on legacy construction.
    import dataclasses as _dc
    ...
    # ── 3. Expert palette and order are consistent.
    ...
    # ── 4. Framework expert does NOT route mu through TheoryRegimeExpert.
    ...
    # ── 5. gamma0_dot framework expert fires when theory is active.
    ...
    # ── 6. Cross-theory tag penalty is still decisive.
    ...
    # ── 7. Density expert bins provenance correctly.
    ...
    # ── 8. End-to-end: scorer produces a complete breakdown per candidate.
    ext_full = [{
    ...
    # ── 9. Stacked chart renders without error and returns a Figure.
    fig = plot_stacked_latentmoe_v10(
        candidates=bucket,
    ...
    # ── 10. Empty candidate list returns None cleanly.
...
def _run_regression_suite() -> None:
    """Run the v8.8–v10.0.0 battery.  Names resolve via globals().get()
    ...
    suite = [
        ...
        ('v10.0.0', '_regression_test_v10'),   # ── NEW
    ]
