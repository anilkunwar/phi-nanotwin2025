...
class NanotwinnedCuSolver:
    ...
    def __init__(self, params):
        ...
        # --- FIX: Confinement of twin evolution to the twinned grain ---
        # This parameter determines whether φ is allowed to evolve outside the twinned grain.
        # When set to True (default), dphi_dt is multiplied by η₁², effectively pinning φ = +1
        # where the grain is absent. This eliminates the unphysical φ = 0 lines that appear
        # due to numerical diffusion in the non‑twinned region.
    ...
    def evolve_twin_field(self, sxx, syy, sxy, eps_p_mag):
        try:
            ...
            # --- PHYSICAL CONFINEMENT: suppress evolution outside the twinned grain ---
            # Multiplying the whole right‑hand side by η₁² ensures that φ only changes where
            # the twinned grain actually exists. Outside (η₁ ≈ 0) the evolution is zero,
            # pinning φ to its initial value +1. This eliminates spurious φ = 0 contours
            # while keeping the thermodynamic driving force intact inside the grain.
            if self.confine_twin:
                dphi_dt = dphi_dt * self.eta1**2
...
def main():
    ...
    • <span style="color: green;">RESET TO DEFAULTS:</span> One‑click restore of all sidebar parameters.<br>
    • <span style="color: green;">PHYSICAL TWIN CONFINEMENT:</span> Eliminates spurious φ = 0 lines outside the twinned grain.<br>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        if st.button("🔄 Reset to Defaults", type="secondary", help="Reset all parameters to default values"):
            ...
            keys_to_clear = ['N', 'dx', 'dt', 'twin_spacing', 'W', 'A', 'B', 'kappa0', 'gamma_aniso',
                            ...
                            'n_steps', 'save_freq', 'confine_twin']
        ...
        if operation_mode == "Run New Simulation":
            ...
            with st.expander("🔧 Advanced Options"):
                ...
                # --- NEW: Twin confinement toggle ---
                confine_twin = st.checkbox("Confine twin evolution to twinned grain", True, key="confine_twin",
                                          help="Prevents φ from evolving outside η₁>0 – eliminates spurious φ=0 lines.")
        ...
        # ====================================================================
            ...
            base_params = {
                ...
                'confine_twin': True,   # Always use confinement in sweep
            }
