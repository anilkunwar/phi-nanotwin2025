...
# ███ + PLASTICITY PARAMETER INTELLIGENT RECOMMENDER v10.1.1             ███
...
# ███     Maps (synthesis, architecture, λ) → log-uniform [low, high]      ███
# ███     band drawn from a curated table of 8 regimes.  λ modulates      ███
# ███     the log-center inside the band (smaller λ → higher ρ₀).  NO      ███
# ███     hard quantity is emitted — the LLM still chooses within [low,   ███
# ███     high].                                                          ███
# ███   · `build_rho0_physics_inference_prompt`: dedicated ρ₀ prompt       ███
# ███     that forces the three-factor reasoning chain and asks for       ███
# ███     2–3 candidates INSIDE the regime band, each tagged with a       ███
# ███     `dominant_factor` ∈ {synthesis, architecture, twin_spacing,     ███
# ███     combined}.  Confidence capped at 0.50.                          ███
# ███   · `PhysicsRegimeExpert`: new LatentMoE expert (weight 0.25 for    ███
# ███     ρ₀ only, activated when any contextual factor is known).        ███
# ███     Rewards candidates inside [low, high] with a Gaussian peak at   ███
# ███     the log-center; exponential decay (1 decade ≈ 0.37) outside.    ███
# ███   · `PlasticityLatentMoEScorer.score` gains `synthesis`,            ███
# ███     `architecture`, `twin_spacing` kwargs and per-parameter weight   ███
# ███     switching (regime-aware for ρ₀ only).                           ███
# ███   · `recommend_rho0_physics`: dedicated ρ₀ cascade — Tier 1 grounded ███
# ███     LLM ∧ Tier 2 snippet-annotated NER ∧ Tier 3 physics-regime LLM  ███
# ███     ∧ Tier 3b pure-classifier fallback.  Snippet findings annotate   ███
# ███     Tier-1 hits so the regime expert fires even on verbatim values.  ███
# ███   · `PlasticityRecommender.__init__` gains rho0_synthesis /         ███
# ███     rho0_architecture / rho0_twin_spacing kwargs;                    ███
# ███     `recommend_grounded` special-cases the ρ₀ branch.               ███
# ███   · Sidebar ρ₀ physics-regime inputs expander with live classifier  ███
# ███     preview before Analyse.                                         ███
# ███   · `ValueCandidate.dominant_factor` field carried through           ███
# ███     `gatekeep`.                                                     ███
# ███   · Regression test `_regression_test_v90` — classifier monotonicity,███
# ███     snippet extraction, expert in/out of regime, weight switching.  ███
# ███ NEW (v9.1.0): THEORY-AWARE γ̇₀ NER                                ███
# ███   · GAZETTEER gains a third entity type:                            ███
# ███       `theory_framework`      (Johnson-Cook, power-law, CPFEM,      ███
# ███                                DDD/ParaDiS, MD, phase-field,        ███
# ███                                experimental tensile/SHPB)           ███
# ███   · `GAMMA0_DOT_REGIMES`: seven-entry regime table mapping each      ███
# ███     framework to a log-uniform [low, high] band.  JC / power-law /   ███
# ███     CPFEM / phase-field / experimental cluster at 1e-4..1e-2 s⁻¹;    ███
# ███     DDD sits at 1e3..1e4 s⁻¹; MD at 1e6..1e8 s⁻¹.                    ███
# ███   · `_norm_theory`: canonicaliser free text → one of                 ███
# ███     {johnson_cook, power_law, cpfem, ddd, md, phase_field,           ███
# ███      experimental}.                                                  ███
# ███   · `classify_gamma0_regime`: framework → [low, high, inferred]      ███
# ███     log-uniform band.  No hard quantity is emitted.                  ███
# ███   · `extract_gamma0_with_context`: ±500-char snippet-enhanced NER    ███
# ███     that captures a theory-framework keyword alongside every γ̇₀      ███
# ███     mention, larger window than ρ₀ because theory context often      ███
# ███     appears in the preceding sentence.                               ███
# ███   · `build_gamma0_theory_aware_prompt`: dedicated Tier-1 prompt       ███
# ███     that presents the seven-framework table a priori and forces a    ███
# ███     `theory` tag on every extracted value.                           ███
# ███   · `build_gamma0_theory_inference_prompt`: Tier-3 prior prompt       ███
# ███     anchored to a user-selected target framework with a three-step   ███
# ███     reasoning chain (framework → material/T cross-check → combine).  ███
# ███   · `TheoryRegimeExpert`: new LatentMoE expert (weight 0.25 for      ███
# ███     γ̇₀ only, activated when a target theory is selected).           ███
# ███     Two-factor score = tag_match × regime_membership.  Cross-        ███
# ███     theory tag mismatch (e.g. CPFEM tag when user asked DDD) drops   ███
# ███     the score to 0.05, decisively outranking a mere out-of-band       ███
# ███     penalty (~0.37 at 1.5 decades out).                              ███
# ███   · `PlasticityLatentMoEScorer.score` gains `target_theory` kwarg    ███
# ███     and per-parameter weight switching (theory-aware for γ̇₀ only).  ███
# ███   · `recommend_gamma0_theory_aware`: dedicated γ̇₀ cascade —          ███
# ███     Tier 1 theory-tagged grounded LLM ∧ Tier 2 snippet-annotated     ███
# ███     NER ∧ Tier 3 theory-regime LLM ∧ Tier 3b pure-classifier         ███
# ███     fallback.                                                        ███
# ███   · `PlasticityCandidate` and `ValueCandidate` gain a `theory` field ███
# ███     carried through gatekeep so downstream display and Sankey/       ███
# ███     Treemap can disambiguate regimes visually.                       ███
# ███   · `PlasticityRecommender.__init__` gains `gamma0_target_theory`;    ███
# ███     `recommend_grounded` special-cases the γ̇₀ branch.               ███
# ███   · Sidebar γ̇₀ theory-regime inputs expander with live classifier    ███
# ███     preview before Analyse.                                          ███
# ███   · Regression test `_regression_test_v91` — regime ordering         ███
# ███     (JC < DDD < MD), snippet extraction, tag-match / cross-theory    ███
# ███     expert behaviour.                                                ███
...
# ███   RENAMED to `physics_inferred` because these values are not     ███
# ███   constructed from nothing; they apply physical laws (Hall–Petch,███
# ███   Peierls–Nabarro, M·τ_P) to MEASURED side-note context.  All    ███
# ███   `_provenance="derived"` emissions and `provenance="derived"`   ███
# ███   ValueCandidates updated.  `_DERIVED_HINTS` retains backward-   ███
# ███   compatible matching on the old token so frozen snapshots keep  ███
# ███   classifying correctly.  Fine legend label updated to          ███
# ███   "Physics-inferred (Hall–Petch / P–N)".  NEW: chart-tab        ███
# ███   toggle "🔓 Show all routes as competitive" — a vis-only       ███
# ███   audit switch that reveals the FULL ranking without moving the  ███
# ███   pinned recommendation.  Regression test `_regression_test_v931` ███
# ███   pins: fine-key rename exhaustive; old 'derived' token still    ███
# ███   classifies as physics_inferred; gatekeep conf-cap path;        ███
# ███   toggle semantics in the chart UI.                              ███
# ███ NEW (v10.0.0): PHYSICS-THREAD MoE + STACKED XAI CHART              ███
# ███   · PlasticityCandidate gains `moe_breakdown` field so the       ███
# ███     scorer can hand exact per-expert contributions to the        ███
# ███     visualizer.  Defaults to {} for full backward compat.        ███
...
# ███     Weights sum to 1.00; regime experts (PhysicsRegimeExpert,    ███
# ███     TheoryRegimeExpert) intact but re-weighted via the threads.  ███
# ███   · NEW `plot_stacked_latentmoe_v10` — exact deterministic       ███
# ███     feature attribution (XAI) with no post-hoc SHAP/LIME         ███
# ███     approximation: the bar height IS the score, and each         ███
# ███     coloured segment IS one expert's weighted contribution.      ███
# ███   · `render_recommender_bars_pub` gains a toggle to switch       ███
# ███     between the v9.x provenance chart and the v10.x stacked      ███
# ███     MoE chart.  Context candidates are excluded from the         ███
# ███     stacked view so the visible ranking matches the pin-list.    ███
# ███ FIX (v10.0.0): `compute_yield_stress` unit mismatch — `h`        ███
# ███   arrives in nm from `compute_twin_spacing_from_gradient`, but   ███
# ███   `b` is passed in metres.  The Hall–Petch hardening term         ███
# ███   therefore evaluated to a few Pa instead of ~100 MPa and was    ███
# ███   effectively inert.  Call site now converts h → h * 1e-9.       ███
# ███ FIX (v10.0.0): typo `tau_p_pnpn` (undefined name, dead-branch    ███
# ███   guarded by `if False`) in `derive_sigma0_and_tau_p` —          ███
# ███   replaced with `tau_p_pn`.                                       ███
# ███ FIX (v10.0.0): Framework expert (Thread A) no longer routes      ███
# ███   `mu` candidates through `TheoryRegimeExpert`, whose regime     ███
# ███   table is defined in s⁻¹.  `mu` now receives the neutral 0.8    ███
# ███   default regardless of `target_theory`.                          ███
# ███ FIX (v10.1.1): σ₀ disaggregated routes are NON-CONTEXT — they    ███
# ███   used to be forced `context=True` (a v9.3.0 artifact) which     ███
# ███   contradicted `INCUMBENT_ROUTES['sigma0_fric']=('deterministic',)`.
# ███   Because `physics_inferred ∈ deterministic`, `_is_context() `    ███
# ███   returns False, so the four σ₀ routes AND the consensus row     ███
# ███   now appear in the stacked LatentMoE chart with their full      ███
# ███   per-expert breakdown.                                           ███
# ███ FIX (v10.1.1): Stacked LatentMoE chart NO LONGER renders the      ███
# ███   "⭐ BEST" text annotation.  The winning non-context bar is     ███
# ███   now marked with a thin orange underline — publication-clean.   ███
# ███ NEW (v10.1.1): `plot_stacked_latentmoe_v10` gains `show_context`  ███
# ███   kwarg.  When True, non-incumbent context candidates (e.g. a    ███
# ███   DDD-tagged γ̇₀ = 5e3 s⁻¹ when the user selected CPFEM) appear   ███
# ███   as translucent grey-overlaid bars alongside the pinned winner. ███
# ███   Exposed via a "🩶 Show context candidates in stacked chart"    ███
# ███   checkbox in the bar-chart tab (default off).                    ███
# ███ NEW (v10.1.1): `plot_candidate_scores` gains `show_best_legend`   ███
# ███   kwarg (default False).  The "Best match" yellow Patch legend    ███
# ███   entry is now OPT-IN via a checkbox in the bar-chart tab — the   ███
# ███   yellow bar itself still renders on the chart either way.        ███
# ███ FIX (v10.1.1): `m` axis label corrected from "Strain-Rate         ███
# ███   Sensitivity Exponent" to "Inverse Strain-Rate Sensitivity       ███
# ███   Exponent" to match the nt-Cu literature (Ye 2012, Parween       ███
# ███   2026) where m is the stress exponent in σ ∝ ε̇^(1/m).            ███
# ███   New aliases "inverse strain rate sensitivity",                   ███
# ███   "inverse strain-rate sensitivity", "inverse srs", and           ███
# ███   "srs index" added to the canonical alias list; the LLM prompt    ███
# ███   chain clarifies the inverse-exponent semantics.                  ███
# ███ NEW (v10.1.1): `_regression_test_v1011` pins the four fixes:      ███
# ███   σ₀ routes are non-context; no '⭐ BEST' string anywhere in the  ███
# ███   stacked figure; 'Best match' legend entry is opt-in only;       ███
# ███   `m` label and PARAM_META title both read "Inverse Strain-Rate   ███
...
# ███ The fine key formerly named `derived` is now `physics_inferred`. ███
# ███ These values are not constructed from nothing — they apply        ███
# ███ physical laws (Hall–Petch, Peierls–Nabarro, M·τ_P, solver-law    ███
# ███ inversion) to MEASURED side-note context.  The old token          ███
# ███ `'derived'` remains in `_DERIVED_HINTS` so any frozen snapshot    ███
# ███ that references the legacy label still classifies correctly.      ███
# ============================================================================
...
# Coarse (default, main text): ≤ 3 entries + Best match.  Shape only.
...
# Fine (opt-in, SI / audit): shape = method, fill = corpus evidence.
PROVENANCE_MARKERS: Dict[str, Dict[str, Any]] = {
    ...
    # ── v9.3.1 honest naming: `derived` → `physics_inferred` ─────────
    'physics_inferred': dict(marker='v', ms=7.0, fill=False,
                             label='Physics-inferred (Hall–Petch / P–N)'),
}

# ════════════════════════════════════════════════════════════════════════
# ███ v9.3.0 — INCUMBENT ROUTE PIN-LIST + CONTEXT CLASSIFIER          ███
# ════════════════════════════════════════════════════════════════════════
INCUMBENT_ROUTES: Dict[str, Tuple[str, ...]] = {
    'gamma0_dot':  ('llm_grounded',),     # ◆ won
    'srs':         ('deterministic',),    # ▲ won
    'mu':          ('deterministic',),
    'rho0':        ('deterministic',),
...
def _is_context(param: str, prov: Any) -> bool:
    ...
    A candidate is context iff (a) `param` is pinned in INCUMBENT_ROUTES,
    and (b) its coarse bucket is not among the pinned buckets."""
...
_DERIVED_HINTS: Tuple[str, ...] = (
    # v9.3.1 — 'physics_inferred' is the canonical token; 'derived' is
    # retained for backward-compat with frozen snapshots + legacy strings.
...
def _norm_provenance(p: Any) -> str:
    ...
    Ordering is LOAD-BEARING — do not reorder without re-running
    _regression_test_v921:

        1. exact fine key
        2. physics inference      — 'model_inversion' etc. never LLM
        3. regime classifier      — MUST precede prior, else compounds
                                     like 'physics_regime_prior' become
                                     LLM priors (the mislabel v9.2.1
                                     exists to kill)
        4. LLM prior
    ...
    NOTE (v9.2.2): 'llm_reasoned' is a RESERVED fine key.  With Defect-1
    Option B (default) no emitter stamps it — the bucket is empty by
    design and the present-only legend can never show it.

    NOTE (v9.3.1): the fine key formerly named 'derived' is renamed to
    'physics_inferred' because these values apply physical laws
    (Hall–Petch, Peierls–Nabarro, M·τ_P, solver-law inversion) to
    MEASURED side-note context.  The legacy token 'derived' is matched
    in _DERIVED_HINTS and re-mapped to the new key for backward
    compatibility.
    """
    s = str(p).strip().lower()
...
def _norm_granularity(g: Any) -> str:
    """Single normalization point — invalid values warn and default to
    'coarse'."""
...
PARAM_META = {
    ...
    # ── v10.1.1 FIX: `m` is the INVERSE strain-rate sensitivity exponent
    # (stress ∝ strain-rate^(1/m)), matching the nt-Cu literature.
    'srs':            dict(title='Inverse Strain-Rate Sensitivity Exponent',
                           symbol='m', unit=None),
...
def plot_candidate_scores(candidates, scores, provenance, best_idx=None, *,
                          ...
                          context_flags=None,
                          show_best_legend: bool = False):   # ← NEW (v10.1.1)
    n = len(candidates)
    ...
    for key, st_ in marker_table.items():
        ...
        handles.append(Line2D([], [], marker=st_['marker'],
                              ...
                              label=lbl))
    # ── v10.1.1: "Best match" legend entry is now OPT-IN ─────────────
    # (default off — the yellow bar still appears on the chart, but the
    # explicit "Best match" Patch in the legend is suppressed unless the
    # user requests it via the `show_best_legend` kwarg / UI checkbox.)
...
# ============================================================================
# ███ v10.0.0 / v10.1.1 — STACKED LATENT MoE VISUALIZATION (EXACT XAI)   ███
# ============================================================================
def plot_stacked_latentmoe_v10(candidates, param_title, param_symbol,
                               unit, journal='nature',
                               fig_size=(6.5, 4.0),
                               max_headroom: float = 1.18,
                               show_context: bool = False):   # ← NEW (v10.1.1)
    """Publication plot: bar height = total LatentMoE score; each coloured
    segment = one expert's exact weighted contribution.

    Because the MoE is deterministic and linear, this is a **exact
    feature attribution** — no SHAP/LIME approximation needed.  The
    decomposition is algebraically identical to the score itself:

        score(c) = Σ_e  w_e · s_e(c)

    Args:
        show_context: when True, non-incumbent context candidates are
            included (drawn as translucent grey-overlaid bars).  When
            False, only non-context candidates appear.

    Returns the matplotlib Figure (caller handles rendering + export)
    ...
    # Sort ascending by score; the top-scoring (rightmost) bar is the
    # pinned winner when it is not a context candidate.
    ...
    # ── 1. Draw stacked bars, expert by expert ─────────────────────────
    ...
    # ── 2. Translucent grey overlay on context bars (if included) ──────
    if show_context:
    ...
    # ── 3. Annotations: total score above each bar + winner underline ──
    #  v10.1.1: the "⭐ BEST" text annotation is GONE.  The winning
    #  non-context bar is instead marked with a thin orange underline
    #  drawn just below the x-axis — publication-clean, no icons.
    winner_x: Optional[int] = None
    for i, c in enumerate(cands_sorted):
        is_ctx = bool(getattr(c, 'context', False))
        if not is_ctx:
            winner_x = i     # last non-context index wins (sorted ascending)
    for i, (x, c) in enumerate(zip(xs, cands_sorted)):
        ...
        ax.text(
            ...
            color='0.45' if is_ctx else 'black',
            zorder=6)
        if is_winner:
            ax.plot(
    ...
    # X-tick labels show the raw recommended value in UI units
    ...
    # ── 5. Legend — 2 columns, no frame, upper-left of chart area ──────
    if drawn_experts:
        ax.legend(
...
PARAM_CANON: Dict[str, Dict[str, Any]] = {
    ...
    "srs":            dict(aliases=["inverse strain rate sensitivity exponent",
                                    "inverse strain-rate sensitivity exponent",
                                    "inverse strain rate sensitivity",
                                    "inverse strain-rate sensitivity",
                                    "inverse srs",
...
# v9.3.1 — the emitted provenance token is now 'physics_inferred' but the
#          value-level method labels ('hall_petch_intercept', …) are
#          unchanged because they are semantic descriptions, not taxonomy.
# ============================================================================
...
def _pick_best(cands):
    """v9.3.0 — context candidates are excluded from ranking.  A pool
    consisting entirely of context candidates triggers a loud warning
    and falls back to ranking all candidates (degenerate path)."""
    if not cands:
        return None
...
def gatekeep(items, param_key, provenance="regex_ner", conf_cap=None):
    """Unit conversion + plausibility + dedupe + confidence cap.

    v9.2.2: provenance is now a FINE key.
    v9.3.0: `_context` flag on the input dict is copied through.
    v9.3.1: `physics_inferred` gets the derived-value confidence cap of 0.75.
    """
    canon = PARAM_CANON.get(param_key, {})
    (lo, hi) = canon.get("plausible", (float("-inf"), float("inf")))
...
def recommend_param_values(param_key, material="Cu", value_hints=None,
    ...
    """Generic 3-tier cascade.

    cascade_mode='fallback': short-circuit; each param's pool is exactly
    one tier's output.

    cascade_mode='union': every viable tier runs.  Non-incumbent routes
    (i.e., routes whose coarse bucket is not in INCUMBENT_ROUTES[param])
    are tagged `_context=True` — greyed, non-ranked variation bars.
    """
...
def recommend_gamma0_theory_aware(
    ...
    # ── v9.3.0 — Tier 2b: regex NER candidate pool in union mode ──
...
class NanotwinnedCuSolver:
    ...
    def step(self):
        try:
            ...
            plastic_params = self.mat_props['plasticity']
            # ── v10.0.0 FIX ─────────────────────────────────────────────
            # `h` is returned by compute_twin_spacing_from_gradient in nm
            # (because kx is in 1/nm), but `b` is stored in metres.
            # Passing h directly made the Hall–Petch term evaluate to a
            # few Pa instead of ~100 MPa — i.e. effectively inert.  We
            # convert h → metres here so the units are consistent.
            h_m = h * 1e-9
...
PLASTICITY_ONTOLOGY: Dict[str, Dict[str, Any]] = {
    ...
    # ── v10.1.1 FIX: `m` is the INVERSE strain-rate sensitivity exponent
    # (stress ∝ strain-rate^(1/m)).  Matches the nt-Cu literature:
    # Ye 2012; Parween 2026 ("SRS index or m of nt-Cu typically ranges
    # from 16.67 to 71.42…").
    "srs": {
        "label": "Inverse Strain-Rate Sensitivity Exponent",
        "symbol": "m",
...
_PARAM_ALIASES: Dict[str, str] = {
    ...
    "inverse_strain_rate_sensitivity": "srs",
    "inverse_strain-rate_sensitivity": "srs",
    "inverse_srs": "srs", "srs_index": "srs",
...
five ASCII strings — do NOT use symbols or synonyms):
  ...
  - "srs"         (inverse strain-rate sensitivity exponent, dimensionless)
...
these ASCII strings (character-for-character):
    ...
    "mu"            — shear modulus                          [Pa]
    "gamma0_dot"    — reference strain rate                  [s^-1]
    "srs"           — INVERSE strain-rate sensitivity exponent m  [dimensionless]
...
REASONING CHAINS (use these formulas, do NOT just look up a constant):
  ...
  srs (m, material, T):
    ...
    Step 2: m is the INVERSE strain-rate sensitivity exponent
            (stress ∝ strain-rate^(1/m)).
    Step 3: Output dimensionless value.
...
# ============================================================================
# ███ v10.0.0 — LATENT MoE SCORER WITH PHYSICS-THREAD EXPERTS            ███
# ============================================================================
class PlasticityLatentMoEScorer:
    """Physics-Informed Mixture of Experts (MoE) ranker.

    Ten experts, weighted linearly.  Weight vector sums to 1.00.
    ...
    Context-Aware Gating:
      Thread A activates the TheoryRegimeExpert when the user supplies a
      target theory — this is what makes a DDD-tagged γ̇₀ = 5e3 s⁻¹
      outrank an MD-tagged γ̇₀ = 1e7 s⁻¹ for a CPFEM target.

      Thread B activates the PhysicsRegimeExpert when the user supplies
      any of (synthesis, architecture, twin_spacing) — this is what
      makes an electrodeposited+equiaxed ρ₀ = 5e16 m⁻² outrank a
      sputtered+columnar ρ₀ = 1e12 m⁻².

      Threads A and B fall back to neutral priors when their gating
      context is absent, so the expert vector never goes degenerate.
    """
    ...
    def _framework_expert(self, p: str, v_si: float, ext: Dict[str, Any],
                          target_theory: Optional[str],
                          theory_active: bool) -> float:
        """Thread A — Theory/Framework expert.  Only fires meaningfully for
        gamma0_dot.  mu receives a neutral 0.8 in all cases."""
        if p == "gamma0_dot":
            ...
            return 0.8
        if p == "mu":
            # v10.0.0 FIX — `mu` MUST NOT be routed through
            # TheoryRegimeExpert, whose regime table is defined in s⁻¹.
            return 0.8
        return 0.0
    ...
    @staticmethod
    def _thermomech_expert(p: str, v_si: float,
                           target_temp: float) -> float:
        """Thread C — Thermo-mechanical consistency (currently srs only)."""
        if p != "srs":
            return 0.0
        try:
            v = float(v_si)
    ...
    def score(self, extractions, target_material, target_temp,
        ...
        for ext in extractions:
            ...
            s_tm  = self._thermomech_expert(p, v_si_clamped, target_temp)
        ...
        try:
            ...
            if cons:
                ...
                derived_cand = PlasticityCandidate(
                    ...
                    provenance='physics_inferred',
                    # ── v10.1.1 FIX: physics_inferred IS the incumbent
                    # bucket for σ₀, so consensus is NON-context.
            ...
            for r in derived_stress.get('routes', []):
                ...
                # ── v10.1.1 FIX: physics_inferred IS the incumbent bucket
                # for σ₀, so the disaggregated routes are non-context and
                # appear in the stacked LatentMoE chart.  (Previously
                # forced to True — a v9.3.0 artifact that hid the routes
                # from the exact-XAI view despite being the pinned winner
                # bucket for σ₀.)
...
def render_plasticity_recommender_sidebar(
    ...
    st.subheader("🤖 Intelligent Plasticity Recommender v10.1.1")
    st.caption(
        "**v10.1.1:** σ₀ disaggregated routes are now non-context (they "
        "were erroneously forced to `context=True` in v9.3.0 despite "
        "`INCUMBENT_ROUTES['sigma0_fric']=('deterministic',)` and "
        "`physics_inferred ∈ deterministic`).  The four routes and the "
        "consensus row now appear in the stacked LatentMoE chart.  The "
        "stacked chart no longer draws a '⭐ BEST' icon (a thin orange "
        "underline marks the winner instead), and the 'Best match' legend "
        "entry on the v9.x provenance chart is opt-in.  The `m` axis "
        "label was corrected to **Inverse Strain-Rate Sensitivity "
        "Exponent** to match the nt-Cu literature."
    )
    st.caption(
        "**v10.0.0 physics-thread MoE:** ten-expert ranker with Framework "
        "(A), Genesis (B), Thermo-Mech (C) and Corpus Density threads. "
        "The bar-chart tab now offers an exact-XAI stacked LatentMoE view "
        "where each coloured segment IS one expert's weighted contribution "
        "— no post-hoc SHAP/LIME approximation."
    )
    st.caption(
...
def render_recommender_bars_pub(bundle, style):
    ...
    if use_stacked_moe:
        # ════════════════════════════════════════════════════════════════
        # ███ v10.1.1 — Show context toggle                            ███
        # ════════════════════════════════════════════════════════════════
        show_context_in_stacked = st.checkbox(
            "🩶 Show context candidates in stacked chart",
            value=False,
            key="rec_stacked_show_context",
            help="Includes non-incumbent routes (e.g., a DDD-tagged "
                 "γ̇₀ = 5e3 s⁻¹ when the user selected CPFEM) as "
                 "semi-transparent greyed bars.  The pinned winner is "
                 "unchanged — this is an audit view.")

        if show_context_in_stacked:
        ...
        fig = plot_stacked_latentmoe_v10(
            ...
            show_context=show_context_in_stacked,
        )
        if fig is not None:
            st.pyplot(fig)
            buf = BytesIO()
    ...
    # ════════════════════════════════════════════════════════════════════
    # ███ v9.3.1 — Audit toggle: temporarily disable pin-list for view. ███
    # ════════════════════════════════════════════════════════════════════
    ...
    # ════════════════════════════════════════════════════════════════════
    # ███ v10.1.1 — Best-match legend opt-in toggle                    ███
    # ════════════════════════════════════════════════════════════════════
    show_best_legend = st.checkbox(
        "Show 'Best match' legend entry",
        value=False,
        key="rec_bar_show_best_legend",
        help="Adds a yellow Patch to the legend marking the best-scoring "
             "non-context candidate. Off by default for a cleaner "
             "publication figure.")
...
def render_recommender_visuals_dashboard():
    ...
    st.caption(
        ...
        "candidates. v10.1.1: σ₀ disaggregated routes are non-context "
        "(they appear in the stacked chart); no '⭐ BEST' icon; the "
        "'Best match' legend entry is opt-in; `m` label reads "
        "**Inverse Strain-Rate Sensitivity Exponent**.")
...
def main():
    ...
    <strong>✅ PURE FFT SPECTRAL + AI PLASTICITY RECOMMENDER v10.1.1:</strong><br>
    ...
    • <span style="color: green;">🌟 v10.0.0 PHYSICS-THREAD MoE:</span> 10-expert ranker (Material/Thermal/Strain/Method/Confidence/Reasoning + Framework A/Genesis B/Thermo-Mech C/Corpus Density) with exact-XAI stacked bar chart.<br>
    • <span style="color: green;">✨ v10.1.1 FIXES:</span> σ₀ routes are non-context (they now appear in the stacked chart); no '⭐ BEST' icon (thin orange underline instead); 'Best match' legend entry opt-in; `m` label = "Inverse Strain-Rate Sensitivity Exponent"; stacked chart has a 🩶 show-context toggle.<br>
    ...
    with st.sidebar:
        ...
        if st.button("🔄 Reset to Defaults", type="secondary"):
            ...
            st.rerun()
        st.markdown("---")
        with st.expander("🧠 AI Plasticity Recommender v10.1.1", expanded=False):
...
def _regression_test_v92() -> None:
    """v9.2 — 3-bucket rollup: total, exhaustive, ≤ 3 coarse entries."""
...
def _regression_test_v921() -> None:
    """v9.2.1 hotfix pin-list: ordering, granularity guard, deltas."""
...
def _regression_test_v922() -> None:
    """v9.2.2 — read-side plumbing + fallback contract."""
    import dataclasses as _dc
...
def _regression_test_v930() -> None:
    """v9.3.0 — union+context invariants."""
...
def _regression_test_v931() -> None:
    """v9.3.1 — honest naming + audit toggle invariants."""
...
def _regression_test_v10() -> None:
    """v10.0.0 — Physics-thread MoE + stacked XAI invariants."""
...
def _regression_test_v1011() -> None:
    """v10.1.1 — σ₀ disaggregated routes are non-context; BEST icon is
    gone from the stacked chart; 'Best match' legend entry is opt-in;
    m axis label is corrected to 'Inverse Strain-Rate Sensitivity
    Exponent'."""

    # ── 1. σ₀ physics_inferred is the incumbent bucket → non-context.
    c_pinf = PlasticityCandidate(
        param='sigma0_fric', value_si=5e7, raw_value=50.0, raw_unit='MPa',
        score=0.55, confidence=0.55, material='Cu', temp_k=None,
        strain_rate=None, method='hall_petch_intercept',
        ...
        provenance='physics_inferred',
        context=_is_context('sigma0_fric', 'physics_inferred'))
    assert c_pinf.context is False, \
        "physics_inferred σ₀ must be non-context after the v10.1.1 fix"

    # Also check the σ₀ route methods classify the same way.
    for route_method in ('sigma0_route_hall_petch_d',
                         'sigma0_route_hall_petch_lam',
                         'sigma0_route_model_inversion',
                         'sigma0_route_peierls_nabarro',
                         'derived_consensus'):
        assert _is_context('sigma0_fric', route_method) is False, \
            f"σ₀ route {route_method!r} must be non-context"

    # ── 2. Stacked chart contains NO '⭐ BEST' string anywhere.
    # Provide a candidate with a full moe_breakdown so the chart renders.
    sc = PlasticityLatentMoEScorer()
    b = sc.score(
        [{"param": "sigma0_fric", "value": 5e7, "unit": "Pa",
          "material": "Cu", "temp": 300.0, "strain_rate": 1e-3,
    ...
    # ── 2b. Stacked chart honours show_context kwarg.
    c_ctx = PlasticityCandidate(
        param='sigma0_fric', value_si=8e7, raw_value=80.0, raw_unit='MPa',
        score=0.40, confidence=0.40, material='Cu', temp_k=None,
        strain_rate=None, method='prior inference',
        source_file='', source_title='', evidence='',
        provenance='llm_prior', context=True,
        moe_breakdown={k: 0.05 for k in PlasticityLatentMoEScorer.EXPERT_ORDER})
    fig_ctx = plot_stacked_latentmoe_v10(
        candidates=bucket + [c_ctx],
        param_title='Friction stress',
        param_symbol=r'\sigma_0',
        unit='MPa',
        journal='nature',
        show_context=True)
    assert fig_ctx is not None
    plt.close(fig_ctx)

    # ── 3. plot_candidate_scores: 'Best match' legend entry is opt-in.
    fig = plot_candidate_scores(
        [1e14, 1e15], [0.8, 0.9],
        ['regex_ner', 'physics_inferred'],
        best_idx=1, legend_granularity='coarse',
        show_best_legend=False)
    texts = {t.get_text() for t in fig.axes[0].get_legend().get_texts()}
    assert 'Best match' not in texts, \
        f"'Best match' legend entry appeared despite show_best_legend=False: {texts}"
    plt.close(fig)
    ...
    assert 'Best match' in texts, \
        f"'Best match' legend entry missing despite show_best_legend=True: {texts}"
    plt.close(fig)

    # ── 4. m axis label says "Inverse Strain-Rate Sensitivity Exponent".
    assert PLASTICITY_ONTOLOGY["srs"]["label"] == \
        "Inverse Strain-Rate Sensitivity Exponent", \
        f"srs label = {PLASTICITY_ONTOLOGY['srs']['label']!r}"
    assert PARAM_META["srs"]["title"] == \
        "Inverse Strain-Rate Sensitivity Exponent", \
        f"PARAM_META srs title = {PARAM_META['srs']['title']!r}"

    # ── 5. New aliases are wired in.
    for alias in ("inverse strain rate sensitivity",
                  "inverse strain-rate sensitivity",
                  "inverse srs", "srs index"):
        assert alias in PARAM_CANON["srs"]["aliases"], \
            f"missing srs alias {alias!r}"

    logger.info("✅ _regression_test_v1011 passed — "
                "σ₀ routes non-context, no '⭐ BEST', "
                "m label = 'Inverse Strain-Rate Sensitivity Exponent'")


def _run_regression_suite() -> None:
    """Run the v8.8–v10.1.1 battery.  Names resolve via globals().get()
