...
# ============================================================================
# YIELD STRESS  (EDIT 1: Taylor hardening term + continuous at h = 2b)
# ============================================================================
def compute_yield_stress(h, sigma0, mu, b, nu, rho0=None, alpha_taylor=0.3):
    ...
    σ_y = σ0 + (μ b)/(2π h (1-ν)) · ln(h/b) + (1/2) μ b α √ρ0

    The Hall-Petch term is evaluated on a floored h_eff = max(h, 2.001·b)
    so the function is continuous and C0 at the cutoff (no 2.9× kink).
    The Taylor term is only added when rho0 is a positive scalar/array.

    Parameters
    ----------
    h : ndarray
        Local twin spacing [m]. Floored internally at 2.001·b.
    sigma0 : float
        Lattice friction stress [Pa].
    mu : float
        Shear modulus [Pa].
    b : float
    ...
    nu : float
        Poisson ratio [-].
    rho0 : float or ndarray or None
        Initial dislocation density [m^-2]. If None or all-zero, Taylor
        term is skipped (backward-compatible with old call sites).
    alpha_taylor : float
        Taylor constant (literature range 0.2–0.5). The 1/2 prefactor
        is a modeling choice for the CRSS-like yield surface; change to
        M·α (M ≈ 3.06 for FCC) for tensile Taylor hardening.
    """
    h_arr = np.asarray(h, dtype=float)
    h_eff = np.maximum(h_arr, 2.001 * b)
    sigma_y = sigma0 + (mu * b / (2 * np.pi * h_eff * (1 - nu))) * np.log(h_eff / b)

    # Safe Taylor term: works for scalars, numpy scalars, and arrays.
    if rho0 is not None:
        rho_arr = np.asarray(rho0, dtype=float)
        if np.any(rho_arr > 0):
            sigma_y = sigma_y + 0.5 * mu * b * alpha_taylor * np.sqrt(
                np.maximum(rho_arr, 0.0))
    return sigma_y
...
# ============================================================================
# MATERIAL PROPERTIES  (now returns deep copies)
# ============================================================================
class MaterialProperties:
    ...
    def get_material(material_name='Cu'):
        """
        Returns a DEEP COPY of the material properties so that downstream
        mutation (e.g. LLM override) cannot pollute the class-level template.
        """
        if material_name == 'Cu':
            props = MaterialProperties.get_cu_properties()
        elif material_name == 'Al':
            props = MaterialProperties.get_al_properties()
        elif material_name == 'Ni':
            props = MaterialProperties.get_ni_properties()
        else:
            props = MaterialProperties.get_cu_properties()
...
# ============================================================================
# MAIN SOLVER (PURE FFT SPECTRAL METHOD)  -- EDIT 2 lives here
# ============================================================================
class NanotwinnedCuSolver:
    ...
    def __init__(self, params):
        ...
        # get_material now returns a deep copy; safe to mutate.
        ...
        if override:
            ...
            if clean:
                self.mat_props['plasticity'].update(clean)
                logger.info(f"Applied plasticity override for {material_name}: {clean}")
    ...
    def step(self):
        ...
        try:
            ...
            plastic_params = self.mat_props['plasticity']
            # ---- EDIT 2: pass rho0 and alpha_taylor to yield stress ----
...
# ============================================================================
# LLM RECOMMENDER BACKEND CACHE  (Stage F helper)
# ============================================================================
@st.cache_resource(show_spinner=False)
def _get_plasticity_backend(ollama_url="http://localhost:11434",
                            llm_model="qwen2.5:7b",
                            embed_model="sentence-transformers/all-MiniLM-L6-v2"):
    """
    Cached backend probe. Import is lazy so the app runs even if
    plasticity_recommender.py is absent.
    """
    try:
        from plasticity_recommender import get_backend
    except ImportError as e:
        logger.warning(f"plasticity_recommender not importable: {e}")
        return None
    return get_backend(ollama_url=ollama_url, llm_model=llm_model,
...
def main():
    ...
    • <span style="color: green;">LLM PLASTICITY RECOMMENDER:</span> literature-driven μ, σ₀, ρ₀, γ̇₀, m override.<br>
    ...
    with st.sidebar:
        ...
        if operation_mode == "Run New Simulation":
            ...
            # ============================================================
            # EDIT 3: LLM PLASTICITY RECOMMENDATION (Stage F)
            # ============================================================
            st.subheader("🦙 LLM Plasticity Recommendation")
            json_db_dir = st.text_input(
                "Metadatabase directory",
                value="./json_metadatabase",
                key="json_db_dir",
                help="Folder containing the five plasticity metadatabase JSON files.")

            # Material-change invalidation: if the user recommends for Cu, then
            # switches to Ni without clicking again, we must NOT silently apply
            # Cu values to Ni. We store the material with the override.
            current_material = material_choice.lower()
            stored_override = st.session_state.get("plasticity_override")
            if stored_override and stored_override.get("_material") != current_material:
                st.info(
                    f"Active override was generated for "
                    f"'{stored_override.get('_material')}', but the selected "
                    f"material is now '{current_material}'. Click below to "
                    f"regenerate, or clear the override.")

            col_rec, col_clr = st.columns([3, 1])
            with col_rec:
                do_recommend = st.button(
                    "🦙 Let LLM recommend",
                    type="primary", use_container_width=True,
                    key="btn_llm_recommend")
            with col_clr:
                if st.button("🧹 Clear override", use_container_width=True,
                             key="btn_clear_override"):
                    st.session_state.pop("plasticity_override", None)
                    st.success("Override cleared. Simulator defaults restored.")
                    st.rerun()

            if do_recommend:
                backend = _get_plasticity_backend()
                if backend is None:
                    st.warning(
                        "plasticity_recommender.py not found on the Python path. "
                        "Place it next to this app or on PYTHONPATH and retry. "
                        "Keeping simulator defaults.")
                else:
                    try:
                        from plasticity_recommender import (
                            load_evidence_corpus, recommend)
                    except ImportError as e:
                        st.error(f"Could not import recommender internals: {e}")
                        backend = None

                if backend is not None:
                    try:
                        with st.spinner(
                                "Loading metadatabase…"):
                            corpus = load_evidence_corpus(json_db_dir)
                        if not corpus:
                            st.warning(
                                "No metadatabase records found in "
                                f"{json_db_dir}; keeping defaults.")
                        else:
                            if not backend.get("llm_available"):
                                st.info(
                                    "Ollama not reachable — running "
                                    "deterministic (rule + regex + "
                                    "weighted-median) pipeline.")
                            mat_props = MaterialProperties.get_material(material_choice)
                            with st.spinner(
                                    "Categorizing → NER → graph reasoning → MoE fusion…"):
                                rec = recommend(
                                    corpus, mat_props,
                                    material=material_choice.lower(),
                                    backend=backend,
                                    json_dir=json_db_dir)

                            # ---- Provenance FIRST, values second ----
                            with st.expander(
                                    f"Evidence: {rec['n_evidence']} records "
                                    f"({rec['n_excluded']} excluded — e.g. non-Cu)",
                                    expanded=False):
                                if rec.get("evidence_table"):
                                    st.dataframe(pd.DataFrame(rec["evidence_table"]),
                                                 use_container_width=True)
                                else:
                                    st.caption("No evidence rows survived filtering.")
                                st.caption(f"MoE gate: {rec.get('gate')}")
                                for note in rec.get("notes", []):
                                    st.caption(f"• {note}")
                                if rec.get("top_attended_papers"):
                                    st.caption(
                                        "Top-weighted evidence: "
                                        + "; ".join(rec["top_attended_papers"]))
                                bk = rec.get("backend", {})
                                st.caption(
                                    f"Backend: llm={bk.get('llm_available')}, "
                                    f"model={bk.get('model')}")

                            # ---- Store the override, namespaced by material ----
                            override = {
                                "_material": current_material,
                                "mu": rec.get("mu"),
                                "sigma0": rec.get("sigma0"),
                                "rho0": rec.get("rho0"),
                                "gamma0_dot": rec.get("gamma0_dot"),
                                "m": rec.get("m"),
                            }
                            st.session_state["plasticity_override"] = override

                            # ---- Report success or partial fallback ----
                            present = {k: v for k, v in override.items()
                                       if k != "_material" and v is not None}
                            if len(present) == 5:
                                st.success(
                                    f"Recommended for {material_choice}: "
                                    f"μ={rec['mu']/1e9:.1f} GPa, "
                                    f"σ₀={rec['sigma0']/1e6:.0f} MPa, "
                                    f"ρ₀={rec['rho0']:.2e} m⁻², "
                                    f"γ̇₀={rec['gamma0_dot']:.1e} s⁻¹, "
                                    f"m={rec['m']} (from SRS={rec['srs']:.3f}).")
                            elif present:
                                st.warning(
                                    "Partial recommendation — missing "
                                    f"parameters ({', '.join(k for k in override if k!='_material' and override[k] is None)}) "
                                    "keep simulator defaults. Present values: "
                                    + ", ".join(f"{k}={v:g}" for k, v in present.items()))
                            else:
                                st.warning("Recommender returned no usable "
                                           "parameters. Keeping defaults.")
                    except Exception as e:
                        st.error(f"LLM recommendation failed: {e}")
                        st.caption("Falling back to simulator defaults.")
                        logger.error(traceback.format_exc())

            # ---- Show active override ----
            if "plasticity_override" in st.session_state:
                ov = st.session_state["plasticity_override"]
                ov_mat = ov.get("_material", "?")
                shown = {k: v for k, v in ov.items()
                         if k != "_material" and v is not None}
                if shown:
                    st.caption(
                        f"Active override (material: {ov_mat}): "
                        + ", ".join(f"{k}={v:g}" for k, v in shown.items()))
                else:
                    st.caption("No active override values.")
            ...
            if st.button("🚀 Initialize Simulation", type="primary", use_container_width=True):
                params = {
                    ...
                    # ---- EDIT 3: hand the (material-matched) override to the solver ----
                    'plasticity_override': (
                        st.session_state.get('plasticity_override')
                        if st.session_state.get('plasticity_override', {}).get('_material')
                           == material_choice.lower()
                        else None)
                }
                if geometry_type == "Twin Grain with Defect":
                ...
                if errors:
                    ...
                    if params.get('plasticity_override'):
                        st.success("✅ Simulation initialized with LLM override applied!")
                    else:
            ...
            base_params = {
                ...
                # Sweeps use simulator defaults; override not applied here to
                # keep the swept parameter the sole independent variable.
                'plasticity_override': None}
    ...
    if operation_mode == "Compare Saved Simulations" and 'comparison_config' in st.session_state:
        ...
        if sim_data:
            ...
            if params.get('plasticity_override'):
                ov = {k: v for k, v in params['plasticity_override'].items()
                      if k != '_material' and v is not None}
                if ov:
                    st.info("LLM override active: "
        ...
        with tabs[1]:
            ...
            ov = params.get('plasticity_override')
            if ov:
                ov_clean = {k: v for k, v in ov.items()
                            if k != '_material' and v is not None}
                if ov_clean:
                    st.info("LLM override active for this run: "
                            + ", ".join(f"{k}={v:g}" for k, v in ov_clean.items()))
