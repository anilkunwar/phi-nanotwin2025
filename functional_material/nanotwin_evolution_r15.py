...
class MetadataManager:
    ...
    def create_metadata(sim_params, history, run_time=None, **kwargs):
        ...
        metadata = {
            ...
            'colormaps': {
                ...
                'eta2': sim_params.get('cmap_eta2', 'Blues'),    # NEW
                'eps_p_mag': sim_params.get('cmap_eps_p', 'YlOrRd'),
                'sigma_y': sim_params.get('cmap_sigma_y', 'viridis')
            },
            'material_properties': {
                'C11': 168.4e9,
                'C12': 121.4e9,
    ...
    def validate_metadata(metadata):
        ...
        if 'colormaps' not in metadata:
            ...
            # Add missing eta2 colormap if not present
            if 'eta2' not in metadata['colormaps']:
                metadata['colormaps']['eta2'] = 'Blues'
...
class EnhancedTwinVisualizer:
    ...
    def create_multi_field_comparison(self, results_dict, style_params=None):
        ...
        Now includes grain order parameters (eta1, eta2) and masked twin boundaries.
        """
        if style_params is None:
            style_params = {}
        
        # Set robust defaults for font sizes and colormaps
        defaults = {
            'title_font_size': 10,
            'label_font_size': 8,
        ...
        # --------------------------------------------------------------------
        # Define the fields to plot – now includes eta1 and eta2
        # --------------------------------------------------------------------
        fields_to_plot = [
            ...
            ('eta1', 'Grain Order η₁', style_params['eta1_cmap'], [0, 1]),      # NEW
            ('eta2', 'Grain Order η₂', style_params['eta2_cmap'], [0, 1]),      # NEW
            ('eps_p_mag', 'Plastic Strain', style_params['eps_p_mag_cmap'], None),
            ('sigma_y', 'Yield Stress (MPa)', style_params['sigma_y_cmap'], None),
        ]
       
        n_fields = len(fields_to_plot)          # now 8
        cols = min(3, n_fields)                 # 3 columns
        rows = (n_fields + cols - 1) // cols    # 3 rows (8 fields -> 3×3)
        ...
        for idx, (field_name, title, default_cmap, vrange) in enumerate(fields_to_plot):
            ...
            # -----------------------------------------------------------------
            # Masked twin boundary contours: only where η₁ > 0.5
            # -----------------------------------------------------------------
            if field_name != 'phi' and 'phi' in results_dict and 'eta1' in results_dict:
                phi_data = results_dict['phi']
                eta1_data = results_dict['eta1']
                # Mask φ where η₁ ≤ 0.5
                phi_masked = np.ma.masked_where(eta1_data <= 0.5, phi_data)
            ...
            # Set appropriate labels and format ticks without scientific notation
            ...
            # Prevent scientific notation on colorbar ticks
            ...
            # Add scale bar for selected fields
            if field_name in ['phi', 'sigma_eq', 'sigma_h', 'eta1', 'eta2']:
    ...
    # ========================================================================
    # Plotly 2D Interactive Visualization (with masked contours)
    # ========================================================================
    @handle_errors
    def create_plotly_heatmap(self, results_dict, field_name, frame_idx=0):
        ...
        # Add twin boundary contour (φ=0) only where η₁ > 0.5
        if field_name != 'phi' and 'phi' in results_dict and 'eta1' in results_dict:
            phi_data = results_dict['phi']
            eta1_data = results_dict['eta1']
            # Mask φ where η₁ ≤ 0.5
            phi_masked = np.where(eta1_data > 0.5, phi_data, np.nan)
...
def main():
    ...
    • <span style="color: green;">GRAIN ORDER VISUALIZATION:</span> η₁ and η₂ now appear in the multi‑field comparison.<br>
    • <span style="color: green;">MASKED TWIN BOUNDARIES:</span> φ = 0 contours are only drawn where η₁ > 0.5 (no spurious lines).<br>
    • <span style="color: green;">CLEAN COLORBAR TICKS:</span> No more scientific notation on stress/MPa plots.
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        if operation_mode == "Run New Simulation":
            ...
            # ================================================================
            # Visualization settings (including eta1/eta2 colormaps)
            # ================================================================
            ...
            # NEW: colormaps for grain order parameters
            sim_cmap_eta1 = st.selectbox("η₁ colormap", cmap_list, index=cmap_list.index('Reds') if 'Reds' in cmap_list else 0)
            sim_cmap_eta2 = st.selectbox("η₂ colormap", cmap_list, index=cmap_list.index('Blues') if 'Blues' in cmap_list else 0)
            ...
            if st.button("🚀 Initialize Simulation", type="primary", use_container_width=True):
                params = {
                    ...
                    'cmap_eta1': sim_cmap_eta1,      # NEW
                    'cmap_eta2': sim_cmap_eta2,      # NEW
            ...
            if not simulations:
                ...
                field_to_compare = st.selectbox(
                    ...
                    ["phi (Twin Order)", "sigma_eq (Von Mises Stress)", "sigma_h (Hydrostatic Stress)",
                     "h (Twin Spacing)", "sigma_y (Yield Stress)", "eta1 (Grain Order)", "eta2 (Grain Order)"],
                    index=1
                )
    ...
    if operation_mode == "Compare Saved Simulations" and 'comparison_config' in st.session_state:
        # ----------------------------------------------
        # COMPARISON DISPLAY (unchanged, but now supports eta1/eta2)
        # ----------------------------------------------
        ...
        if not simulations:
            ...
            # (Comparison sub‑modes remain as in original code; omitted for brevity)
        ...
        # ----------------------------------------------
        # SINGLE SIMULATION VIEW (with animation, now shows eta1/eta2 in multi‑field plot)
        # ----------------------------------------------
        ...
        if sim_data:
            ...
            if history:
                ...
                # Visualize using enhanced visualizer (now includes eta1/eta2 and masked contours)
                ...
                style_params = {
                    ...
                    'eta2_cmap': params.get('cmap_eta2', 'Blues'),      # NEW
        ...
        with tabs[0]:  # Initial Geometry
            ...
            h = compute_twin_spacing_numba(phi_gx, phi_gy)
            initial_results = {'phi': phi, 'eta1': eta1, 'eta2': eta2, 'h': h}
           
            style_params = {
                'scalebar_color': params.get('scalebar_color', 'black'),
                'scalebar_fontsize': params.get('scalebar_fontsize', 10),
                'eta1_cmap': params.get('cmap_eta1', 'Reds'),
                'eta2_cmap': params.get('cmap_eta2', 'Blues')
            }
            ...
            if 'results_history' in st.session_state:
                ...
                with col2:
                    field_to_profile = st.selectbox(
                        "Field to Profile",
                        ["phi", "sigma_eq", "sigma_h", "h", "sigma_y", "eta1", "eta2"],
                        index=1
                    )
        ...
        with tabs[4]:  # Plotly Interactive (2D)
            if 'results_history' in st.session_state:
                ...
                plotly_field = st.selectbox(
                    ...
                    ["phi", "sigma_eq", "sigma_h", "h", "eps_p_mag", "sigma_y", "eta1", "eta2"],
        ...
        with tabs[5]:
            ...
            if 'results_history' in st.session_state:
                ...
                # Field selector for 3D (now includes eta1/eta2)
